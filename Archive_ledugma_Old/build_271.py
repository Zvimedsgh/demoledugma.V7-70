...
for line in lines:
    if '3760 Set shp = wsMain.Shapes.AddShape' in line:
        skip = True
    if skip and '3840 shp.OnAction' in line and 'ToggleHiddenSheets' in line:
        skip = False
        continue
    
    if 'Add Toggle Layout Button' in line:
        skip = True
    if skip and '3960 shpExit.OnAction' in line and 'ExitSystem' in line:
        skip = False
        continue

    if 'Add Left Button: Operating Manual' in line:
        skip = True
    if skip and 'shpManual.OnAction' in line and 'OpenManualSheet' in line:
        skip = False
        continue
...
# Update version
content = content.replace('APP_VERSION As String = "2.269"', 'APP_VERSION As String = "2.271"')
content = content.replace('VERSION: V2.269', 'VERSION: V2.271')
content = content.replace('Error in V2.269!', 'Error in V2.271!')
content = content.replace('Attribute VB_Name = "Goren_Claude_V2_269"', 'Attribute VB_Name = "Goren_Claude_V2_271"')

changelog = """' CHANGES IN 2.271:
...
content = content.replace("3975 wsMain.Range(\"A16\").Value", new_system_buttons + "\n    3975 wsMain.Range(\"A16\").Value")
...
with open(r'c:\LEVAV PROJECT\SOURCE\Goren_Claude_V2.271.bas', 'w', encoding='utf-8') as f:
    f.write(content)
print('Generated 271')
