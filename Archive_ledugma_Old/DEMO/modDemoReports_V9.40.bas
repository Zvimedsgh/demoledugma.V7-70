...
' ============================================================================
' MODULE: modReports' VERSION: 9.40 (2026-06-29)
' PURPOSE: Complete reporting system - BuildReview + ApplyCorrectionsAndBuildReports
'          VERSION 9.40
' ============================================================================
' CHANGES IN 9.40:
'   - Guaranteed physical Row Insert by removing IF condition. Macro renamed to A00_SetupMainSheet_V9_40.
' CHANGES IN 9.39:
'   - Renamed SetupMainSheet to A00_SetupMainSheet so it appears at top of Macro list
' CHANGES IN 9.37:
'   - Shifted entire wsMain layout down by 1 row to accommodate Agency Name in row 1.
'   - Added Agency Name from wsMgmt.Range("K46") to A1 in red text.
' CHANGES IN 9.36:
'   - Fixed UpdateFilterValueDropdown (range reference for valList)
'   - Changed BuildPresentation processing message to F16
' CHANGES IN 9.34:
'   - Completely removed PositionNextMsgBox calls so messages center naturally on the screen
' CHANGES IN 9.33:
'   - Fixed PositionNextMsgBox coordinates to 400, 80 so they don't hide the buttons in B2
' CHANGES IN 9.32:
...
' V9.40 (2026-06-29)
' - Guaranteed physical Row Insert
Public Const SYSTEM_VERSION = "9.40"
...
2785    On Error Resume Next
        With ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Range("F17")
            .Value = ""
...
Private Function GetDollarRate(ByVal wsMain As Worksheet) As Double
    ...
    If FetchBOIRates(rateUSD, rateEUR) Then
        ...
        On Error Resume Next
        If rateUSD > 0 Then wsMain.Range("K4").Value = rateUSD
        If rateEUR > 0 Then wsMain.Range("K5").Value = rateEUR
        On Error GoTo 0
    ...
    Else
        ...
        GetDollarRate = 1
        On Error Resume Next
        If Not IsBlankValue(wsMain.Range("K4").Value2) Then
            If IsNumeric(wsMain.Range("K4").Value2) Then GetDollarRate = CDbl(wsMain.Range("K4").Value2)
        End If
        On Error GoTo 0
    End If
End Function
...
908     On Error GoTo ERR_HANDLER
        ...
        wsMain.Range("J5").Value = ChrW(1488) & ChrW(1497) & ChrW(1512) & ChrW(1493) & " " & ChrW(8364) ' Euro + € symbol
...
260         MsgBoxU ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & listsName & " " & ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & ChrW(1492) & ChrW(1512) & ChrW(1509) & " " & ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 1 " & ChrW(1514) & ChrW(1495) & ChrW(1497) & ChrW(1500) & ChrW(1492) & ".", vbExclamation
270         GoTo CLEAN_EXIT
280     End If
...
310     If lastR < 2 Then GoTo CLEAN_EXIT
        ...
        ' Build range reference to bypass 255 char limit
320     valList = "='" & listsName & "'!" & wsLists.Range(wsLists.Cells(2, targetCol), wsLists.Cells(lastR, targetCol)).Address
...
90      If periodDetail <> "" Then periodDesc = periodDetail
        ...
        dateType = Trim$(CStr(wsMain.Range("G8").Value2))
        detailBy = Trim$(CStr(wsMain.Range("G11").Value2))
        clientName = Trim$(CStr(wsMain.Range("G13").Value2))
...
1010    With wsMain.Range("F17")
1020        .Value = ""
1030        .Interior.Color = RGB(204, 255, 204)
1040    End With
...
Public Sub DoClientSearch()
    ...
    If dict.count = 1 Then
        ...
        wsMain.Range("G13").Value = dict.keys()(0)
...
Public Sub ConfirmClientSelection()
    ...
    Dim wsMain As Worksheet
    Set wsMain = ThisWorkbook.Worksheets(CONTROL_SHEET_NAME())
    wsMain.Range("G13").Value = selectedName
...
Public Sub ClearClientFilter()
    ...
    Dim wsMain As Worksheet
    Set wsMain = ThisWorkbook.Worksheets(CONTROL_SHEET_NAME())
    wsMain.Range("G13").Value = TXT_BACHAR_I()  ' 7.33: was inline ChrW chain
End Sub
...
Public Sub ClearHomePageSelection()
    ...
    ' G5: sug tkupa = shnati
    wsMain.Range("G6").Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
    ' G6: period value = empty
    wsMain.Range("G7").Value = ""
    ' G7: date type = bordereu
    wsMain.Range("G8").Value = ChrW(1489) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1512) & ChrW(1493)
    ' G9: filter type = bechar/i
    wsMain.Range("G10").Value = TXT_BACHAR_I()
    ' G10: filter value = bechar/i
    wsMain.Range("G11").Value = TXT_BACHAR_I()
    ' G12: client name = bechar/i
    wsMain.Range("G13").Value = TXT_BACHAR_I()
...
' so you don't have to re-run the full A00_SetupMainSheet.
' ============================================================================
Public Sub RebuildHomeButtons()
    ...
    baseLeft = wsMain.Range("H13").Left
    baseTop = wsMain.Range("H13").Top + 1
    h = wsMain.Range("H13").Height - 2
