...
320  lblSugTaarih = ChrW(1505) & ChrW(1493) & ChrW(1490) & " " & ChrW(1514) & ChrW(1488) & ChrW(1512) & ChrW(1497) & ChrW(1498)
    ...
    ' ---- ONE-TIME HARD RESET FOR LAYOUT (V1.63) ----
    Dim permRowCheck As Long
    permRowCheck = 290 + offsetRow
    
    ' If the Permissions header is NOT at its new correct location, it means the sheet is in a messy hybrid state.
    ' We will completely wipe the area from 196 downwards and forcefully recreate the headers.
    If wsMgmt.Range("A" & permRowCheck).Value <> ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514) Then
        ' 1. Wipe everything from 196+offsetRow to 1000
        wsMgmt.Range("A" & (196 + offsetRow) & ":F" & (1000 + offsetRow)).ClearContents
        wsMgmt.Range("A" & (196 + offsetRow) & ":F" & (1000 + offsetRow)).Font.Bold = False
        wsMgmt.Range("A" & (196 + offsetRow) & ":F" & (1000 + offsetRow)).Font.Size = 11
