...
' CHANGES IN 7.54:
'   - Fully converted BuildComparisonSheet to use in-memory Variant Arrays (curData, refDataComp) for massive speedup.
' CHANGES IN 7.53:
...
1417        rowBordMonth = GetBordMonth(wsSrc.Cells(r, dateCol).Value2)
...
3195        rowBordMonth = GetBordMonth(wsRef.Cells(r, dateCol).Value2)
