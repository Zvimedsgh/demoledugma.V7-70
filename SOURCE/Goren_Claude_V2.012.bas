Attribute VB_Name = "Goren_Claude2_007"
' ============================================================================
' MODULE: modLevav
...
Private Const APP_VERSION As String = "2.012"
...
110                 Dim wsPeriods As Worksheet
                Set wsPeriods = ThisWorkbook.Worksheets(H_SET_PERIODS())
                monthName = Trim$(CStr(wsPeriods.Cells(15 + monthIdx, 1).Value2))
...
280     Dim rng As Range
        ...
        If CStr(wsMsgSrc.Cells(1 + 20, 1).Value) <> "1" Then
            confirmMsg = CStr(wsMsgSrc.Cells(1 + 13, 1).Value) & vbNewLine & vbNewLine & CStr(wsMsgSrc.Cells(1 + 14, 1).Value) & vbNewLine & vbNewLine & CStr(wsMsgSrc.Cells(1 + 15, 1).Value) & vbNewLine & vbNewLine & CStr(wsMsgSrc.Cells(1 + 16, 1).Value)
            If MsgBoxU(confirmMsg, vbOKCancel + vbExclamation, CStr(wsMsgSrc.Cells(1 + 8, 1).Value)) <> vbOK Then
                Exit Sub
            End If
            ' Ask if user wants to keep showing this message
            If MsgBoxU(CStr(wsMsgSrc.Cells(1 + 17, 1).Value), vbYesNo + vbQuestion) = vbNo Then
                wsMsgSrc.Cells(1 + 20, 1).Value = "1"
            End If
        End If
...
1240    If outRow > 3 Then
            ...
            MsgBoxU wsMsgSrc.Cells(1 + 2, 1).Value & (outRow - 3) & wsMsgSrc.Cells(1 + 3, 1).Value, vbInformation
        Else
            ' No issues found - go straight to home
            MsgBoxU wsMsgSrc.Cells(1 + 4, 1).Value, vbInformation
...
1360    MsgBoxU wsMsgSrc.Cells(1 + 5, 1).Value & errLine & vbCrLf & wsMsgSrc.Cells(1 + 6, 1).Value & errNum & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
...
2960    MsgBoxU wsMsgSrc2.Cells(1 + 5, 1).Value & errLine & vbCrLf & wsMsgSrc2.Cells(1 + 6, 1).Value & errNum & vbCrLf & wsMsgSrc2.Cells(1 + 7, 1).Value & debugStep & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
