...
375    ' --- Delete all generated sheets safely ---
ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Activate
Dim yearCurr As String
Dim yearBase As String

yearCurr = Trim$(CStr(Range("rngCurrentYear").Value))
yearBase = Trim$(CStr(Range("rngBaseYear").Value))

For iSheet = ThisWorkbook.Worksheets.Count To 1 Step -1

    Set wsCleanup = ThisWorkbook.Worksheets(iSheet)

    If wsCleanup.Name <> CONTROL_SHEET_NAME() _
       And wsCleanup.Name <> MANAGEMENT_SHEET_NAME() _
       And wsCleanup.Name <> refBaseKeep _
       And wsCleanup.Name <> refBaseKeepOld Then

        If ThisWorkbook.Worksheets.Count > 1 Then
            wsCleanup.Visible = xlSheetVisible
            wsCleanup.Delete
        End If
...
510     yearVal = Trim$(CStr(Range("rngCurrentYear").Value))
520        If yearVal = "" Then Err.Raise vbObjectError + 1002, "BuildReview", "rngCurrentYear IS EMPTY"
...
450     With wsProgress.Range("B8")
452            .Value = ChrW(1502) & ChrW(1506) & ChrW(1489) & ChrW(1491) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & "," & " " & ChrW(1488) & ChrW(1504) & ChrW(1488) & " " & ChrW(1492) & ChrW(1502) & ChrW(1514) & ChrW(1503) & "/" & ChrW(1497) & "." & "." & "."
454            .Font.Size = 20
            .Font.Bold = True
...
600     yearVal = Trim$(CStr(Range("rngCurrentYear").Value))
610     refYear = Trim$(CStr(Range("rngBaseYear").Value))

620     If yearVal = "" Or refYear = "" Then
621         Err.Raise vbObjectError + 2001, "ApplyCorrections", "Year parameters are missing"
622     End If
...
2795    If Trim$(CStr(wsMsgSrc2.Cells(18, 19).Value)) <> "" Then
2796        MsgBoxU CStr(wsMsgSrc2.Cells(18, 19).Value), vbInformation
2797    Else
2798        MsgBoxU ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491) & " " & ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1497) & ChrW(1497) & ChrW(1501), vbInformation
2799    End If
2800    Exit Sub
