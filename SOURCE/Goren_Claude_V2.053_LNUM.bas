...
280 If chartItems > 15 Then chartItems = 15
...
300 Set tmpWs = ThisWorkbook.Worksheets.Add

        ' ---- Chart 1: Premiums ----
310 tmpWs.Cells(1, 1).Value = ""
320 tmpWs.Cells(1, 2).Value = refYear
330 tmpWs.Cells(1, 3).Value = yearVal
340 For ci = 1 To chartItems
350 tmpWs.Cells(ci + 1, 1).Value = arrNames(ci)
360 tmpWs.Cells(ci + 1, 2).Value = arrPremR(ci)
370 tmpWs.Cells(ci + 1, 3).Value = arrPremC(ci)
380 Next ci

390 Set co = tmpWs.ChartObjects.Add(10, 10, 600, 350)
400 Set xlCht = co.Chart
410 xlCht.ChartType = 51
420 xlCht.SetSourceData tmpWs.Range(tmpWs.Cells(1, 1), tmpWs.Cells(chartItems + 1, 3)), 2  ' xlColumns
430 xlCht.HasTitle = False
440 xlCht.HasLegend = True

        ' --- Colors: Yellow=ref year, Blue=current year ---
450 xlCht.SeriesCollection(1).Format.Fill.ForeColor.RGB = RGB(255, 192, 0)     ' yellow/gold
460 xlCht.SeriesCollection(2).Format.Fill.ForeColor.RGB = RGB(68, 114, 196)    ' blue

        ' --- Data labels (show in K, vertical/upward) ---
        Dim sP As Long
470 For sP = 1 To 2
480 xlCht.SeriesCollection(sP).HasDataLabels = True
490 xlCht.SeriesCollection(sP).DataLabels.NumberFormat = "#,##0,""K"""
500 xlCht.SeriesCollection(sP).DataLabels.Font.Size = 9
510 xlCht.SeriesCollection(sP).DataLabels.Orientation = 90
520 Next sP

        ' --- Y-axis number format (in K) ---
530 xlCht.Axes(2).TickLabels.NumberFormat = "#,##0,""K"""

540 xlCht.Export imgPrem

550 tmpWs.ChartObjects.Delete
560 tmpWs.Cells.Clear

        ' ---- Chart 2: Commissions ----
570 tmpWs.Cells(1, 1).Value = ""
580 tmpWs.Cells(1, 2).Value = refYear
590 tmpWs.Cells(1, 3).Value = yearVal
600 For ci = 1 To chartItems
610 tmpWs.Cells(ci + 1, 1).Value = arrNames(ci)
620 tmpWs.Cells(ci + 1, 2).Value = arrCommR(ci)
630 tmpWs.Cells(ci + 1, 3).Value = arrCommC(ci)
640 Next ci

650 Set co = tmpWs.ChartObjects.Add(10, 10, 600, 350)
660 Set xlCht = co.Chart
670 xlCht.ChartType = 51
680 xlCht.SetSourceData tmpWs.Range(tmpWs.Cells(1, 1), tmpWs.Cells(chartItems + 1, 3)), 2  ' xlColumns
690 xlCht.HasTitle = False
700 xlCht.HasLegend = True

        ' --- Colors: Orange=ref year, Green=current year ---
710 xlCht.SeriesCollection(1).Format.Fill.ForeColor.RGB = RGB(237, 125, 49)    ' orange
720 xlCht.SeriesCollection(2).Format.Fill.ForeColor.RGB = RGB(112, 173, 71)    ' green

        ' --- Data labels (show in K, vertical/upward) ---
        Dim sC As Long
730 For sC = 1 To 2
740 xlCht.SeriesCollection(sC).HasDataLabels = True
750 xlCht.SeriesCollection(sC).DataLabels.NumberFormat = "#,##0,""K"""
760 xlCht.SeriesCollection(sC).DataLabels.Font.Size = 9
770 xlCht.SeriesCollection(sC).DataLabels.Orientation = 90
780 Next sC

        ' --- Y-axis number format (in K) ---
790 xlCht.Axes(2).TickLabels.NumberFormat = "#,##0,""K"""

800 xlCht.Export imgComm

        ' ---- Chart 3: Documents (optional) ----
810 If imgDocs <> "" Then
820 tmpWs.ChartObjects.Delete
830 tmpWs.Cells.Clear
840 tmpWs.Cells(1, 1).Value = ""
850 tmpWs.Cells(1, 2).Value = refYear
860 tmpWs.Cells(1, 3).Value = yearVal
870 For ci = 1 To chartItems
880 tmpWs.Cells(ci + 1, 1).Value = arrNames(ci)
890 tmpWs.Cells(ci + 1, 2).Value = arrDocsR(ci)
900 tmpWs.Cells(ci + 1, 3).Value = arrDocsC(ci)
910 Next ci
920 Set co = tmpWs.ChartObjects.Add(10, 10, 600, 350)
930 Set xlCht = co.Chart
940 xlCht.ChartType = 51
950 xlCht.SetSourceData tmpWs.Range(tmpWs.Cells(1, 1), tmpWs.Cells(chartItems + 1, 3)), 2
960 xlCht.HasTitle = False
970 xlCht.HasLegend = True
980 xlCht.SeriesCollection(1).Format.Fill.ForeColor.RGB = RGB(180, 130, 70)    ' brown/tan
990 xlCht.SeriesCollection(2).Format.Fill.ForeColor.RGB = RGB(91, 155, 213)    ' steel blue
            Dim sD As Long
1000 For sD = 1 To 2
1010 xlCht.SeriesCollection(sD).HasDataLabels = True
1020 xlCht.SeriesCollection(sD).DataLabels.NumberFormat = "#,##0"
1030 xlCht.SeriesCollection(sD).DataLabels.Font.Size = 9
1040 xlCht.SeriesCollection(sD).DataLabels.Orientation = 90
1050 Next sD
1060 xlCht.Axes(2).TickLabels.NumberFormat = "#,##0"
1070 xlCht.Export imgDocs
1080 End If

        ' ---- Chart 4: Insured persons (optional) ----
1090 If imgInsured <> "" Then
1100 tmpWs.ChartObjects.Delete
1110 tmpWs.Cells.Clear
1120 tmpWs.Cells(1, 1).Value = ""
1130 tmpWs.Cells(1, 2).Value = refYear
1140 tmpWs.Cells(1, 3).Value = yearVal
1150 For ci = 1 To chartItems
1160 tmpWs.Cells(ci + 1, 1).Value = arrNames(ci)
1170 tmpWs.Cells(ci + 1, 2).Value = arrInsR(ci)
1180 tmpWs.Cells(ci + 1, 3).Value = arrInsC(ci)
1190 Next ci
1200 Set co = tmpWs.ChartObjects.Add(10, 10, 600, 350)
1210 Set xlCht = co.Chart
1220 xlCht.ChartType = 51
1230 xlCht.SetSourceData tmpWs.Range(tmpWs.Cells(1, 1), tmpWs.Cells(chartItems + 1, 3)), 2
1240 xlCht.HasTitle = False
1250 xlCht.HasLegend = True
1260 xlCht.SeriesCollection(1).Format.Fill.ForeColor.RGB = RGB(128, 0, 128)     ' purple
1270 xlCht.SeriesCollection(2).Format.Fill.ForeColor.RGB = RGB(0, 176, 80)      ' green
            Dim si As Long
1280 For si = 1 To 2
1290 xlCht.SeriesCollection(si).HasDataLabels = True
1300 xlCht.SeriesCollection(si).DataLabels.NumberFormat = "#,##0"
1310 xlCht.SeriesCollection(si).DataLabels.Font.Size = 9
1320 xlCht.SeriesCollection(si).DataLabels.Orientation = 90
1330 Next si
1340 xlCht.Axes(2).TickLabels.NumberFormat = "#,##0"
1350 xlCht.Export imgInsured
1360 End If

1370 Application.DisplayAlerts = False
1380 tmpWs.Delete
1390 Application.DisplayAlerts = True

1400 Exit Sub
1410 ERR_HANDLER:
1420 Application.EnableEvents = True
1430 Application.DisplayAlerts = True
620     On Error Resume Next
1440 Application.DisplayAlerts = False
1450 If Not tmpWs Is Nothing Then tmpWs.Delete
1460 Application.DisplayAlerts = True
1470 Err.Raise Err.Number, "ExportCompCharts(" & sheetName & "):" & Erl, Err.Description
End Sub
