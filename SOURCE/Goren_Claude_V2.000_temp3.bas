...
936                 outRow = outRow + 1
                    ...
                    On Error Resume Next
                    brEndRow = ThisWorkbook.Names("rngSection_Params").RefersToRange.Row - 1
                    On Error GoTo ERR_HANDLER
...
960                     corrCount = corrCount + 1
                        ...
                        If InStr(1, reasonText, unknownBrText, vbTextCompare) > 0 And fixText <> "" Then
                            ...
                            On Error Resume Next
                            brFixLast = ThisWorkbook.Names("rngSection_Params").RefersToRange.Row - 1
                            On Error GoTo 0
...
Public Sub SetupSectionNamedRanges()
    ...
    ' --- Delete ALL existing rngSection_* names ---
    On Error Resume Next
    For Each nm In ThisWorkbook.Names
        If LCase$(Left$(nm.Name, 10)) = "rngsection" Then nm.Delete
        If LCase$(Left$(nm.Name, 10)) = "rngsrction" Then nm.Delete
    Next nm
    ThisWorkbook.Names("rngFILES_FOLDER").Delete
    ThisWorkbook.Names("rngREPORTS_FOLDER").Delete
    Err.Clear
    ...
    ' --- Create section Named Ranges Dynamically by searching for Headers ---
    SetDynamicNamedRange wsMgmt, "rngSection_FieldMap", ChrW(1502) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1497), 1 + offsetRow
    ...
    On Error Resume Next
    paramRow = ThisWorkbook.Names("rngSection_Params").RefersToRange.Row
    On Error GoTo 0
    If paramRow > 0 Then
        ThisWorkbook.Names.Add "rngFILES_FOLDER", wsMgmt.Range("B" & paramRow + 3)
        ThisWorkbook.Names.Add "rngREPORTS_FOLDER", wsMgmt.Range("B" & paramRow + 4)
    Else
...
380 wsMgmt.Cells(startRow, 1).Font.Size = 12
    ...
    wsMgmt.Range("A2:A" & wsMgmt.Rows.Count).ClearContents
    
420 For i = 0 To UBound(arrAll)
430     wsMgmt.Cells(startRow + 1 + i, 1).Value = arrAll(i)
440 Next i
    wsMgmt.Cells(startRow + 1 + UBound(arrAll) + 1, 1).Value = EOD_MARKER
