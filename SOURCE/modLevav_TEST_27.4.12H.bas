...
Public Sub RunLevav_TEST()
    ...
    T_ProcessCompanySheet wsBase, branchDict, 11, 19, 20, 4, 7, 18, 0
    T_ProcessCompanySheet wsCurrentFixed, branchDict, 11, 19, 20, 4, 7, 18, 1
    Call T_ProcessMainBranchSheet(wsBase, mainBranchDict, 11, 19, 20, 4, 7, 18, 0)
    Call T_ProcessMainBranchSheet(wsCurrentFixed, mainBranchDict, 11, 19, 20, 4, 7, 18, 1)
    
    T_WriteCompaniesOutput dict, baseYear, currentYear
   T_WriteBranchesOutput branchDict, baseYear, currentYear, "branches_comparison"
    
    T_WriteBranchesOutput mainBranchDict, baseYear, currentYear, "main_branch_comparison"
...
colID = T_GetMappedColumnIndex("ID_NUMBER")
colPolicy = T_GetMappedColumnIndex("POLICY")
colCompany = T_GetMappedColumnIndex("COMPANY_NAME")
colAction = T_GetMappedColumnIndex("ACTION")
colPremium = T_GetMappedColumnIndex("PREMIUM")
colCommission = T_GetMappedColumnIndex("COMPANY_COMMISSION")
colBranch = T_GetMappedColumnIndex("BRANCH_NAME")
colAgent = T_GetMappedColumnIndex("AGENT_NAME")
...
Private Sub T_ProcessCompanySheet( _
    ...
    For r = 2 To lastRow
        companyKey = T_GetMainBranchName(Trim(CStr(ws.Cells(r, colCompany).Value)))
...
Private Sub T_ProcessMainBranchSheet( _
    ByVal ws As Worksheet, _
    ByVal dict As Object, _
    ByVal colCompany As Long, _
    ByVal colPremium As Long, _
...
Private Function T_GetMainBranchName(ByVal branchName As String) As String
    ...
    For Each r In rng.Rows
        If Trim(CStr(r.Cells(1, 1).Value)) = cleanBranch Then

            If Len(Trim(CStr(r.Cells(1, 2).Value))) > 0 Then
                T_GetMainBranchName = Trim(CStr(r.Cells(1, 2).Value))
            Else
...
End Function
Private Sub T_WriteBranchesOutput(ByVal dict As Object, ByVal baseYear As Variant, ByVal currentYear As Variant, ByVal outputSheetName As String)
    Dim wsOut As Worksheet
    Dim outRow As Long, totalRow As Long
    Dim k As Variant, arr As Variant
...
Private Sub T_WriteMainBranchesOutput(ByVal dict As Object, ByVal baseYear As Variant, ByVal currentYear As Variant)
    Dim wsOut As Worksheet
    Dim outRow As Long, totalRow As Long
    Dim k As Variant, arr As Variant

    Set wsOut = T_RecreateSheet("main_branch_comparison")
    wsOut.DisplayRightToLeft = True
...
Private Function T_RecreateSheet(ByVal sheetName As String) As Worksheet
    ...
    If Err.Number <> 0 Then
        ws.Name = "tmp_" & Format(Now, "hhmmss")
        Err.Clear
    End If
    On Error GoTo 0
...
End Function
Private Function T_GetMessage(ByVal messageKey As String) As String
    ...
    For r = 1 To lastRow
        If Trim(CStr(ws.Cells(r, "T").Value)) = messageKey Then
            T_GetMessage = Trim(CStr(ws.Cells(r, "S").Value))
            Exit Function
        End If
    Next r

    T_GetMessage = messageKey
