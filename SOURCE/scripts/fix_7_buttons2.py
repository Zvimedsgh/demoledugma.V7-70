...
# Replace buttons block in A00_SetupMainSheet
start_btn = -1
end_btn = -1
for i, line in enumerate(lines):
    if '2880 Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, btnLeft, wsMain.Range("C2").Top' in line:
        start_btn = i
    if '3410 shp.OnAction = "NewClients"' in line and start_btn != -1:
        end_btn = i + 1
        break

new_buttons_code = """    2880 Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, btnLeft, wsMain.Range("C2").Top, btnW, btnH)
...
if start_btn != -1 and end_btn != -1:
    new_lines = lines[:start_btn]
    new_lines.append(new_buttons_code)
    new_lines.extend(lines[end_btn:])
    lines = new_lines
else:
    print("Failed to find btn block!")

# Replace AssignButtonMacros block
start_assign = -1
end_assign = -1
for i, line in enumerate(lines):
    if 'Case "btnBuildReview": shp.OnAction = "BuildReview"' in line:
        start_assign = i
    if 'Case "btnNewClients": shp.OnAction = "NewClients"' in line and start_assign != -1:
        end_assign = i + 1
        break

new_assign_code = """        Case "btnBuildReview": shp.OnAction = "BuildReview"
...
if start_assign != -1 and end_assign != -1:
    new_lines = lines[:start_assign]
    new_lines.append(new_assign_code)
    new_lines.extend(lines[end_assign:])
    lines = new_lines
else:
    print("Failed to find assign block!")

for i in range(len(lines)):
    lines[i] = lines[i].replace('Attribute VB_Name = "Goren_Claude_V2_170"', 'Attribute VB_Name = "Goren_Claude_V2_171"')
    lines[i] = lines[i].replace('VERSION: V2.170', 'VERSION: V2.171')
    lines[i] = lines[i].replace('APP_VERSION As String = "2.170"', 'APP_VERSION As String = "2.171"')

with open(r'c:\LEVAV PROJECT\SOURCE\Goren_Claude_V2.171.bas', 'w', encoding='utf-8') as f:
    f.writelines(lines)

print("Created V2.171 properly.")
