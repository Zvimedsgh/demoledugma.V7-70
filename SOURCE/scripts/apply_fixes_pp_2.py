...
start_idx = content.find(target_slide)

if start_idx != -1:
    end_str = "On Error GoTo ERR_HANDLER\n910     Set ppPres = Nothing"
    end_idx = content.find(end_str, start_idx)
    if end_idx != -1:
        block = content[start_idx:end_idx]
        new_block = "' Automatic slideshow removed (will prompt user instead at the end)\n"
        content = content.replace(block, new_block)
        print("Replaced target_slide")
    else:
...
print("V2.121 created via manual search.")
