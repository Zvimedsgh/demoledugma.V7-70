...
Public Sub SetupSettingsMenu()
    ...
    ' In RTL, Left is distance from the left edge of the sheet, so A1 has a large Left value.
    startLeft = wsMgmt.Range("A1").Left + wsMgmt.Range("A1").Width
    
    ' We place the first button 10 points from the right margin
    btnLeft = startLeft - btnW - 10
    ...
    ' Move LEFT for the next button (subtract gap)
    ...
    ' Reset btnLeft to rightmost position for ROW 2
    btnLeft = startLeft - btnW - 10
