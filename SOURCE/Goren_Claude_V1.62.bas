Attribute VB_Name = "Goren_Claude1_46"
' ============================================================================
' MODULE: modLevav
...
Private Const APP_VERSION As String = "1.46"
...
290     On Error GoTo ERR_HANDLER
        ...
        wsProgress1.Unprotect
        wsProgress1.Range("G15:K15").UnMerge
        wsProgress1.Range("G15:K15").Merge
        With wsProgress1.Range("G15")
            .Value = ChrW(1502) & ChrW(1506) & ChrW(1489) & ChrW(1491) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1488) & ChrW(1504) & ChrW(1488) & " " & ChrW(1492) & ChrW(1502) & ChrW(1514) & ChrW(1497) & ChrW(1504) & ChrW(1504) & ChrW(1493) & "..."
            .Font.Size = 18
            .Font.Bold = True
...
2785    On Error Resume Next
        ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Unprotect
...
320  lblSugTaarih = ChrW(1505) & ChrW(1493) & ChrW(1490) & " " & ChrW(1514) & ChrW(1488) & ChrW(1512) & ChrW(1497) & ChrW(1498)
    ...
    ' ---- ONE-TIME CLEANUP (V1.60) ----
    Call MigrateSettingsLayout
    ' ---- RESCUE MISSING HEADERS (V1.62) ----
    ' In V1.60, the Messages section accidentally overwrote the Permissions and Clients sections.
    ' We must force the headers to appear at the new correct locations (290 and 299) if they are missing.
    
    Dim permRow As Long, clientRow As Long
    permRow = 290 + offsetRow
    clientRow = 299 + offsetRow
    
    If wsMgmt.Range("A" & permRow).Value <> ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514) Then
        wsMgmt.Range("A" & permRow).Value = ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514)
        wsMgmt.Range("A" & permRow).Font.Bold = True
        wsMgmt.Range("A" & permRow).Font.Size = 12
        If wsMgmt.Range("A" & permRow + 1).Value = "" Then wsMgmt.Range("A" & permRow + 1).Value = "EOD"
    End If
    
    If wsMgmt.Range("A" & clientRow).Value <> ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) Then
        wsMgmt.Range("A" & clientRow).Value = ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514)
        wsMgmt.Range("A" & clientRow).Font.Bold = True
        wsMgmt.Range("A" & clientRow).Font.Size = 12
        If wsMgmt.Range("A" & clientRow + 1).Value = "" Then wsMgmt.Range("A" & clientRow + 1).Value = "EOD"
    End If
    
    ' Also force bold header for Messages
    Dim msgRow As Long
    msgRow = 241 + offsetRow
    wsMgmt.Range("A" & msgRow).Value = ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1493) & ChrW(1514) & " " & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1514)
    wsMgmt.Range("A" & msgRow).Font.Bold = True
    wsMgmt.Range("A" & msgRow).Font.Size = 12
...
Public Sub CheckUserPermissions()
    ...
    If Len(creditVersionCell) > 0 And creditVersionCell <> "A23" Then
        ...
        Err.Clear
    End If
    wsMain.Range("A21").Value = creditText & " - v" & APP_VERSION
    wsMain.Range("A21").Font.Size = 10
    wsMain.Range("A21").Font.Color = RGB(150, 150, 150)
    wsMain.Range("A21").Font.Bold = False
    wsMain.Range("A21").HorizontalAlignment = xlLeft
...
Public Sub SetupSectionNamedRanges()
    ...
    ThisWorkbook.Names.Add "rngSection_Messages", wsMgmt.Range("A" & 241 + offsetRow)
      ThisWorkbook.Names.Add "rngSection_Permissions", wsMgmt.Range("A" & 290 + offsetRow)
      ThisWorkbook.Names.Add "rngSection_Clients", wsMgmt.Range("A" & 299 + offsetRow)
...
' ============================================================================
Public Sub MigrateSettingsLayout()
    Dim wsMgmt As Worksheet
    On Error Resume Next
    Set wsMgmt = ThisWorkbook.Worksheets(MANAGEMENT_SHEET_NAME())
    On Error GoTo 0
    ...
    ' Check if Permissions is at the old location (A240+offsetRow)
    Dim pCell As Range
    Set pCell = wsMgmt.Range("A" & 240 + offsetRow)
    
    Dim permText As String
    permText = ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514)
    
    If pCell.Value = permText Then
        Application.ScreenUpdating = False
        
        ' 1. Push Permissions and Clients down by 50 rows (insert 30 at 235, 20 at 220)
        wsMgmt.Rows((235 + offsetRow) & ":" & (264 + offsetRow)).Insert Shift:=-4121 ' xlDown
        wsMgmt.Rows((220 + offsetRow) & ":" & (239 + offsetRow)).Insert Shift:=-4121 ' xlDown
        
        ' 2. Clear the old mess (between 196+offsetRow and 289+offsetRow)
        wsMgmt.Range("A" & (196 + offsetRow) & ":E" & (289 + offsetRow)).ClearContents
