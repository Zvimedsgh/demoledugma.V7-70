...
End Function
Public Sub BuildReportsAndCharts_OLD()
    ...
    CreateReportFromTemplate "חברות"
    CreateReportFromTemplate "חודשים"
    CreateReportFromTemplate "סוכנים"
    CreateReportFromTemplate "טלרים"

    Application.DisplayAlerts = True
    Application.EnableEvents = True
    Application.ScreenUpdating = True

    MsgBox "Reports created from templates.", vbInformation
    Exit Sub
...
ERR_HANDLER:
    ...
    Application.EnableEvents = True
    Application.ScreenUpdating = True
    MsgBox "BuildReportsAndCharts failed: " & Err.Description, vbExclamation

End Sub

Private Function CreateReportFromTemplate(ByVal reportName As String) As Worksheet

    Dim templateName As String
    Dim wsTemplate As Worksheet
    Dim wsNew As Worksheet

    templateName = reportName & "_TEMPLATE"

    If Not SheetExists(templateName) Then
        Err.Raise vbObjectError + 1400, , "Missing template sheet: " & templateName
    End If
    ...
    Set CreateReportFromTemplate = wsNew
