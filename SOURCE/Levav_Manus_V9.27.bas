Attribute VB_Name = "Levav_Manus_V9_27"
' ============================================================================
' MODULE: modLevav
...
'   - New: Home page layout: J3=$, K3=USD rate, J4=€, K4=EUR rate, J5:K7=info message
...
' CHANGES IN 8.04:
'   - Fixed: ApplyUserPermissions now disables buttons (grey + no action) instead of hiding
...
280     Dim rng As Range
        ...
        chkSheetName = ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505) & "_" & chkBaseYear
        If SheetExists(chkSheetName) Or SheetExists("base_" & chkBaseYear) Then
            ...
            alreadyMsg = ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1488) & ChrW(1500) & ChrW(1493) & " " & ChrW(1499) & ChrW(1489) & ChrW(1512) & " " & ChrW(1504) & ChrW(1489) & ChrW(1491) & ChrW(1511) & ChrW(1493) & "." & vbCrLf & vbCrLf & _
                ...
                ChrW(1513) & ChrW(1504) & ChrW(1492) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1497) & ChrW(1501) & " " & ChrW(1513) & ChrW(1500) & " " & ChrW(1513) & ChrW(1504) & ChrW(1514) & " " & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505) & " " & ChrW(1493) & ChrW(1513) & ChrW(1504) & ChrW(1492) & " " & ChrW(1504) & ChrW(1493) & ChrW(1499) & ChrW(1495) & ChrW(1497) & ChrW(1514) & "." & vbCrLf & vbCrLf & _
...
1247    wsProgress1.Range("G15:K15").Interior.ColorIndex = xlNone
1250    Exit Sub
...
3170    outRow = 2
        ...
        Dim baseDataRef As Variant
        If lastRow >= 2 Then
            refData = wsRef.Range(wsRef.Cells(1, 1), wsRef.Cells(lastRow, 50)).Value2
            ReDim baseDataRef(1 To lastRow, 1 To BASE_COL_TOFIX)
        Else
            ReDim baseDataRef(1 To 1, 1 To BASE_COL_TOFIX)
        End If
...
3190        premVal = 0
3192        commVal = 0
3194        currCode = refData(r, RAW_CURRENCY)
            ' Per-row dollar rate by bordero month (ref year)
3195        rowBordMonth = GetBordMonth(refData(r, dateCol))
3196        If rowBordMonth <> "" Then
3197            rowRate = GetDollarRate(wsMain, rowBordMonth, True)
            Else
                rowRate = dollarRate
            End If
3200        If Not IsBlankValue(refData(r, RAW_PREMIUM)) Then
3210            If TryParseVariantNumber(refData(r, RAW_PREMIUM), premVal) Then
                    ' Currency conversion: 1=dollar (per-month rate)
3211                If IsNumeric(currCode) Then
3212                    If CLng(currCode) = 1 Then premVal = premVal * rowRate
3213                End If
3214                If Abs(premVal) > threshold And Not dictRefCorr.Exists(CStr(r)) Then GoTo NextRefRow
3215            End If
3216        End If
            ' Convert ref commission too (per-month rate)
3218        If Not IsBlankValue(refData(r, RAW_COMMISSION)) Then
3220            If TryParseVariantNumber(refData(r, RAW_COMMISSION), commVal) Then
3222                If IsNumeric(currCode) Then
3224                    If CLng(currCode) = 1 Then commVal = commVal * rowRate
3226                End If
3228            End If
3230        End If

2210        monthVal = 0
2220        bordereu = refData(r, dateCol)
...
2320        baseDataRef(outRow - 1, BASE_COL_ID) = r
2330        baseDataRef(outRow - 1, BASE_COL_YEAR) = refYear
2340        baseDataRef(outRow - 1, BASE_COL_MONTH) = monthVal
2350        baseDataRef(outRow - 1, BASE_COL_IDENTITY) = refData(r, RAW_IDNUMBER)
2360        baseDataRef(outRow - 1, BASE_COL_CUSTOMER) = refData(r, RAW_CUSTOMER)
2370        baseDataRef(outRow - 1, BASE_COL_CUSTNAME) = refData(r, RAW_CUSTNAME)
2380        baseDataRef(outRow - 1, BASE_COL_POLICY) = refData(r, RAW_POLICY)
2390        baseDataRef(outRow - 1, BASE_COL_ADDENDUM) = refData(r, RAW_ADDENDUM)
2400        baseDataRef(outRow - 1, BASE_COL_COMPANY) = refData(r, RAW_COMPANY)
2410        baseDataRef(outRow - 1, BASE_COL_COMPNUM) = refData(r, RAW_COMPNUM)
2420        baseDataRef(outRow - 1, BASE_COL_BRANCHNAME) = refData(r, RAW_BRANCHNAME)
2430        baseDataRef(outRow - 1, BASE_COL_BRANCHNUM) = refData(r, RAW_BRANCHNUM)

2440        brKey = UCase$(Trim$(CStr(refData(r, RAW_BRANCHNAME))))
2450        If dictBranch.Exists(brKey) Then
2460            baseDataRef(outRow - 1, BASE_COL_MAINBRANCH) = dictBranch(brKey)
2470        Else
2480            baseDataRef(outRow - 1, BASE_COL_MAINBRANCH) = refData(r, RAW_BRANCHNAME)
2490        End If

2500        baseDataRef(outRow - 1, BASE_COL_AGENTNAME) = refData(r, RAW_AGENTNAME)
2510        baseDataRef(outRow - 1, BASE_COL_AGENTNUM) = refData(r, RAW_AGENTNUM)
2520        baseDataRef(outRow - 1, BASE_COL_TELLER) = refData(r, RAW_TELLERNAME)
2530        baseDataRef(outRow - 1, BASE_COL_TELLERNUM) = refData(r, RAW_TELLERNUM)
2540        baseDataRef(outRow - 1, BASE_COL_ACTION) = refData(r, RAW_ACTIONCOL)
2550        baseDataRef(outRow - 1, BASE_COL_PREMIUM) = premVal
2560        baseDataRef(outRow - 1, BASE_COL_COMMISSION) = commVal

            ' Apply corrections from ref REVIEW using reason-based mapping
2562        If dictRefCorr.Exists(CStr(r)) Then
2563            baseDataRef(outRow - 1, BASE_COL_ISSUE) = "CORRECTED"
                ' Iterate each reason->fix pair and apply to the correct column
...
2592                            baseDataRef(outRow - 1, BASE_COL_PREMIUM) = fixPrem
2594                        End If
                        ' Commission
...
2600                            baseDataRef(outRow - 1, BASE_COL_COMMISSION) = fixComm
2602                        End If
2604                    End If
2606                End If
2608            Next refFixKey
...
3600    Next r
        ...
        If outRow > 2 Then wsBaseRef.Range(wsBaseRef.Cells(2, 1), wsBaseRef.Cells(outRow - 1, BASE_COL_TOFIX)).Value = baseDataRef
...
158     If Not dict.Exists("MISSING_COMPANY_NAME") Then dict("MISSING_COMPANY_NAME") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1495) & ChrW(1489) & ChrW(1512) & ChrW(1492)
160     If Not dict.Exists("MISSING_BRANCH_NAME") Then dict("MISSING_BRANCH_NAME") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1506) & ChrW(1504) & ChrW(1507)
162     If Not dict.Exists("MISSING_AGENT_NAME") Then dict("MISSING_AGENT_NAME") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1505) & ChrW(1493) & ChrW(1499) & ChrW(1503)
164     If Not dict.Exists("MISSING_UNDERWRITER_TELLER_NAME") Then dict("MISSING_UNDERWRITER_TELLER_NAME") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1496) & ChrW(1500) & ChrW(1512)
...
168     If Not dict.Exists("MISSING_PREMIUM") Then dict("MISSING_PREMIUM") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1497) & ChrW(1492)
170     If Not dict.Exists("MISSING_COMPANY_COMMISSION") Then dict("MISSING_COMPANY_COMMISSION") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1506) & ChrW(1502) & ChrW(1500) & ChrW(1514) & " " & ChrW(1495) & ChrW(1489) & ChrW(1512) & ChrW(1492)
172     If Not dict.Exists("PREMIUM_OVER_THRESHOLD") Then dict("PREMIUM_OVER_THRESHOLD") = ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1497) & ChrW(1492) & " " & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1492)
174     If Not dict.Exists("PREMIUM_NOT_NUMERIC") Then dict("PREMIUM_NOT_NUMERIC") = ChrW(1506) & ChrW(1512) & ChrW(1498) & " " & ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1497) & ChrW(1492) & " " & ChrW(1500) & ChrW(1488) & " " & ChrW(1502) & ChrW(1505) & ChrW(1508) & ChrW(1512) & ChrW(1497)
...
Private Function FetchBOIMonthlyAvg(ByVal currencyKey As String, Optional ByVal monthStr As String = "") As Double
    ...
    ' CSV columns: ...,TIME_PERIOD(12),OBS_VALUE(13),RELEASE_STATUS(14)
...
280  lblShnBasis = ChrW(1513) & ChrW(1504) & ChrW(1514) & " " & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505)
...
330  wsMain.Range("A2:D5").ClearContents

    ' ---- Style row 2 headers (F2, G2, J2, K2) ----
340  Set hdrRng = Union(wsMain.Range("F2"), wsMain.Range("G2"), wsMain.Range("J2"), wsMain.Range("K2"))
350  hdrRng.Interior.Color = RGB(0, 100, 0)
360  hdrRng.Font.Color = RGB(255, 255, 255)
370  hdrRng.Font.Bold = True
380  hdrRng.Font.Size = 12
390  hdrRng.HorizontalAlignment = xlCenter
    ...
    ' periodBase+1 header: "PERIOD_TYPE"
400  wsMgmt.Cells(periodBaseRow + 1, 1).Value = "PERIOD_TYPE"
    ' periodBase+2 to +5: the four period types
    ' shnatit = yearly
410  wsMgmt.Cells(periodBaseRow + 2, 1).Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
    ' chatzi shnati = half yearly
420  wsMgmt.Cells(periodBaseRow + 3, 1).Value = ChrW(1495) & ChrW(1510) & ChrW(1497) & " " & ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
    ' riv'oni = quarterly
430  wsMgmt.Cells(periodBaseRow + 4, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1504) & ChrW(1497)
    ' chodshi = monthly
440  wsMgmt.Cells(periodBaseRow + 5, 1).Value = ChrW(1495) & ChrW(1493) & ChrW(1491) & ChrW(1513) & ChrW(1497)

    ' Half year header + values (periodBase+6, +7, +8)
450  wsMgmt.Cells(periodBaseRow + 6, 1).Value = "HALF_YEAR"
    ' machatzit rishona = first half
460  wsMgmt.Cells(periodBaseRow + 7, 1).Value = ChrW(1502) & ChrW(1495) & ChrW(1510) & ChrW(1497) & ChrW(1514) & " " & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1493) & ChrW(1504) & ChrW(1492)
    ' machatzit shniya = second half
470  wsMgmt.Cells(periodBaseRow + 8, 1).Value = ChrW(1502) & ChrW(1495) & ChrW(1510) & ChrW(1497) & ChrW(1514) & " " & ChrW(1513) & ChrW(1504) & ChrW(1497) & ChrW(1492)

    ' Define named ranges on NIHUL for dropdown sources
480  On Error Resume Next
490  ThisWorkbook.Names("lst_period_type").Delete
500  ThisWorkbook.Names("lst_half_year").Delete
510  ThisWorkbook.Names("lst_quarter").Delete
520  ThisWorkbook.Names("lst_month").Delete
530  Err.Clear
540  On Error GoTo ERR_HANDLER

550  ThisWorkbook.Names.Add "lst_period_type", wsMgmt.Range(wsMgmt.Cells(periodBaseRow + 2, 1), wsMgmt.Cells(periodBaseRow + 5, 1))
560  ThisWorkbook.Names.Add "lst_half_year", wsMgmt.Range(wsMgmt.Cells(periodBaseRow + 7, 1), wsMgmt.Cells(periodBaseRow + 8, 1))

    ' Quarter list (periodBase+10 to +13)
    ' riv'on rishon = Q1
570  wsMgmt.Cells(periodBaseRow + 10, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1493) & ChrW(1503)
    ' riv'on sheni = Q2
580  wsMgmt.Cells(periodBaseRow + 11, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1513) & ChrW(1504) & ChrW(1497)
    ' riv'on shlishi = Q3
590  wsMgmt.Cells(periodBaseRow + 12, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1513) & ChrW(1500) & ChrW(1497) & ChrW(1513) & ChrW(1497)
    ' riv'on revi'i = Q4
600  wsMgmt.Cells(periodBaseRow + 13, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1512) & ChrW(1489) & ChrW(1497) & ChrW(1506) & ChrW(1497)
610  ThisWorkbook.Names.Add "lst_quarter", wsMgmt.Range(wsMgmt.Cells(periodBaseRow + 10, 1), wsMgmt.Cells(periodBaseRow + 13, 1))

    ' Month list (periodBase+15 to +26 = 12 Hebrew month names)
620  wsMgmt.Cells(periodBaseRow + 15, 1).Value = ChrW(1497) & ChrW(1504) & ChrW(1493) & ChrW(1488) & ChrW(1512)
630  wsMgmt.Cells(periodBaseRow + 16, 1).Value = ChrW(1508) & ChrW(1489) & ChrW(1512) & ChrW(1493) & ChrW(1488) & ChrW(1512)
640  wsMgmt.Cells(periodBaseRow + 17, 1).Value = ChrW(1502) & ChrW(1512) & ChrW(1509)
650  wsMgmt.Cells(periodBaseRow + 18, 1).Value = ChrW(1488) & ChrW(1508) & ChrW(1512) & ChrW(1497) & ChrW(1500)
660  wsMgmt.Cells(periodBaseRow + 19, 1).Value = ChrW(1502) & ChrW(1488) & ChrW(1497)
670  wsMgmt.Cells(periodBaseRow + 20, 1).Value = ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1497)
680  wsMgmt.Cells(periodBaseRow + 21, 1).Value = ChrW(1497) & ChrW(1493) & ChrW(1500) & ChrW(1497)
690  wsMgmt.Cells(periodBaseRow + 22, 1).Value = ChrW(1488) & ChrW(1493) & ChrW(1490) & ChrW(1493) & ChrW(1505) & ChrW(1496)
700  wsMgmt.Cells(periodBaseRow + 23, 1).Value = ChrW(1505) & ChrW(1508) & ChrW(1496) & ChrW(1502) & ChrW(1489) & ChrW(1512)
710  wsMgmt.Cells(periodBaseRow + 24, 1).Value = ChrW(1488) & ChrW(1493) & ChrW(1511) & ChrW(1496) & ChrW(1493) & ChrW(1489) & ChrW(1512)
720  wsMgmt.Cells(periodBaseRow + 25, 1).Value = ChrW(1504) & ChrW(1493) & ChrW(1489) & ChrW(1502) & ChrW(1489) & ChrW(1512)
730  wsMgmt.Cells(periodBaseRow + 26, 1).Value = ChrW(1491) & ChrW(1510) & ChrW(1502) & ChrW(1489) & ChrW(1512)
740  ThisWorkbook.Names.Add "lst_month", wsMgmt.Range(wsMgmt.Cells(periodBaseRow + 15, 1), wsMgmt.Cells(periodBaseRow + 26, 1))

    ' ---- rngPeriodType dropdown: period type ----
750  wsMain.Range("G5").Validation.Delete
760  wsMain.Range("G5").Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=lst_period_type"

    ' ---- rngDateType dropdown: date type ----
    ' bordereu / thilat bituah
770  wsMain.Range("G7").Validation.Delete

780  dateTypeList = ChrW(1489) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1512) & ChrW(1493) & "," & ChrW(1514) & ChrW(1495) & ChrW(1497) & ChrW(1500) & ChrW(1514) & " " & ChrW(1489) & ChrW(1497) & ChrW(1496) & ChrW(1493) & ChrW(1495)
790  wsMain.Range("G7").Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=dateTypeList
...
800  wsMain.Range("G6").Validation.Delete

    ' ---- Column widths and row heights for home page (MUST be before button placement) ----
810  wsMain.Columns("A").ColumnWidth = 8
820  wsMain.Columns("B").ColumnWidth = 8
830  wsMain.Columns("C").ColumnWidth = 8
840  wsMain.Columns("D").ColumnWidth = 8
850  wsMain.Columns("E").ColumnWidth = 8
860  wsMain.Columns("F").ColumnWidth = 12
870  wsMain.Columns("G").ColumnWidth = 17.2
880  wsMain.Columns("H").ColumnWidth = 10
890  wsMain.Columns("I").ColumnWidth = 5.4
900  wsMain.Columns("J").ColumnWidth = 10
910  wsMain.Columns("K").ColumnWidth = 10
920  wsMain.Columns("L").ColumnWidth = 3
930  wsMain.Columns("M").ColumnWidth = 8
940  wsMain.Columns("N").ColumnWidth = 8
950  wsMain.Columns("O").ColumnWidth = 8
960  wsMain.Columns("P").ColumnWidth = 8
970  wsMain.Columns("Q").ColumnWidth = 8
980  wsMain.Columns("R").ColumnWidth = 8
990  wsMain.Rows("1:24").RowHeight = 22
1000  wsMain.Rows("1").RowHeight = 30

    ' ---- Remove ALL old buttons ----
1010  On Error Resume Next
1020  For Each s In wsMain.Shapes
1030  s.Delete
1040  Next s
1050  Err.Clear
1060  On Error GoTo ERR_HANDLER
...
1070  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M3").Top, 160, 35)
1080  shp.Name = "btnBuildReview"
1090  shp.Fill.ForeColor.RGB = blueClr
1100  shp.TextFrame2.TextRange.Text = "1 - " & ChrW(1489) & ChrW(1491) & ChrW(1497) & ChrW(1511) & ChrW(1514) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501)
1110  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1120  shp.TextFrame2.TextRange.Font.Size = 12
1130  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1140  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1150  shp.OnAction = "BuildReview"

    ' ---- Button 2: ApplyCorrectionsAndBuildReports ----
    ' "2 - yisshum vedochot" = Apply & Reports
1160  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M5").Top, 160, 35)
1170  shp.Name = "btnApplyCorrections"
1180  shp.Fill.ForeColor.RGB = RGB(0, 120, 60)
1190  shp.TextFrame2.TextRange.Text = "2 - " & ChrW(1497) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1501) & " " & ChrW(1493) & ChrW(1491) & ChrW(1493) & ChrW(34) & ChrW(1495) & ChrW(1493) & ChrW(1514)
1200  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1210  shp.TextFrame2.TextRange.Font.Size = 12
1220  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1230  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1240  shp.OnAction = "ApplyCorrectionsAndBuildReports"

    ' ---- Button 3: BuildPresentation ----
    ' "3 - yitzur matzget" = Create Presentation
1250  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M7").Top, 160, 35)
1260  shp.Name = "btnBuildPresentation"
1270  shp.Fill.ForeColor.RGB = RGB(160, 80, 0)
1280  shp.TextFrame2.TextRange.Text = "3 - " & ChrW(1497) & ChrW(1497) & ChrW(1510) & ChrW(1493) & ChrW(1512) & " " & ChrW(1502) & ChrW(1510) & ChrW(1490) & ChrW(1514)
1290  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1300  shp.TextFrame2.TextRange.Font.Size = 12
1310  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1320  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1330  shp.OnAction = "BuildPresentation"

    ' ---- Button 4: SaveReports ----
    ' "4 - shmirat dochot" = Save Reports
1340  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M9").Top, 160, 35)
1350  shp.Name = "btnSaveReports"
1360  shp.Fill.ForeColor.RGB = RGB(100, 60, 140)
1370  shp.TextFrame2.TextRange.Text = "4 - " & ChrW(1513) & ChrW(1502) & ChrW(1497) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514)
1380  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1390  shp.TextFrame2.TextRange.Font.Size = 12
1400  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1410  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1420  shp.OnAction = "SaveReportsToFolder"

    ' ---- Button 5: ViewReports ----
    ' "5 - tzfiya bedochot" = View Reports
1430  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M11").Top, 160, 35)
1440  shp.Name = "btnViewReports"
1450  shp.Fill.ForeColor.RGB = RGB(70, 180, 220)
1460  shp.TextFrame2.TextRange.Text = "5" & " - " & ChrW(1510) & ChrW(1508) & ChrW(1497) & ChrW(1497) & ChrW(1492) & " " & ChrW(1489) & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514)
1470  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1480  shp.TextFrame2.TextRange.Font.Size = 12
1490  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1500  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1510  shp.OnAction = "ViewReportsFolder"

    ' ---- Button 6: NewClients ----
    ' "6 - lekochot chadashim" = New Clients
1520  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("M3").Left + wsMain.Range("M3").Width - 5, wsMain.Range("M13").Top, 160, 35)
1530  shp.Name = "btnNewClients"
1540  shp.Fill.ForeColor.RGB = RGB(230, 130, 0)
1550  shp.TextFrame2.TextRange.Text = "6" & " - " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501)
1560  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1570  shp.TextFrame2.TextRange.Font.Size = 12
1580  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1590  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1600  shp.OnAction = "NewClients"

    ' ---- Button: Exit System at B16 ----
1610  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("B16").Left, wsMain.Range("B16").Top, 130, 30)
1620  shp.Name = "btnExitSystem"
1630  shp.Fill.ForeColor.RGB = RGB(180, 0, 0)
1640  shp.TextFrame2.TextRange.Text = ChrW(1497) & ChrW(1510) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1502) & ChrW(1492) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1514)
1650  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1660  shp.TextFrame2.TextRange.Font.Size = 11
1670  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1680  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1690  shp.OnAction = "ExitSystem"

    ' Button: Show/Hide hidden sheets - placed at P16
1700  Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("P16").Left, wsMain.Range("P16").Top, 160, 30)
1710  shp.Name = "btnShowHidden"
1720  shp.Fill.ForeColor.RGB = RGB(80, 80, 80)
1730  shp.TextFrame2.TextRange.Text = ChrW(1492) & ChrW(1510) & ChrW(1490) & "/" & ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1512) & " " & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514)
1740  shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
1750  shp.TextFrame2.TextRange.Font.Size = 10
1760  shp.TextFrame2.TextRange.Font.Bold = msoTrue
1770  shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
1780  shp.OnAction = "ToggleHiddenSheets"
...
2140  If IsEmpty(wsMain.Range("G5").Value) Or wsMain.Range("G5").Value = "" Then
2150  wsMain.Range("G5").Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
2160  End If

    ' ---- Set default value for rngFilterType if empty ----
2170  If IsEmpty(wsMain.Range("G9").Value) Or wsMain.Range("G9").Value = "" Then
2180  wsMain.Range("G9").Value = ChrW(1489) & ChrW(1495) & ChrW(1512) & ChrW(47) & ChrW(1497)
2190  End If

    ' ---- rngFilterType dropdown: report filter type ----
    ' Options: bechar/i, chevra, teler, sochen, anaf, anaf merkaz
2200  filterTypeList = ChrW(1489) & ChrW(1495) & ChrW(1512) & ChrW(47) & ChrW(1497) & "," & ChrW(1495) & ChrW(1489) & ChrW(1512) & ChrW(1492) & "," & ChrW(1496) & ChrW(1500) & ChrW(1512) & "," & ChrW(1505) & ChrW(1493) & ChrW(1499) & ChrW(1503) & "," & ChrW(1506) & ChrW(1504) & ChrW(1507) & "," & ChrW(1506) & ChrW(1504) & ChrW(1507) & " " & ChrW(1502) & ChrW(1512) & ChrW(1499) & ChrW(1494)
    ' Use direct cell reference G9 in case named range not yet defined
2210  On Error Resume Next
2220  wsMain.Range("G9").Validation.Delete
2230  wsMain.Range("G9").Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=filterTypeList
2240  wsMain.Range("G10").Validation.Delete
2250  Err.Clear
    ' Create/update ALL named ranges for home sheet cells
2260  On Error Resume Next
2270  ThisWorkbook.Names("rngCurrentYear").Delete
2280  ThisWorkbook.Names("rngBaseYear").Delete
2290  ThisWorkbook.Names("rngPeriodType").Delete
2300  ThisWorkbook.Names("rngPeriodValue").Delete
2310  ThisWorkbook.Names("rngDateType").Delete
2320  ThisWorkbook.Names("rngFilterType").Delete
2330  ThisWorkbook.Names("rngFilterValue").Delete
2340  Err.Clear
2350  On Error GoTo ERR_HANDLER
    ' Use direct Range reference (avoids sheet name encoding issues)
2360  ThisWorkbook.Names.Add "rngBaseYear", wsMain.Range("G3")
2370  ThisWorkbook.Names.Add "rngCurrentYear", wsMain.Range("G4")
2380  ThisWorkbook.Names.Add "rngPeriodType", wsMain.Range("G5")
2390  ThisWorkbook.Names.Add "rngPeriodValue", wsMain.Range("G6")
2400  ThisWorkbook.Names.Add "rngDateType", wsMain.Range("G7")
2410  ThisWorkbook.Names.Add "rngFilterType", wsMain.Range("G9")
2420  ThisWorkbook.Names.Add "rngFilterValue", wsMain.Range("G10")

    ' ---- Exchange rate display: J3:K4 = currency signs + rates, J5:K7 = message ----
2430  On Error Resume Next
2440  ThisWorkbook.Names("rngDOLAR").Delete
2450  Err.Clear
2460  On Error GoTo ERR_HANDLER
2470  ThisWorkbook.Names.Add "rngDOLAR", wsMain.Range("K3")
    ' J3 = dollar sign "$"
2475  wsMain.Range("J3").Value = "$"
2476  wsMain.Range("J3").Font.Bold = True
2477  wsMain.Range("J3").Font.Size = 14
2478  wsMain.Range("J3").HorizontalAlignment = xlCenter
    ' K3 = USD rate from BOI
2480  curRate = FetchBOIRate()
2490  If curRate > 0 Then
2500  wsMain.Range("K3").Value = curRate
2510  ElseIf IsEmpty(wsMain.Range("K3").Value) Then
2520  wsMain.Range("K3").Value = 3.6
2530  End If
2540  wsMain.Range("K3").NumberFormat = "0.0000"
    ' J4 = euro sign
2550  wsMain.Range("J4").Value = ChrW(8364)
2555  wsMain.Range("J4").Font.Bold = True
2556  wsMain.Range("J4").Font.Size = 14
2557  wsMain.Range("J4").HorizontalAlignment = xlCenter
    ' K4 = EUR rate from BOI
2560  eurRate = FetchBOIMonthlyAvg("EUR", "")
2565  If eurRate > 0 Then
2566  wsMain.Range("K4").Value = eurRate
2567  ElseIf IsEmpty(wsMain.Range("K4").Value) Then
2568  wsMain.Range("K4").Value = 3.9
2569  End If
2570  wsMain.Range("K4").NumberFormat = "0.0000"
    ' J5:K7 = message about exchange rate source
    ' "arachim bematach mitadknim lefi hashaar hayatziv Bank Israel la-15 bachodesh habordero"
2580  matachMsg = ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1497) & ChrW(1501) & " " & _
    ...
    ' Merge J5:K7 for the message
2585  On Error Resume Next
    wsMain.Range("J5:K7").UnMerge
2586  On Error GoTo ERR_HANDLER
2587  wsMain.Range("J5:K7").Merge
2588  wsMain.Range("J5").Value = matachMsg
2589  wsMain.Range("J5").WrapText = True
2590  wsMain.Range("J5").Font.Size = 9
2591  wsMain.Range("J5").HorizontalAlignment = xlCenter
2592  wsMain.Range("J5").VerticalAlignment = xlCenter

    ' ---- Client name filter (F12 label, G12 Data Validation dropdown) ----
    ' "shem lakoach" = ?? ????
2600  wsMain.Range("F12").Value = ChrW(1513) & ChrW(1501) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495)
2610  wsMain.Range("F12").Font.Bold = True
2620  wsMain.Range("F12").Font.Size = 12
2630  wsMain.Range("F12").Font.Color = RGB(0, 70, 140)

    ' Create/update named range for G12
2640  On Error Resume Next
2650  ThisWorkbook.Names("rngClientName").Delete
2660  Err.Clear
2670  On Error GoTo ERR_HANDLER
2680  ThisWorkbook.Names.Add "rngClientName", wsMain.Range("G12")

    ' Set default value "bechar/i" = ???/?
2690  If IsEmpty(wsMain.Range("G12").Value) Or wsMain.Range("G12").Value = "" Then
2700  wsMain.Range("G12").Value = ChrW(1489) & ChrW(1495) & ChrW(1512) & ChrW(47) & ChrW(1497)
2710  End If

    ' ---- Create/preserve rngFILES_FOLDER and rngREPORTS_FOLDER (on NIHUL sheet) ----
    ' These named ranges point to cells on the NIHUL/hagdarot sheet (Params section)
    ' B176 = source files folder path, B177 = reports folder path
2715  On Error Resume Next
    ' Only create if not already defined (preserve user-set paths)
    existingPath = ""
    ...
    End If
    Err.Clear
2718  On Error GoTo ERR_HANDLER

    ' Remove old ComboBox if exists (from previous versions)
2720  On Error Resume Next
2730  wsMain.OLEObjects("cmbClientName").Delete
2740  Err.Clear
2750  On Error GoTo ERR_HANDLER

    ' Remove Data Validation from G12 (from previous versions)
2760  On Error Resume Next
2770  wsMain.Range("G12").Validation.Delete
2780  Err.Clear
2790  On Error GoTo ERR_HANDLER

    ' ---- Create utility buttons (search, all, reset) ----
    ' Button block under Resume Next so failures don't stop setup
2800  On Error Resume Next
2810  wsMain.Shapes("btnSearchClient").Delete
2820  wsMain.Shapes("btnAllClients").Delete
2830  wsMain.Shapes("btnResetDefaults").Delete
2840  Err.Clear

    ' Search button in H12
2850  Set btnSearch = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, _
    wsMain.Range("H12").Left + 2, wsMain.Range("H12").Top + 1, _
    50, wsMain.Range("H12").Height - 2)
2860  If Not btnSearch Is Nothing Then
2870  btnSearch.Name = "btnSearchClient"
2880  btnSearch.TextFrame2.TextRange.Text = ChrW(1495) & ChrW(1508) & ChrW(1513)
2890  btnSearch.TextFrame2.TextRange.Font.Size = 9
2900  btnSearch.TextFrame2.TextRange.Font.Bold = msoTrue
2910  btnSearch.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
2920  btnSearch.Fill.ForeColor.RGB = RGB(70, 130, 180)
2930  btnSearch.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
2940  btnSearch.OnAction = "SearchClientName"
2950  End If

    ' All clients button next to search
2960  Set btnAll = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, _
    wsMain.Range("H12").Left + 55, wsMain.Range("H12").Top + 1, _
    50, wsMain.Range("H12").Height - 2)
2970  If Not btnAll Is Nothing Then
2980  btnAll.Name = "btnAllClients"
2990  btnAll.TextFrame2.TextRange.Text = ChrW(1499) & ChrW(1493) & ChrW(1500) & ChrW(1501)
3000  btnAll.TextFrame2.TextRange.Font.Size = 9
3010  btnAll.TextFrame2.TextRange.Font.Bold = msoTrue
3020  btnAll.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
3030  btnAll.Fill.ForeColor.RGB = RGB(60, 160, 60)
3040  btnAll.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
3050  btnAll.OnAction = "ResetClientFilter"
3060  End If

    ' Reset to defaults button at G13
3070  Set btnReset = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, _
    wsMain.Range("G13").Left, wsMain.Range("G13").Top + 1, _
    110, wsMain.Range("G13").Height - 2)
3080  If Not btnReset Is Nothing Then
3090  btnReset.Name = "btnResetDefaults"
3100  btnReset.TextFrame2.TextRange.Text = ChrW(1495) & ChrW(1494) & ChrW(1512) & ChrW(1492) & " " & ChrW(1500) & ChrW(1489) & ChrW(1512) & ChrW(1497) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1502) & ChrW(1495) & ChrW(1491) & ChrW(1500)
3110  btnReset.TextFrame2.TextRange.Font.Size = 9
3120  btnReset.TextFrame2.TextRange.Font.Bold = msoTrue
3130  btnReset.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
3140  btnReset.Fill.ForeColor.RGB = RGB(180, 80, 80)
3150  btnReset.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
3160  btnReset.OnAction = "ResetHomeDefaults"
3170  End If
3180  Err.Clear
    ' --- END of button block: restore structured error handling ---
3190  On Error GoTo ERR_HANDLER
...
3230  Next wsLoop
    ...
    ' UnMerge safely (may already be unmerged - not an error)
3240  On Error Resume Next
3250  wsMain.Range("J5:L7").UnMerge
3260  Err.Clear
3270  On Error GoTo ERR_HANDLER
    ' Clear any leftover borders in column L (narrow separator column)
3280  wsMain.Range("L1:L24").Borders.LineStyle = xlNone
3290  Set rngMsg = wsMain.Range("J5:K7")
    ' Merge safely
3300  On Error Resume Next
3310  rngMsg.Merge
3320  Err.Clear
3330  On Error GoTo ERR_HANDLER
    ' Message: line1="???? ?????? ?"? ??? ?????" line2="?? ?? ????? ???" line3="???? ???? ????? ???"
3340  msgTxt = ChrW(1492) & ChrW(1513) & ChrW(1506) & ChrW(1512) & " " & _
...
3350  msgTxt = msgTxt & ChrW(1488) & ChrW(1501) & " " & _
    ChrW(1500) & ChrW(1488) & " " & _
    ChrW(1497) & ChrW(1497) & ChrW(1502) & ChrW(1510) & ChrW(1488) & " " & _
    ChrW(1513) & ChrW(1506) & ChrW(1512) & vbLf
3360  msgTxt = msgTxt & ChrW(1497) & ChrW(1513) & ChrW(1502) & ChrW(1513) & " " & _
    ChrW(1492) & ChrW(1513) & ChrW(1506) & ChrW(1512) & " " & _
    ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1493) & ChrW(1501) & " " & _
    ChrW(1499) & ChrW(1488) & ChrW(1503)
3370  rngMsg.Value = msgTxt
3380  rngMsg.Font.Size = 10
3390  rngMsg.Font.Color = RGB(0, 70, 180)
3400  rngMsg.Font.Italic = True
3410  rngMsg.WrapText = True
3420  rngMsg.VerticalAlignment = xlCenter
3430  rngMsg.HorizontalAlignment = xlCenter

    ' Border as Shape (rectangle) around J5:K7
3440  On Error Resume Next
3450  wsMain.Shapes("shpMsgBorder").Delete
3460  Err.Clear
3470  Set borderShp = wsMain.Shapes.AddShape(msoShapeRectangle, _
    rngMsg.Left, rngMsg.Top, rngMsg.Width, rngMsg.Height)
3480  If Not borderShp Is Nothing Then
3490  borderShp.Name = "shpMsgBorder"
3500  borderShp.Fill.Visible = msoFalse
3510  borderShp.Line.ForeColor.RGB = RGB(0, 0, 120)
3520  borderShp.Line.Weight = 1.5
3530  borderShp.Placement = xlMoveAndSizeWithCells
3540  End If
3550  Err.Clear
3560  On Error GoTo ERR_HANDLER

    ' ---- Cell borders for exchange rate cells J3:K7 ----
3570  On Error Resume Next
3580  wsMain.Shapes("shpRateBorder").Delete
3590  Err.Clear
3600  On Error GoTo ERR_HANDLER
3610  For Each cellRate In wsMain.Range("J3:K7")
3620  For Each rEdge In Array(xlEdgeLeft, xlEdgeTop, xlEdgeBottom, xlEdgeRight)
3630  cellRate.Borders(rEdge).LineStyle = xlContinuous
3640  cellRate.Borders(rEdge).Color = RGB(0, 70, 140)
3650  cellRate.Borders(rEdge).Weight = xlThin
3660  Next rEdge
3670  Next cellRate

    ' ---- Borders for parameter cells F3:G12 ----
3680  For Each cellBorder In wsMain.Range("F3:G12")
3690  For Each bEdge In Array(xlEdgeLeft, xlEdgeTop, xlEdgeBottom, xlEdgeRight)
3700  cellBorder.Borders(bEdge).LineStyle = xlContinuous
3710  cellBorder.Borders(bEdge).Color = RGB(0, 70, 140)
3720  cellBorder.Borders(bEdge).Weight = xlThin
3730  Next bEdge
3740  Next cellBorder

    ' ---- Green pastel background for home page A1:U24 ----
3750  wsMain.Range("A1:U24").Interior.Color = RGB(220, 240, 220)

    ' ---- Re-apply row 2 header colors (overwritten by pastel above) ----
3760  hdrRng.Interior.Color = RGB(0, 100, 0)

    ' ---- Clear old date/credit/version cells from previous versions ----
      On Error Resume Next
3770  wsMain.Range("R1").UnMerge
      wsMain.Range("R1").ClearContents
3780  wsMain.Range("S1").UnMerge
      wsMain.Range("S1").ClearContents
3790  wsMain.Range("R18").UnMerge
      wsMain.Range("R18").ClearContents
3800  wsMain.Range("A18").UnMerge
      wsMain.Range("A18").ClearContents
3810  wsMain.Range("R23").UnMerge
      wsMain.Range("R23").ClearContents
3815  wsMain.Range("A23").UnMerge
      wsMain.Range("A23").ClearContents
      On Error GoTo ERR_HANDLER
    ' ---- Date in T1 ----
3820  wsMain.Range("T1").Value = Format$(Date, "dd/mm/yyyy")
3830  wsMain.Range("T1").Font.Size = 11
3840  wsMain.Range("T1").Font.Color = RGB(0, 0, 102)
3850  wsMain.Range("T1").Font.Bold = True
3860  wsMain.Range("T1").HorizontalAlignment = xlCenter
    ' ---- Version in A23 ----
3870  wsMain.Range("D23").Value = "v" & APP_VERSION
3880  wsMain.Range("D23").Font.Size = 11
3890  wsMain.Range("D23").Font.Color = RGB(0, 0, 102)
3900  wsMain.Range("D23").Font.Bold = True
    ' ---- Credit in T23: "nbneh al yedei gornatek 054-6677396" ----
3910  wsMain.Range("T23").Value = ChrW(1504) & ChrW(1489) & ChrW(1504) & ChrW(1492) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1497) & ChrW(1491) & ChrW(1497) & " " & ChrW(1490) & ChrW(1493) & ChrW(1512) & ChrW(1504) & ChrW(1496) & ChrW(1511) & " 054-6677396"
3920  wsMain.Range("T23").Font.Size = 11
3930  wsMain.Range("T23").Font.Color = RGB(0, 0, 102)
3940  wsMain.Range("T23").Font.Bold = True
3950  wsMain.Range("T23").HorizontalAlignment = xlCenter
...
Public Sub NewClients()
    ...
    wsOut.Cells(2, 3).Value = ChrW(1495) & ChrW(1489) & ChrW(1512) & ChrW(1492)  ' ????
    wsOut.Cells(2, 4).Value = ChrW(1506) & ChrW(1504) & ChrW(1507)  ' ???
    wsOut.Cells(2, 5).Value = ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1497) & ChrW(1492)  ' ?????
    ...
    Else
        ...
        wsMaster.Cells(2, 3).Value = ChrW(1495) & ChrW(1489) & ChrW(1512) & ChrW(1492)  ' ????
        wsMaster.Cells(2, 4).Value = ChrW(1506) & ChrW(1504) & ChrW(1507)  ' ???
        wsMaster.Cells(2, 5).Value = ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1497) & ChrW(1492)  ' ?????
...
Public Sub ApplyUserPermissions()
    ...
    On Error Resume Next
    If Len(creditVersionCell) > 0 Then
...
10  SettingsScrollTo "rngSection_Permissions"
End Sub
...
' Called from SetupMainSheet
' Buttons are placed in column I so they remain visible when scrolling vertically
' ----------------------------------------------------------------------------
...
710 shp.TextFrame2.TextRange.Text = ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1493) & ChrW(1514) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1492)  ' "?????? ?????"
...
1160 shp.OnAction = "NavToIndex"
    ...
    ' Set column I width for nav buttons
1170 wsMgmt.Columns("I").ColumnWidth = 22
...
160     If InStr(1, ws.Name, ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505), vbTextCompare) > 0 Then
