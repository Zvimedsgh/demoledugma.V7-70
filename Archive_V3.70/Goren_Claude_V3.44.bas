Attribute VB_Name = "Goren_Claude_V3_44"
' ============================================================================
' MODULE: modLevav
...
'     .bas being re-saved as ASCII. No behaviour change.
' CHANGES IN V3.42:
'   - The home page now fits the window automatically on open (and on every
'     return to it), so the "התאמת מסך" button is always visible - a large saved
'     zoom used to push it below the fold, which looked broken. Done by calling
'     FitHomeToScreen at the end of CheckUserPermissions. The manual button
'     still works for re-fitting.
' CHANGES IN V3.41:
'   - On landing on the home page (open, return-from-settings, and fit) the view
'     now scrolls to the top-right corner so the right side and the "התאמת מסך"
'     button are visible, and the cursor rests on F10 instead of G10. Applies in
'     both Demo and Full modes (done in CheckUserPermissions, which runs for
'     both), plus FitHomeToScreen and A00.
' CHANGES IN V3.40:
'   - FitHomeToScreen: the plain fit came out too small, so it now scales the
'     fitted zoom up 30% (clamped, capped at 130). Also ignores stray shapes
'     parked far off-screen, which would otherwise inflate the fit range and
'     shrink everything.
' CHANGES IN V3.39:
'   - The "התאמת מסך" button now actually fits the home page to the window.
'     It called ToggleDisplayMode, which only flips zoom 80<->100, and A00
'     fitted a fixed A1:L20 range - so on a laptop with a different resolution
'     the page overflowed and had to be scrolled. New FitHomeToScreen measures
'     the real extent of every shape and zooms that whole range to fit any
'     screen. A00 uses it too, and AssignButtonMacros repoints the button on
'     already-built copies so they self-heal on open.
' CHANGES IN V3.38:
'   - Demo/Full access no longer leave home-page buttons grey. The Limited
'     branch paints buttons 1-6 grey, and once the workbook was saved in that
'     state nothing ever restored their colour - so a demo customer who got a
'     copy saved that way saw grey buttons (the tester''s complaint). The Demo
'     and Full branches now call RestoreButtonColor on every button first, so
'     all 7 come up in their designed colours; the demo''s buttons 5-7 still
'     pop the "available in the full version" message.
' CHANGES IN V3.37:
'   - FixArrows: the zoom flicker was not a strong enough repaint on this
'     machine, so it now minimizes and restores the window instead - the
'     strongest full-window redraw Excel offers. Visibly flickers the window
'     for an instant but reliably repaints the dropdown arrows.
' CHANGES IN V3.36:
'   - Added FixArrows, called as the last step of A00 (the "back to home"
'     button's action). This machine's Excel stops repainting the in-cell
'     dropdown arrows after A00's UI work even though everything is correct;
'     FixArrows forces the repaint (formula bar + Ribbon visible, zoom flicker)
'     so the arrows are drawn on landing. Can also be run manually via Alt+F8.
' CHANGES IN V3.35:
'   - The real, final cause of the disappearing G5..G10 (and B7) dropdown
'     arrows: A00 minimized the Ribbon. Excel does not draw in-cell dropdown
'     arrows while the Ribbon is minimized (same failure mode as the hidden
'     formula bar in V3.30). A00 runs on every return to the home page, so the
'     arrows vanished the instant you went into settings and back, while every
'     measurable property (unprotected, validation present, formula bar on,
'     objects shown) stayed perfect. Confirmed by hand: Ctrl+F1 restored them.
'     A00 now forces the Ribbon EXPANDED instead of minimized.
' CHANGES IN V3.34:
'   - THE fix that ends the dropdown saga. Confirmed by hand: with the home
'     sheet UNPROTECTED, the G5..G10 pickers show their arrows reliably; the
'     moment code re-protects the sheet the arrows vanish - even with
'     DrawingObjects:=False, which restores them when the same Protect is run
'     by hand but not from code. So the two "land on a sheet" finalizers now
'     leave the sheet unprotected instead of re-protecting it:
'       * CheckUserPermissions no longer re-protects the home sheet (this runs
'         last on every return to the home page).
'       * The A00 DEMO_MODE block no longer re-protects the params sheet, which
'         also restores the B7 dropdown arrow.
'     UserInterfaceOnly protection on these sheets was never real security.
' CHANGES IN V3.33:
'   - The V3.32 re-protect in SettingsGoToSheet did not restore the DEMO_MODE
'     dropdown arrow, even though the same Protect DrawingObjects:=False call
'     restores it when run by hand in the Immediate window. Rather than keep
'     chasing that, SettingsGoToSheet now simply leaves the settings sheet
'     unprotected while it is open - which always restores the arrow. The
'     settings sheets are meant to be edited, and navigating home re-secures
'     the workbook.
' CHANGES IN V3.32:
'   - Follow-up to 3.31: the home-page dropdowns came back but DEMO_MODE on the
'     params sheet stayed dead. Cause: SettingsGoToSheet (the only path to the
'     settings sheets) never re-protected the sheet it opened, so a settings
'     sheet kept the DrawingObjects:=True protection baked into a file saved by
'     an older build. The home sheet self-healed because A00 re-protects it
'     directly; the settings sheets had no such path. SettingsGoToSheet now
'     re-applies Protect with DrawingObjects:=False on the sheet it opens.
' CHANGES IN V3.31:
'   - THE dropdown bug. Every Worksheet.Protect call relied on the default
'     DrawingObjects:=True. Excel treats the data-validation in-cell dropdown
'     arrow as a drawing object, so protecting a sheet that way hides the arrow
'     even on unlocked cells - the validation stays perfectly intact, only the
'     arrow is gone. That killed every dropdown in the workbook at once: the
'     year, period and filter pickers on the home page and DEMO_MODE on the
'     params sheet. It looked intermittent because Workbook_Open protects
'     nothing, so a freshly opened workbook worked until the first visit to the
'     home page ran A00_SetupMainSheet.
'     All 15 Protect calls now pass DrawingObjects:=False. Cell protection is
'     unchanged - only the arrows come back.
'     Confirmed by hand: unprotecting restored the arrow; re-protecting with
'     DrawingObjects:=False kept it; returning home (which re-protected with
'     the old default) removed it again.
' CHANGES IN V3.30:
'   - A00_SetupMainSheet no longer hides the formula bar. Excel does not draw
'     data-validation in-cell dropdown arrows while the formula bar is hidden,
'     so hiding it killed every dropdown in the workbook. A00 runs on every
'     return to the home page, which is why the arrows were present on a fresh
'     open and gone after navigating home and back. The line now forces the bar
'     back on, so a session an earlier build left with it hidden repairs itself.
'     Minimizing the Ribbon is unrelated and kept.
' CHANGES IN V3.29:
'   - A00_SetupMainSheet: the DEMO_MODE dropdown block deleted and re-added the
'     validation on every run - and A00 runs on every return to the home page.
'     Because the block is inside On Error Resume Next, any failure of the Add
'     left the cell with its validation deleted and nothing put back, so the
'     arrow disappeared with no error. It now checks first and rebuilds only
'     when the validation is genuinely missing.
'     Reproduction: open workbook, go to settings, B7 has an arrow; return to
'     the home page and go back to settings, the arrow is gone.
'   - Dropped Operator:=xlBetween from that Validation.Add - Operator is not
'     valid for xlValidateList and was a second way the Add could fail.
'   - The params sheet is re-protected with UserInterfaceOnly so later macro
'     writes to it still work.
' CHANGES IN V3.28:
'   - A00_SetupMainSheet: the block that builds the Yes/No dropdown on the
'     DEMO_MODE parameter cell unprotected the params sheet with "1234" while
'     the rest of the module protects with "Z961814r". On an already-protected
'     sheet the unprotect failed and, under On Error Resume Next, the dropdown
'     was never rebuilt - with no error shown. It also re-protected with
'     "1234", leaving a sheet no other routine could unlock. Both now use the
'     real password, with "1234" kept only as a recovery attempt on unprotect.
'   - ToggleDisplayMode: same "1234" pair on the home sheet, same fix, and it
'     now re-protects with UserInterfaceOnly so macros keep write access.
' CHANGES IN V3.27:
'   - SendForReview: the "must run from a review sheet" guard used ActiveSheet,
'     which resolves against the ACTIVE WINDOW rather than the sheet whose
'     button was clicked. With a second window open on the same workbook
'     (View > New Window) it saw the home sheet and refused to run, even with
'     the review sheet on screen. It now falls back to the review sheet of the
'     current year, or to the single review sheet if only one exists. If
'     several exist and none matches the current year it still refuses, rather
'     than risk mailing the wrong year.
' CHANGES IN V3.26:
'   - NewClients: the master sheet's "back to home" button now calls the new
'     NC_BackToHome, which hides the master sheet after rebuilding the home
'     page (it used to call A00_SetupMainSheet and leave the sheet on screen).
'     The hide runs after A00 on purpose - A00 ends in CheckUserPermissions,
'     which re-shows that sheet for Limited Access users.
'   - AssignButtonMacros now also re-points btnBackHome on the master sheet, so
'     a workbook built by an earlier version picks up NC_BackToHome on open
'     rather than only after a full NewClients rebuild.
' CHANGES IN V3.25:
'   - NewClients: the "already checked" dialog was a dead end - once the master
'     sheet existed the sub always exited, so the append-history logic could
'     never run and the list froze forever. Now Yes/No/Cancel, where No
'     refreshes and appends only customers not already listed.
'   - NewClients: unhide the master sheet before Activate (raised 1004 on the
'     refresh path, which was previously unreachable).
'   - NewClients: the temp sheet (NC_TEMP_SHEET_NAME) is now deleted at the end
'     of the run. It mirrored the master's layout but reset every status to "open" on
'     each run, so editing it lost the underwriter's work. It was referenced
'     nowhere else in the module. The master keeps status/date history and its
'     filter buttons already provide the "still to handle" view.
'   - Corrected "button 5" to "button 6" in the two "already exists" dialogs
'     (ApplyCorrectionsAndBuildReports and BuildPresentation) - viewing reports
'     is ViewReportsFolder, which is button 6.
'   - Cleared the placeholder developer accounts from the permissions sheet.
' CHANGES IN V3.24:
'   - Fixed compile error by moving g_CachedUserAccess to top of module.
' CHANGES IN V3.23:
'   - Fixed compile error from V3.22 (On Error GoTo ER).
' CHANGES IN V3.22:
'   - Removed the pink instruction message box (shpInstallMsg) that appeared unnecessarily during A00.
' CHANGES IN V3.21:
'   - Fixed RTL off-screen shape bug for existing buttons calling A00 by forcing active sheet switch inside A00.
'   - Updated button assignments in code to use NavSettings_Home.
' CHANGES IN V3.20:
'   - Removed A00 from NavSettings_Home and replaced it with CheckUserPermissions to prevent RTL shape deletion bug.
' CHANGES IN V3.19:
'   - Fixed NavSettings_Home to activate the main sheet before running A00 to prevent shape creation glitches.
' CHANGES IN V3.18:
'   - Added ForceTabColor to prevent Worksheet_Deactivate crashes when returning from settings.
' CHANGES IN V3.17:
'   - Fixed ScreenUpdating freeze in CheckUserPermissions so UI refreshes.
' CHANGES IN V3.16:
'   - Prevented CheckUserPermissions from automatically hiding sheets for Full Access users.
' CHANGES IN V3.15:
'   - Removed HideWorkSheets from NavSettings_Home so settings stay open.
' CHANGES IN V3.14:
'   - Fixed btnShowResults macro assignment.
'   - Restored Limited Access logic (so only completely unauthorized users fall into Demo Mode).
' CHANGES IN V3.13:
'   - Reverted FORCE_DEMO_MODE to False to allow normal operation.
'   - Version bump to 3.13.
' CHANGES IN V3.12:
...
' DATE: 2026-07-12 11:12:09 (LATEST FIXES)
' ============================================================================
' CHANGES IN 1.51:
...
'   - New: Home page layout: J3=$, K3=USD rate, J4=ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬, K4=EUR rate, J5:K7=info message
...
'   - Added dynamic string parsing for UpdatePeriodDropdown to bypass VeryHidden sheet validation bug
' CHANGES IN 8.04:
...
Private Const APP_VERSION As String = "3.44"


Private Const APP_DATE As String = "23/07/2026 10:00"
Private g_CachedUserAccess As Integer ' 0=uninit, 1=unauth, 2=limited, 3=full
...
Private Function H_SET_FIELDMAP() As String: H_SET_FIELDMAP = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1502) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1497): End Function
Private Function H_SET_BRANCHES() As String: H_SET_BRANCHES = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1506) & ChrW(1504) & ChrW(1508) & ChrW(1497) & ChrW(1501): End Function
Private Function H_SET_PARAMS() As String: H_SET_PARAMS = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1493) & _
        ChrW(1514) & "_" & ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1496) & ChrW(1512) & ChrW(1497) & _
        ChrW(1501): End Function
Private Function H_SET_PERIODS() As String: H_SET_PERIODS = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1493) & _
        ChrW(1514): End Function
Private Function H_SET_MESSAGES() As String: H_SET_MESSAGES = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1493) & _
        ChrW(1514): End Function
Private Function H_SET_PERMISSIONS() As String: H_SET_PERMISSIONS = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & _
        ChrW(1514): End Function
Private Function H_SET_REASONS() As String: H_SET_REASONS = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1505) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1514): End Function
Private Function H_SET_CLIENTS() As String: H_SET_CLIENTS = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514) & "_" & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & _
        ChrW(1514): End Function
...
Private Function H_UNKNOWN_BRANCH() As String: H_UNKNOWN_BRANCH = ChrW(1506) & ChrW(1504) & ChrW(1507) & " " & _
        ChrW(1500) & ChrW(1488) & " " & ChrW(1502) & ChrW(1494) & ChrW(1493) & ChrW(1492) & ChrW(1492): End Function
...
Private Function SOURCE_FOLDER() As String
    ...
    SOURCE_FOLDER = Trim$(CStr(GetStringParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), "FILES_FOLDER")))
    On Error GoTo 0
...
Private Function REPORTS_FOLDER() As String
    On Error Resume Next
    REPORTS_FOLDER = Trim$(CStr(GetStringParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), "REPORTS_FOLDER")))
    On Error GoTo 0
...
REPORTS_FOLDER = Replace(REPORTS_FOLDER, ":\\\\", ":\\")
    REPORTS_FOLDER = Replace(REPORTS_FOLDER, ":\\\\", ":\\")
End Function
...
Private Function NC_MASTER_SHEET_NAME() As String
    NC_MASTER_SHEET_NAME = ChrW(1496) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1500) & "_" & ChrW(1489) & _
        ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1495) & ChrW(1491) & ChrW(1513) & _
        ChrW(1497) & ChrW(1501)  ' ?????_???????_?????
End Function
...
Private Function SafeOpenWorkbook(ByVal fPath As String) As Workbook
    ...
    If wb Is Nothing Then
        MsgBoxU ChrW(1492) & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & fn & " " & ChrW(1495) & _
        ChrW(1505) & ChrW(1493) & ChrW(1501) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1497) & ChrW(1491) & ChrW(1497) & " " & _
        ChrW(1488) & ChrW(1511) & ChrW(1505) & ChrW(1500) & " (" & ChrW(1502) & ChrW(1490) & ChrW(1503) & " / Protected View). " & _
        ChrW(1508) & ChrW(1514) & ChrW(1495) & " " & ChrW(1488) & ChrW(1493) & ChrW(1514) & ChrW(1493) & " " & ChrW(1497) & _
        ChrW(1491) & ChrW(1504) & ChrW(1497) & ChrW(1514) & " " & ChrW(1493) & ChrW(1488) & ChrW(1513) & ChrW(1512) & _
        " Enable Editing.", vbCritical
...
30      Set ws = wb.Worksheets(DATA_SHEET_NAME)
        If Err.Number <> 0 Then
            ...
            If Err.Number <> 0 Then
                MsgBoxU ChrW(1492) & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & wb.Name & " " & _
        ChrW(1495) & ChrW(1505) & ChrW(1493) & ChrW(1501) & " (" & ChrW(1502) & ChrW(1490) & ChrW(1503) & "). " & ChrW(1497) & _
        ChrW(1513) & " " & ChrW(1500) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1495) & " " & ChrW(1488) & ChrW(1493) & _
        ChrW(1514) & ChrW(1493) & " " & ChrW(1497) & ChrW(1491) & ChrW(1504) & ChrW(1497) & ChrW(1514) & " " & ChrW(1493) & _
        ChrW(1500) & ChrW(1488) & ChrW(1513) & ChrW(1512) & " Enable Editing.", vbCritical
...
80          If periodDetail <> "" Then
                  Dim mArr As Variant
                  mArr = Array("", ChrW(1497) & ChrW(1504) & ChrW(1493) & ChrW(1488) & ChrW(1512), ChrW(1508) & _
        ChrW(1489) & ChrW(1512) & ChrW(1493) & ChrW(1488) & ChrW(1512), ChrW(1502) & ChrW(1512) & ChrW(1509), ChrW(1488) & _
        ChrW(1508) & ChrW(1512) & ChrW(1497) & ChrW(1500), ChrW(1502) & ChrW(1488) & ChrW(1497), ChrW(1497) & ChrW(1493) & _
        ChrW(1504) & ChrW(1497), ChrW(1497) & ChrW(1493) & ChrW(1500) & ChrW(1497), ChrW(1488) & ChrW(1493) & ChrW(1490) & _
        ChrW(1493) & ChrW(1505) & ChrW(1496), ChrW(1505) & ChrW(1508) & ChrW(1496) & ChrW(1502) & ChrW(1489) & _
        ChrW(1512), ChrW(1488) & ChrW(1493) & ChrW(1511) & ChrW(1496) & ChrW(1493) & ChrW(1489) & ChrW(1512), ChrW(1504) & _
        ChrW(1493) & ChrW(1489) & ChrW(1502) & ChrW(1489) & ChrW(1512), ChrW(1491) & ChrW(1510) & ChrW(1502) & ChrW(1489) & _
        ChrW(1512))
100               For monthIdx = 1 To 12
                      monthName = CStr(mArr(monthIdx))
...
Public Sub ApplyDemoModeValidation()
    On Error Resume Next
...
10      Dim wsBranches As Worksheet
...
280     Dim rng As Range
        ...
        If Err.Number <> 0 Then
            On Error GoTo 0
            LogDebug "Calling A00_SetupMainSheet"
    A00_SetupMainSheet
    LogDebug "Finished A00_SetupMainSheet"
        ...
        If SheetExists(chkSheetName) Or SheetExists("base_" & chkBaseYear) Then
            ...
            alreadyMsg = ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1488) & _
        ChrW(1500) & ChrW(1493) & " " & ChrW(1499) & ChrW(1489) & ChrW(1512) & " " & ChrW(1504) & ChrW(1489) & ChrW(1491) & _
        ChrW(1511) & ChrW(1493) & "." & vbCrLf & vbCrLf & _
                ChrW(1488) & ChrW(1501) & " " & ChrW(1489) & ChrW(1512) & ChrW(1510) & ChrW(1493) & ChrW(1504) & _
        ChrW(1498) & " " & ChrW(1500) & ChrW(1489) & ChrW(1491) & ChrW(1493) & ChrW(1511) & " " & ChrW(1513) & ChrW(1504) & _
        ChrW(1497) & ChrW(1501) & " " & ChrW(1488) & ChrW(1495) & ChrW(1512) & ChrW(1493) & ChrW(1514) & "," & vbCrLf & _
                ChrW(1513) & ChrW(1504) & ChrW(1492) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1506) & _
        ChrW(1512) & ChrW(1499) & ChrW(1497) & ChrW(1501) & " " & ChrW(1513) & ChrW(1500) & " " & ChrW(1513) & ChrW(1504) & _
        ChrW(1514) & " " & H_BASE() & " " & ChrW(1493) & ChrW(1513) & ChrW(1504) & ChrW(1492) & " " & ChrW(1504) & ChrW(1493) & _
        ChrW(1499) & ChrW(1495) & ChrW(1497) & ChrW(1514) & "." & vbCrLf & vbCrLf & _
        ...
        ' Resolve defensively: no handler is armed yet at this point, and
        ' ERR_HANDLER further down also reads wsMsgSrc - a missing messages
        ' sheet must not crash the run before it has started.
        On Error Resume Next
        Set wsMsgSrc = ThisWorkbook.Worksheets(H_SET_MESSAGES())
        On Error GoTo 0

        If Not wsMsgSrc Is Nothing Then
            If CStr(wsMsgSrc.Cells(20, 1).Value) <> "1" Then
                confirmMsg = MsgText(wsMsgSrc, 13) & vbNewLine & vbNewLine & MsgText(wsMsgSrc, 14) & vbNewLine & vbNewLine & MsgText(wsMsgSrc, 15) & vbNewLine & vbNewLine & MsgText(wsMsgSrc, 16)
                If MsgBoxU(confirmMsg, vbOKCancel + vbExclamation, MsgText(wsMsgSrc, 8)) <> vbOK Then
                    Exit Sub
                End If
                ' Ask if user wants to keep showing this message
                If MsgBoxU(MsgText(wsMsgSrc, 17), vbYesNo + vbQuestion) = vbNo Then
                    wsMsgSrc.Cells(20, 1).Value = "1"
                End If
            End If
        End If
...
290     On Error GoTo ERR_HANDLER
        ...
        With wsProgress1.Range("G15")
            .Value = ChrW(1502) & ChrW(1506) & ChrW(1489) & ChrW(1491) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & _
        ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1488) & ChrW(1504) & ChrW(1488) & " " & ChrW(1492) & ChrW(1502) & _
        ChrW(1514) & ChrW(1497) & ChrW(1504) & ChrW(1493) & "..."
            .Font.Size = 18
            .Font.Bold = True
        ...
        wsProgress1.Protect DrawingObjects:=False, UserInterfaceOnly:=True
...
370     Application.Calculation = xlCalculationManual
        ...
        ' Keep the reference year's review sheet - ApplyCorrections reads its corrections
        refReviewKeep = REVIEW_SHEET_NAME() & "_" & refYearStr
375     For iSheet = ThisWorkbook.Worksheets.Count To 1 Step -1
376         Set wsCleanup = ThisWorkbook.Worksheets(iSheet)
377         If Not IsProtectedSheet(wsCleanup.Name, refBaseKeep, refBaseKeepOld, refReviewKeep) Then
                On Error Resume Next
...
382     Set wsBranches = ThisWorkbook.Worksheets(H_SET_BRANCHES())
390     Set dictHelper = LoadHelperDictionary(ThisWorkbook.Worksheets(H_SET_REASONS()))
...
470     LoadCheckedFields ThisWorkbook.Worksheets(H_SET_FIELDMAP()), dictFieldCol, dictFieldDisp

480     BuildArrays dictFieldCol, dictFieldDisp, keys, cols, disp, cnt
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
490     If cnt = 0 Then MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1492) & ChrW(1493) & ChrW(1490) & ChrW(1491) & _
        ChrW(1512) & ChrW(1493) & " " & ChrW(1513) & ChrW(1491) & ChrW(1493) & ChrW(1514) & " " & ChrW(1500) & ChrW(1489) & _
        ChrW(1491) & ChrW(1497) & ChrW(1511) & ChrW(1492) & " " & ChrW(1489) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & _
        ChrW(1493) & ChrW(1503) & " " & ChrW(1502) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1497) & _
        ".", vbExclamation: Application.EnableEvents = True: Exit Sub

        Dim dictBranch As Object
495     Set dictBranch = LoadBranchMapping(ThisWorkbook.Worksheets(H_SET_BRANCHES()))
...
500     threshold = GetNumericParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), PARAM_PREMIUM_THRESHOLD)
...
LogDebug "Starting Demo Mode checks"
...
LogDebug "Calling OpenDataSheet"
...
580     If lastRow < 2 Then Err.Raise vbObjectError + 1006, "BuildReview", "NO DATA ROWS IN SOURCE"
    ...
    srcData = wsSrc.Range(wsSrc.Cells(1, 1), wsSrc.Cells(lastRow, 100)).Value2
...
700     outRow = 3
    outIdx = 0
    If lastRow > 1 And cnt > 0 Then ReDim outArr(1 To lastRow * cnt, 1 To cnt + 2)

LogDebug "Starting main row loop (2 to " & lastRow & ")"
710     For r = 2 To lastRow

720         If IsIgnorableRowArr(srcData, r, keys, cols, cnt, dictFieldCol) Then GoTo NextRow

            ' Check missing fields - write one REVIEW row per issue
740         For i = 1 To cnt
750             If IsBlankValue(srcData(r, cols(i))) Then
...
756                 End If
                ...
                outArr(outIdx, 1) = r
761                 For j = 1 To cnt
                    outArr(outIdx, j + 1) = srcData(r, cols(j))
763                 Next j
                outArr(outIdx, cnt + 2) = singleText
766                 outRow = outRow + 1
770             End If
780         Next i

            ' Check premium threshold - separate row (after currency conversion)
790         premiumVal = srcData(r, dictFieldCol(KEY_PREMIUM))
...
812                 currCode = srcData(r, RAW_CURRENCY)
813                 rowBordMonthBR = GetBordMonth(srcData(r, dateColBR))
...
900         brKey = UCase$(Trim$(CStr(srcData(r, RAW_BRANCHNAME))))
910         If brKey <> "" Then
...
936                 outRow = outRow + 1
                    ...
                    brOrigName = Trim$(CStr(srcData(r, RAW_BRANCHNAME)))
                    Dim brLastRow As Long
                    Dim brEndRow As Long
                    On Error Resume Next
                    brEndRow = 1 - 1
                    On Error GoTo ERR_HANDLER
                    If brEndRow < 3 Then brEndRow = wsBranches.Cells(wsBranches.Rows.Count, 1).End(xlUp).Row
                    brLastRow = brEndRow
...
End If
                    If Not brExists Then
                        ...
                        If brLastRow > brEndRow Then
                            wsBranches.Rows(brEndRow).Insert Shift:=xlDown
                            brLastRow = brEndRow
                        End If
                        wsBranches.Cells(brLastRow, 1).Value = brOrigName
                        ' Column B (main branch) left empty - to be filled via correction
                        ' Column C: generate BRANCH_KEY via transliteration
                        wsBranches.Cells(brLastRow, 3).Value = HebrewToKey(brOrigName)
...
LogDebug "Finished main row loop. outIdx = " & outIdx
    ' Dump all errors to wsRev
    If outIdx > 0 Then
        wsRev.Range(wsRev.Cells(3, 1), wsRev.Cells(3 + outIdx - 1, cnt + 2)).Value2 = outArr
    End If

        ' ---- Write unique lists to hidden "reshimot" sheet ----
LogDebug "Writing lists to reshimot..."
...
1080    If Not wbSrc Is ThisWorkbook Then wbSrc.Close SaveChanges:=False
...
LogDebug "Adding btnSendForReview..."
...
LogDebug "Calling UpdatePeriodDropdown"
...
1240    If outRow > 3 Then
            ...
            MsgBoxU MsgText(wsMsgSrc, 2) & (outRow - 3) & MsgText(wsMsgSrc, 3), vbInformation
        Else
            ' No issues found - go straight to home
            MsgBoxU MsgText(wsMsgSrc, 4), vbInformation
            ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Activate
        End If

        ' Update client list in settings sheet (non-critical)
        On Error Resume Next
LogDebug "Calling UpdateClientList"
...
1247    wsProgress1.Range("G15:K15").Interior.Color = RGB(220, 240, 220)
        wsProgress1.Protect DrawingObjects:=False, UserInterfaceOnly:=True
LogDebug "BuildReview ENDED SUCCESSFULLY"
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
1250    Exit Sub
...
1310    If Not wbSrc Is Nothing Then
...
1360    MsgBoxU MsgText(wsMsgSrc, 5) & errLine & vbCrLf & MsgText(wsMsgSrc, 6) & errNum & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
...
20      Dim wsBranches As Worksheet
30      Dim wsRev As Worksheet
40      Dim wbSrc As Workbook
...
430     Dim bordereu As Variant
        ...
        ' Resolve the messages sheet BEFORE arming the handler: ERR_HANDLER
        ' dereferences wsMsgSrc2, so it must never be Nothing by the time we
        ' get there, otherwise the handler itself raises error 91.
        On Error Resume Next
        Set wsMsgSrc2 = ThisWorkbook.Worksheets(H_SET_MESSAGES())
        On Error GoTo 0
...
440     On Error GoTo ERR_HANDLER
        ...
        If SheetExists(chkSheets2) Then
            ...
            ' "dochot elu kvar keiyamim. nitan litzpot bahem bekaftor 6. ha'im leharitz shuv?"
            alreadyMsg2 = ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1488) & ChrW(1500) & _
        ChrW(1493) & " " & ChrW(1499) & ChrW(1489) & ChrW(1512) & " " & ChrW(1511) & ChrW(1497) & ChrW(1497) & ChrW(1502) & _
        ChrW(1497) & ChrW(1501) & "." & vbCrLf & _
                ChrW(1504) & ChrW(1497) & ChrW(1514) & ChrW(1503) & " " & ChrW(1500) & ChrW(1510) & ChrW(1508) & _
        ChrW(1493) & ChrW(1514) & " " & ChrW(1489) & ChrW(1492) & ChrW(1501) & " " & ChrW(1489) & ChrW(1499) & ChrW(1508) & _
        ChrW(1514) & ChrW(1493) & ChrW(1512) & " 6." & vbCrLf & vbCrLf & _
                ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1500) & ChrW(1492) & ChrW(1512) & ChrW(1497) & ChrW(1509) & " " & ChrW(1513) & ChrW(1493) & ChrW(1489) & "?"
        ...
        If ptVal <> "" And ptVal <> selText Then
            ...
            If Not isYearly Then
                ...
                If pvVal = "" Or pvVal = selText Then
                    MsgBoxU ChrW(1497) & ChrW(1513) & " " & ChrW(1500) & ChrW(1489) & ChrW(1495) & ChrW(1493) & _
        ChrW(1512) & " " & ChrW(1506) & ChrW(1512) & ChrW(1498) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & _
        ChrW(1492) & " " & ChrW(1489) & ChrW(1514) & ChrW(1488) & " G6", vbExclamation
                    Exit Sub
                End If
            End If
        End If
        ...
        If ftVal <> "" And ftVal <> selText Then
            ...
            If fvVal = "" Or fvVal = selText Then
                MsgBoxU ChrW(1497) & ChrW(1513) & " " & ChrW(1500) & ChrW(1489) & ChrW(1495) & ChrW(1493) & ChrW(1512) & _
        " " & ChrW(1506) & ChrW(1512) & ChrW(1498) & " " & ChrW(1505) & ChrW(1497) & ChrW(1504) & ChrW(1493) & ChrW(1503) & " " & _
        ChrW(1489) & ChrW(1514) & ChrW(1488) & " G9", vbExclamation
                Exit Sub
            End If
        ...
        End If
        ' --- End validation ---
        ' (wsMsgSrc2 is now resolved near the top, before On Error GoTo ERR_HANDLER)
...
470     wsProgress.Range("G15:K15").UnMerge
        ...
        End With
        wsProgress.Activate
        wsProgress.Protect DrawingObjects:=False, UserInterfaceOnly:=True
        Application.ScreenUpdating = True
        DoEvents
        Application.ScreenUpdating = False
...
580     Set wsBranches = ThisWorkbook.Worksheets(H_SET_BRANCHES())
...
640     Set dictHelper = LoadHelperDictionary(ThisWorkbook.Worksheets(H_SET_REASONS()))

650     debugStep = "LOAD_BRANCH"
660     Set dictBranch = LoadBranchMapping(ThisWorkbook.Worksheets(H_SET_BRANCHES()))

670     debugStep = "GET_THRESHOLD"
680     threshold = GetNumericParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), PARAM_PREMIUM_THRESHOLD)
...
1090        Set wsSrc = OpenDataSheet(wbSrc)
    ...
    BuildBaseSheet wsSrc, wsBase, baseSheetName, _
        ...
        dictBranch, _
        minMonth, maxMonth, filterCol, filterValue, dollarRate, dateCol, threshold, wsMain, yearVal, countCurrent

        ' Reusing isDemoMode for Ref source
isDemoMode = FORCE_DEMO_MODE
        demoParam = UCase$(Trim$(GetStringParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), "DEMO_MODE")))
        If Not FORCE_DEMO_MODE Then
            If demoParam = ChrW(1499) & ChrW(1503) Or demoParam = "YES" Or demoParam = "" Then isDemoMode = True
    ...
    BuildBaseSheet wsRef, wsBaseRef, refBaseSheetName, _
        ...
        dictBranch, _
        minMonth, maxMonth, filterCol, filterValue, dollarRate, dateCol, threshold, wsMain, refYear, countRef

2630    debugStep = "BUILD_COMPANIES"
2640    BuildComparisonSheet wsBase, wsBaseRef, SHEET_COMPANIES(), BASE_COL_COMPANY, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_COMPANIES(), clientFilter
        On Error Resume Next
...
2660    BuildComparisonSheet wsBase, wsBaseRef, SHEET_BRANCH(), BASE_COL_BRANCHNAME, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_BRANCH(), clientFilter
        On Error Resume Next
...
2666    BuildComparisonSheet wsBase, wsBaseRef, SHEET_MAINBRANCH(), BASE_COL_MAINBRANCH, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_MAINBRANCH(), clientFilter
        On Error Resume Next
...
2680    BuildComparisonSheet wsBase, wsBaseRef, SHEET_TELLERS(), BASE_COL_TELLER, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_TELLERS(), clientFilter
        On Error Resume Next
...
2700    BuildComparisonSheet wsBase, wsBaseRef, SHEET_AGENTS(), BASE_COL_AGENTNAME, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_AGENTS(), clientFilter
        On Error Resume Next
...
2720    BuildComparisonSheet wsBase, wsBaseRef, SHEET_MONTHS(), BASE_COL_MONTH, minMonth, maxMonth, yearVal, refYear, filterCol, filterValue, SHEET_MONTHS(), clientFilter
        On Error Resume Next
...
2740    BuildSummarySheet countRef, countCurrent, reviewCount, corrCount, ignoreCount, unhandledCount, yearVal, refYear, periodDesc, SHEET_SUMMARY()
...
2785    On Error Resume Next
        ...
        End With
        ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Protect DrawingObjects:=False, UserInterfaceOnly:=True
        On Error GoTo 0
...
UpdateClientList
' REMOVED: Call ShowHiddenSheets (was unhiding all base/data sheets incorrectly)
...
2960    MsgBoxU MsgText(wsMsgSrc2, 5) & errLine & vbCrLf & MsgText(wsMsgSrc2, 6) & errNum & vbCrLf & MsgText(wsMsgSrc2, 7) & debugStep & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
...
' ============================================================================
Private Sub LoadCorrectionsToDicts(ByVal wsBranches As Worksheet, ByVal yearStr As String, ByVal dictHelper As Object, _
...
960                     corrCount = corrCount + 1
                        ...
                        If InStr(1, reasonText, unknownBrText, vbTextCompare) > 0 And fixText <> "" Then
                            ...
                            brFixLast = 1 - 1
                            On Error GoTo 0
                            If brFixLast < 3 Then brFixLast = wsBranches.Cells(wsBranches.Rows.Count, 1).End(xlUp).Row
                            For brFixRow = 3 To brFixLast
                                If Trim$(CStr(wsBranches.Cells(brFixRow, 2).Value2)) = "" Then
                                    ' This is a branch without a main branch assigned
                                    ' Write the main branch
                                    wsBranches.Cells(brFixRow, 2).Value = fixText
                                    ' Find MAIN_BRANCH_KEY from existing rows with same main branch
                                    Dim mbKeyRow As Long
                                    For mbKeyRow = 3 To brFixLast
                                        If StrComp(Trim$(CStr(wsBranches.Cells(mbKeyRow, 2).Value2)), fixText, vbTextCompare) = 0 And mbKeyRow <> brFixRow Then
                                            If Trim$(CStr(wsBranches.Cells(mbKeyRow, 4).Value2)) <> "" Then
                                                wsBranches.Cells(brFixRow, 4).Value = wsBranches.Cells(mbKeyRow, 4).Value2
                                                Exit For
                                            End If
                                        End If
                                    Next mbKeyRow
                                    ' If no existing key found, generate from fixText
                                    If Trim$(CStr(wsBranches.Cells(brFixRow, 4).Value2)) = "" Then
                                        wsBranches.Cells(brFixRow, 4).Value = HebrewToKey(fixText)
                                    End If
                                    Exit For
                                End If
                            Next brFixRow
...
1436                End If
1440                ' Removed buggy premVal filter
1450            End If
1460        End If
...
320     wsOut.Cells(10, 1).Value = ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & _
        ChrW(1501) & " " & ChrW(1513) & ChrW(1497) & ChrW(1493) & ChrW(1513) & ChrW(1502) & ChrW(1493)
330     wsOut.Cells(10, 2).Value = corrCount
340     wsOut.Cells(11, 1).Value = ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1513) & _
        ChrW(1492) & ChrW(1493) & ChrW(1514) & ChrW(1506) & ChrW(1500) & ChrW(1502) & ChrW(1493)
350     wsOut.Cells(11, 2).Value = ignoreCount
360     wsOut.Cells(12, 1).Value = ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1500) & _
        ChrW(1500) & ChrW(1488) & " " & ChrW(1496) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1500)
...
150     If Not dict.Exists("MISSING_CUSTOMER_NUMBER") Then dict("MISSING_CUSTOMER_NUMBER") = ChrW(1495) & ChrW(1505) & _
        ChrW(1512) & " " & ChrW(1502) & ChrW(1505) & ChrW(1508) & ChrW(1512) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & _
        ChrW(1495)
151     If Not dict.Exists("REVIEW_SOURCE_ROW_HEADER") Then dict("REVIEW_SOURCE_ROW_HEADER") = ChrW(1513) & ChrW(1493) & _
        ChrW(1512) & ChrW(1514) & " " & ChrW(1502) & ChrW(1511) & ChrW(1493) & ChrW(1512)
152     If Not dict.Exists("REVIEW_REASON_HEADER") Then dict("REVIEW_REASON_HEADER") = ChrW(1505) & ChrW(1497) & _
        ChrW(1489) & ChrW(1514) & " " & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1492)
153     If Not dict.Exists("REVIEW_REASON_CODE_HEADER") Then dict("REVIEW_REASON_CODE_HEADER") = ChrW(1511) & ChrW(1493) & _
        ChrW(1491) & " " & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1492)
154     If Not dict.Exists("MISSING_CUSTOMER_NAME") Then dict("MISSING_CUSTOMER_NAME") = ChrW(1495) & ChrW(1505) & _
        ChrW(1512) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495)
155     If Not dict.Exists("MISSING_POLICY") Then dict("MISSING_POLICY") = ChrW(1495) & ChrW(1505) & ChrW(1512) & " " & ChrW(1508) & ChrW(1493) & ChrW(1500) & ChrW(1497) & ChrW(1505) & ChrW(1492)
...
174     If Not dict.Exists("PREMIUM_NOT_NUMERIC") Then dict("PREMIUM_NOT_NUMERIC") = ChrW(1506) & ChrW(1512) & _
        ChrW(1498) & " " & H_PREMIUM() & " " & ChrW(1500) & ChrW(1488) & " " & ChrW(1502) & ChrW(1505) & ChrW(1508) & ChrW(1512) & _
        ChrW(1497)
...
isDemoMode = FORCE_DEMO_MODE
    ...
    If isDemoMode Then
        MsgBoxU ChrW(1492) & ChrW(1488) & ChrW(1508) & ChrW(1513) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & _
        ChrW(1494) & ChrW(1502) & ChrW(1497) & ChrW(1504) & ChrW(1492) & " " & ChrW(1489) & ChrW(1490) & ChrW(1512) & ChrW(1505) & _
        ChrW(1492) & " " & ChrW(1492) & ChrW(1502) & ChrW(1500) & ChrW(1488) & ChrW(1492) & ".", vbInformation, ChrW(1502) & _
        ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1514) & " " & ChrW(1491) & ChrW(1502) & ChrW(1493)
        CheckDemoLock = True
    Else
        CheckDemoLock = False
    End If
...
Public Sub A00_SetupMainSheet()
    ...
    ' --- Failsafe: Force ActiveSheet to be Main to prevent RTL shape bug ---
    On Error Resume Next
    If ActiveSheet.Name <> CONTROL_SHEET_NAME() And ActiveSheet.Name <> "Main" Then
        Application.GoTo ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Range("G10")
        DoEvents
    End If
    On Error GoTo 0
    ' -----------------------------------------------------------------------
...
330 wsMain.Range("A1:Z50").UnMerge
340 wsMain.Range("A1:Z1").ClearContents
350     wsMain.Range("A2:L50").ClearContents
    ' Clear all existing rounded rectangle buttons to avoid duplicates if run multiple times
    Dim shpBtn As Shape
360     For Each shpBtn In wsMain.Shapes
370         If shpBtn.Type = 5 Then ' msoShapeRoundedRectangle
380             shpBtn.Delete
390         End If
400     Next shpBtn
410     wsMain.Cells.Font.Size = 14
420     wsMain.Cells.Interior.ColorIndex = xlNone
430     wsMain.Range("A1:AZ40").Interior.Color = RGB(220, 240, 220)
440     wsMain.Range("A2:K20").Borders.LineStyle = xlNone

    ' ---- CREATE 8 SETTINGS SHEETS ----
450     shtNames = Array(H_SET_FIELDMAP(), H_SET_BRANCHES(), H_SET_PARAMS(), H_SET_PERIODS(), H_SET_MESSAGES(), H_SET_PERMISSIONS(), H_SET_REASONS(), H_SET_CLIENTS())
460     shtHeaders = Array(ChrW(1502) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1497) & " " & ChrW(1513) & ChrW(1491) & ChrW(1493) & ChrW(1514), _
                       ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1506) & ChrW(1504) & ChrW(1508) & ChrW(1497) & ChrW(1501), _
                       ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1496) & ChrW(1512) & ChrW(1497) & ChrW(1501) & " " & ChrW(1499) & ChrW(1500) & ChrW(1500) & ChrW(1497) & ChrW(1497) & ChrW(1501), _
                       ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1493) & ChrW(1514), _
                       ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1493) & ChrW(1514) & " " & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1514), _
                       ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514) & " " & ChrW(1502) & ChrW(1513) & ChrW(1514) & ChrW(1502) & ChrW(1513) & ChrW(1497) & ChrW(1501), _
                       ChrW(1505) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1514) & " " & ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492), _
                       ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514))
    
470     For iIdx = 0 To UBound(shtNames)
480         On Error Resume Next
490         Set wsTemp = ThisWorkbook.Worksheets(shtNames(iIdx))
500         If Err.Number <> 0 Then
510             Set wsTemp = ThisWorkbook.Worksheets.Add(After:=ThisWorkbook.Worksheets(ThisWorkbook.Worksheets.Count))
520             wsTemp.Name = shtNames(iIdx)
530             wsTemp.Cells(1, 1).Value = shtHeaders(iIdx)
540             wsTemp.Cells(1, 1).Font.Bold = True
550             wsTemp.Cells(1, 1).Font.Size = 14
560         End If
570         Err.Clear
580         On Error GoTo ERR_HANDLER
590     Next iIdx

    ' ---- Period Lists ----
600     Set wsTemp = ThisWorkbook.Worksheets(H_SET_PERIODS())
610     wsTemp.Range("A2:A50").ClearContents
620     wsTemp.Cells(2, 1).Value = "PERIOD_TYPE"
630     wsTemp.Cells(3, 1).Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
640     wsTemp.Cells(4, 1).Value = ChrW(1495) & ChrW(1510) & ChrW(1497) & " " & ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
650     wsTemp.Cells(5, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503)
660     wsTemp.Cells(6, 1).Value = ChrW(1495) & ChrW(1493) & ChrW(1491) & ChrW(1513) & ChrW(1497)
670     wsTemp.Cells(7, 1).Value = "HALF_YEAR"
680     wsTemp.Cells(8, 1).Value = ChrW(1502) & ChrW(1495) & ChrW(1510) & ChrW(1497) & ChrW(1514) & " " & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1493) & ChrW(1504) & ChrW(1492)
690     wsTemp.Cells(9, 1).Value = ChrW(1502) & ChrW(1495) & ChrW(1510) & ChrW(1497) & ChrW(1514) & " " & ChrW(1513) & ChrW(1504) & ChrW(1497) & ChrW(1492)
    
700     On Error Resume Next
710     ThisWorkbook.Names("lst_period_type").Delete
720     ThisWorkbook.Names("lst_half_year").Delete
730     ThisWorkbook.Names("lst_quarter").Delete
740     ThisWorkbook.Names("lst_month").Delete
750     Err.Clear
760     On Error GoTo ERR_HANDLER
770     ThisWorkbook.Names.Add "lst_empty", wsTemp.Range(wsTemp.Cells(50, 1), wsTemp.Cells(50, 1))
780 ThisWorkbook.Names.Add "lst_period_type", wsTemp.Range(wsTemp.Cells(3, 1), wsTemp.Cells(6, 1))
790     ThisWorkbook.Names.Add "lst_half_year", wsTemp.Range(wsTemp.Cells(8, 1), wsTemp.Cells(9, 1))
    
800     wsTemp.Cells(11, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1493) & ChrW(1503)
810     wsTemp.Cells(12, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1513) & ChrW(1504) & ChrW(1497)
820     wsTemp.Cells(13, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1513) & ChrW(1500) & ChrW(1497) & ChrW(1513) & ChrW(1497)
830     wsTemp.Cells(14, 1).Value = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503) & " " & ChrW(1512) & ChrW(1489) & ChrW(1497) & ChrW(1506) & ChrW(1497)
840     ThisWorkbook.Names.Add "lst_quarter", wsTemp.Range(wsTemp.Cells(11, 1), wsTemp.Cells(14, 1))
    
850     wsTemp.Cells(16, 1).Value = ChrW(1497) & ChrW(1504) & ChrW(1493) & ChrW(1488) & ChrW(1512)
860     wsTemp.Cells(17, 1).Value = ChrW(1508) & ChrW(1489) & ChrW(1512) & ChrW(1493) & ChrW(1488) & ChrW(1512)
870     wsTemp.Cells(18, 1).Value = ChrW(1502) & ChrW(1512) & ChrW(1509)
880     wsTemp.Cells(19, 1).Value = ChrW(1488) & ChrW(1508) & ChrW(1512) & ChrW(1497) & ChrW(1500)
890     wsTemp.Cells(20, 1).Value = ChrW(1502) & ChrW(1488) & ChrW(1497)
900     wsTemp.Cells(21, 1).Value = ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1497)
910     wsTemp.Cells(22, 1).Value = ChrW(1497) & ChrW(1493) & ChrW(1500) & ChrW(1497)
920     wsTemp.Cells(23, 1).Value = ChrW(1488) & ChrW(1493) & ChrW(1490) & ChrW(1493) & ChrW(1505) & ChrW(1496)
930     wsTemp.Cells(24, 1).Value = ChrW(1505) & ChrW(1508) & ChrW(1496) & ChrW(1502) & ChrW(1489) & ChrW(1512)
940     wsTemp.Cells(25, 1).Value = ChrW(1488) & ChrW(1493) & ChrW(1511) & ChrW(1496) & ChrW(1493) & ChrW(1489) & ChrW(1512)
950     wsTemp.Cells(26, 1).Value = ChrW(1504) & ChrW(1493) & ChrW(1489) & ChrW(1502) & ChrW(1489) & ChrW(1512)
960     wsTemp.Cells(27, 1).Value = ChrW(1491) & ChrW(1510) & ChrW(1502) & ChrW(1489) & ChrW(1512)
970     ThisWorkbook.Names.Add "lst_month", wsTemp.Range(wsTemp.Cells(16, 1), wsTemp.Cells(27, 1))

Dim tHalf As String, tQuarter As String, tMonth As String
980 tHalf = ChrW(1495) & ChrW(1510) & ChrW(1497) & " " & ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
990 tQuarter = ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503)
1000 tMonth = ChrW(1495) & ChrW(1493) & ChrW(1491) & ChrW(1513) & ChrW(1497)
' --- Build Maps for Native Dropdowns ---
1010 wsTemp.Cells(1, 5).Value = tHalf: wsTemp.Cells(1, 6).Value = "lst_half_year"
1020 wsTemp.Cells(2, 5).Value = tQuarter: wsTemp.Cells(2, 6).Value = "lst_quarter"
1030 wsTemp.Cells(3, 5).Value = tMonth: wsTemp.Cells(3, 6).Value = "lst_month"
1040 wsTemp.Cells(4, 5).Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497): wsTemp.Cells(4, 6).Value = "lst_empty"
1050 ThisWorkbook.Names.Add "map_period", wsTemp.Range("E1:F4")

1060 wsTemp.Cells(1, 7).Value = H_COMPANY(): wsTemp.Cells(1, 8).Value = "lst_companies"
1070 wsTemp.Cells(2, 7).Value = H_AGENT(): wsTemp.Cells(2, 8).Value = "lst_agents"
1080 wsTemp.Cells(3, 7).Value = H_TELLER(): wsTemp.Cells(3, 8).Value = "lst_tellers"
1090 wsTemp.Cells(4, 7).Value = H_BRANCH(): wsTemp.Cells(4, 8).Value = "lst_branches"
1100 wsTemp.Cells(5, 7).Value = H_BRANCH() & " " & ChrW(1502) & ChrW(1512) & ChrW(1499) & ChrW(1494): wsTemp.Cells(5, 8).Value = "lst_main_branches"
1110 ThisWorkbook.Names.Add "map_filter", wsTemp.Range("G1:H5")
1111         Dim yIdx As Long
1112         For yIdx = 1 To 20
1113             wsTemp.Cells(yIdx, 9).Value = 2020 + yIdx
1114         Next yIdx
1115         ThisWorkbook.Names.Add "lst_years", wsTemp.Range("I1:I20")
1120     wsTemp.Cells(28, 1).Value = EOD_MARKER
    
    ' ---- Layout setup for Home Sheet ----
    ' Parameters Table (F2:G10)
1130     wsMain.Range("F2").Value = ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1496) & ChrW(1512)
1140     wsMain.Range("G2").Value = ChrW(1506) & ChrW(1512) & ChrW(1498)
1150     Set hdrRng = wsMain.Range("F2:G2")
1160     hdrRng.Interior.Color = RGB(0, 100, 0)
1170     hdrRng.Font.Color = RGB(255, 255, 255)
1180     hdrRng.Font.Bold = True
1190     hdrRng.Font.Size = 12
1200     hdrRng.HorizontalAlignment = xlCenter
    
1210     wsMain.Range("F3").Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & " " & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505)
1220     wsMain.Range("F4").Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & " " & ChrW(1504) & ChrW(1493) & ChrW(1499) & ChrW(1495) & ChrW(1497) & ChrW(1514)
1230     wsMain.Range("F5").Value = ChrW(1505) & ChrW(1493) & ChrW(1490) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1492)
1240     wsMain.Range("F6").Value = ChrW(1506) & ChrW(1512) & ChrW(1498) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1492)
1250     wsMain.Range("F7").Value = ChrW(1505) & ChrW(1493) & ChrW(1490) & " " & ChrW(1514) & ChrW(1488) & ChrW(1512) & ChrW(1497) & ChrW(1498)
1260     wsMain.Range("F8").Value = ChrW(1495) & ChrW(1497) & ChrW(1514) & ChrW(1493) & ChrW(1498) & " " & ChrW(1500) & ChrW(1508) & ChrW(1497)
1270     wsMain.Range("F9").Value = ChrW(1506) & ChrW(1512) & ChrW(1498) & " " & ChrW(1495) & ChrW(1497) & ChrW(1514) & ChrW(1493) & ChrW(1498)
1280     wsMain.Range("F10").Value = ChrW(1513) & ChrW(1501) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495)
    
1290     wsMain.Range("F3:F10").Font.Color = RGB(0, 70, 140)
1300     wsMain.Range("F3:F10").Font.Bold = True
1310     wsMain.Range("F3:F10").Interior.Color = RGB(200, 230, 255)
1320     wsMain.Range("G3:G10").Interior.Color = RGB(255, 245, 230)
1330     wsMain.Range("F3:G10").HorizontalAlignment = xlCenter
1340     wsMain.Range("F3:F10").HorizontalAlignment = xlRight
1350     wsMain.Range("G3:G12").Locked = False
    
1360     On Error Resume Next
1370     ThisWorkbook.Names("rngCurrentYear").Delete
1380     ThisWorkbook.Names("rngBaseYear").Delete
1390     ThisWorkbook.Names("rngPeriodType").Delete
1400     ThisWorkbook.Names("rngPeriodValue").Delete
1410     ThisWorkbook.Names("rngDateType").Delete
1420     ThisWorkbook.Names("rngFilterType").Delete
1430     ThisWorkbook.Names("rngFilterValue").Delete
1440     ThisWorkbook.Names("rngClientName").Delete
1450     Err.Clear
1460     On Error GoTo ERR_HANDLER
    
1470     ThisWorkbook.Names.Add "rngBaseYear", wsMain.Range("G3")
1480     ThisWorkbook.Names.Add "rngCurrentYear", wsMain.Range("G4")
1490     ThisWorkbook.Names.Add "rngPeriodType", wsMain.Range("G5")
1500     ThisWorkbook.Names.Add "rngPeriodValue", wsMain.Range("G6")
1510     ThisWorkbook.Names.Add "rngDateType", wsMain.Range("G7")
1520     ThisWorkbook.Names.Add "rngFilterType", wsMain.Range("G8")
1530     ThisWorkbook.Names.Add "rngFilterValue", wsMain.Range("G9")
1540     ThisWorkbook.Names.Add "rngClientName", wsMain.Range("G10")
    
    ' Set defaults
1550     If IsEmpty(wsMain.Range("G3").Value) Then wsMain.Range("G3").Value = 2024
1560     If IsEmpty(wsMain.Range("G4").Value) Then wsMain.Range("G4").Value = 2025

    Dim isDemoMode As Boolean
    Dim demoParam As String
1570 isDemoMode = FORCE_DEMO_MODE
1580     On Error Resume Next
1590     demoParam = UCase$(Trim$(GetStringParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), "DEMO_MODE")))
1600         If Not FORCE_DEMO_MODE Then
1610             If demoParam = ChrW(1499) & ChrW(1503) Or demoParam = "YES" Then isDemoMode = True
1620             If demoParam = ChrW(1500) & ChrW(1488) Or demoParam = "NO" Then isDemoMode = False
1630         End If
1640     On Error GoTo ERR_HANDLER

    ' Lock years if demo mode
1650     If isDemoMode Then
...
1742 wsMain.Range("G4").MergeArea.Interior.Color = RGB(230, 230, 230)
    ...
    On Error GoTo ERR_HANDLER    ' Pink instruction message box removed in V3.22 per user request
...
1780         .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=lst_years"
1790         .IgnoreBlank = True
1800         .InCellDropdown = True
1810     End With
    Err.Clear
...
1830     End If
1840     If IsEmpty(wsMain.Range("G5").Value) Then wsMain.Range("G5").Value = ChrW(1513) & ChrW(1504) & ChrW(1514) & ChrW(1497)
...
1880     wsMain.Range("G5").MergeArea.Validation.Delete
1890     wsMain.Range("G5").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=lst_period_type"
1900     wsMain.Range("G6").MergeArea.Validation.Delete
    
1910 wsMain.Range("G6").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=lst_empty"
    
1920     wsMain.Range("G7").MergeArea.Validation.Delete
1930     dateTypeList = ChrW(1489) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1512) & ChrW(1493) & "," & ChrW(1514) & _
        ChrW(1495) & ChrW(1497) & ChrW(1500) & ChrW(1514) & " " & ChrW(1489) & ChrW(1497) & ChrW(1496) & ChrW(1493) & _
        ChrW(1495)
1940     wsMain.Range("G7").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=dateTypeList
1950         wsMain.Range("G7").MergeArea.Value = ChrW(1489) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1512) & ChrW(1493)
1960     wsMain.Range("G8").MergeArea.Value = ChrW(1489) & ChrW(1495) & ChrW(1512) & "/" & ChrW(1497)
1970     wsMain.Range("G8").MergeArea.Validation.Delete
1980     filterTypeList = ChrW(1489) & ChrW(1495) & ChrW(1512) & ChrW(47) & ChrW(1497) & "," & H_COMPANY() & "," & _
        H_TELLER() & "," & H_AGENT() & "," & H_BRANCH() & "," & H_BRANCH() & " " & ChrW(1502) & ChrW(1512) & ChrW(1499) & _
        ChrW(1494)
1990     wsMain.Range("G8").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=filterTypeList
2000     wsMain.Range("G9").MergeArea.Validation.Delete
    
    Dim tComp As String, tTeller As String, tAgent As String, tBranch As String, tMainBranch As String
2010     tComp = H_COMPANY(): tTeller = H_TELLER(): tAgent = H_AGENT(): tBranch = H_BRANCH(): tMainBranch = H_BRANCH() & " " & ChrW(1502) & ChrW(1512) & ChrW(1499) & ChrW(1494)
2020 wsMain.Range("G9").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=lst_empty"
2030     Err.Clear
2040     On Error GoTo ERR_HANDLER
    
    ' ---- Currency Table J2:K4 ----
2050     wsMain.Range("J2").Value = ChrW(1502) & ChrW(1496) & ChrW(1489) & ChrW(1506)
2060     wsMain.Range("K2").Value = ChrW(1513) & ChrW(1506) & ChrW(1512)
2070     wsMain.Range("J2:K2").Interior.Color = RGB(0, 100, 0)
2080     wsMain.Range("J2:K2").Font.Color = RGB(255, 255, 255)
2090     wsMain.Range("J2:K2").Font.Bold = True
2100     wsMain.Range("J2:K2").Font.Size = 12
2110     wsMain.Range("J2:K2").HorizontalAlignment = xlCenter
2120     wsMain.Range("J3").Value = "$" & " " & ChrW(1491) & ChrW(1493) & ChrW(1500) & ChrW(1512)
2130     wsMain.Range("J4").Value = ChrW(8364) & " " & ChrW(1488) & ChrW(1497) & ChrW(1512) & ChrW(1493)
2140     wsMain.Range("J3:J4").Font.Bold = True
2150     wsMain.Range("K3:K4").HorizontalAlignment = xlCenter
2160     wsMain.Range("J3:J4").Interior.Color = RGB(200, 230, 255)
2170     wsMain.Range("K3:K4").Interior.Color = RGB(255, 245, 230)
    
2180     On Error Resume Next
2190     ThisWorkbook.Names("rngDOLAR").Delete
2200     Err.Clear
2210     On Error GoTo ERR_HANDLER
2220     ThisWorkbook.Names.Add "rngDOLAR", wsMain.Range("K3")
    
2230     curRate = FetchBOIRate()
2240     If curRate > 0 Then
2250         wsMain.Range("K3").Value = curRate
2260     ElseIf IsEmpty(wsMain.Range("K3").Value) Then
2270         wsMain.Range("K3").Value = 3.002
2280     End If
2290     wsMain.Range("K3").NumberFormat = "0.0000"
    
2300     eurRate = FetchBOIMonthlyAvg("EUR", "")
2310     If eurRate > 0 Then
2320         wsMain.Range("K4").Value = eurRate
2330     ElseIf IsEmpty(wsMain.Range("K4").Value) Then
2340         wsMain.Range("K4").Value = 3.4239
2350     End If
2360     wsMain.Range("K4").NumberFormat = "0.0000"
2370     wsMain.Range("K3:K4").HorizontalAlignment = xlCenter
    
2380     matachMsg = ChrW(1492) & ChrW(1513) & ChrW(1506) & ChrW(1512) & " " & _
...
2390     wsMain.Range("J5:L7").UnMerge
2400     wsMain.Range("J5:K7").ClearContents
2410     wsMain.Range("J5").Value = matachMsg
2420     wsMain.Range("J5:K7").Merge
2430     wsMain.Range("J5").WrapText = True
2440     wsMain.Range("J5").Font.Size = 11
2450     wsMain.Range("J5").Font.Color = RGB(0, 70, 140)
2460     wsMain.Range("J5").ShrinkToFit = False
2470     wsMain.Range("J5").VerticalAlignment = xlCenter
2480     wsMain.Range("J5").HorizontalAlignment = xlCenter
2490     wsMain.Range("J5:K7").Interior.Color = RGB(220, 230, 255)
    
    ' ---- Formatting Columns ----
2500     wsMain.Columns("A").ColumnWidth = 43
2510     wsMain.Columns("B").ColumnWidth = 8
2520     wsMain.Columns("C").ColumnWidth = 50
2530     wsMain.Columns("D").ColumnWidth = 8
2540     wsMain.Columns("E").ColumnWidth = 8
2550     wsMain.Columns("F").ColumnWidth = 14
2560     wsMain.Columns("G").ColumnWidth = 24
2570     wsMain.Columns("H").ColumnWidth = 13
2580     wsMain.Columns("I").ColumnWidth = 13
2590     wsMain.Columns("J").ColumnWidth = 12
2600     wsMain.Columns("K").ColumnWidth = 12
2610     wsMain.Columns("L").ColumnWidth = 45
2620     wsMain.Rows("1:24").RowHeight = 22
2630     wsMain.Rows("1").RowHeight = 120
    

    ' ---- Borders ----
2640     For Each cellBorder In wsMain.Range("F2:G10")
2650         For Each bEdge In Array(xlEdgeLeft, xlEdgeTop, xlEdgeBottom, xlEdgeRight)
2660             cellBorder.Borders(bEdge).LineStyle = xlContinuous
2670             cellBorder.Borders(bEdge).Color = RGB(0, 70, 140)
2680             cellBorder.Borders(bEdge).Weight = xlThin
2690         Next bEdge
2700     Next cellBorder
2710     For Each cellBorder In wsMain.Range("J2:K4")
2720         For Each bEdge In Array(xlEdgeLeft, xlEdgeTop, xlEdgeBottom, xlEdgeRight)
2730             cellBorder.Borders(bEdge).LineStyle = xlContinuous
2740             cellBorder.Borders(bEdge).Color = RGB(0, 70, 140)
2750             cellBorder.Borders(bEdge).Weight = xlThin
2760         Next bEdge
2770     Next cellBorder
2780     wsMain.Range("J5:K7").BorderAround LineStyle:=xlContinuous, Weight:=xlThin, Color:=RGB(0, 70, 140)
    
    ' ---- Remove ALL old buttons and stubborn floating shapes ----
...
2850     btnW = 140
2860     btnH = 35
2870     btnLeft = wsMain.Range("C2").Left + (wsMain.Range("C2").Width - btnW) / 2
    
2880     Set shp = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, btnLeft, wsMain.Range("C2").Top, btnW, btnH)
...
2950     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
2955     shp.TextFrame2.MarginRight = 10
...
3000     shp.TextFrame2.TextRange.Text = "2 - " & ChrW(1497) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1501) & " " & _
        ChrW(1493) & ChrW(1491) & ChrW(1493) & ChrW(34) & ChrW(1495) & ChrW(1493) & ChrW(1514)
...
3040     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3045     shp.TextFrame2.MarginRight = 10
...
3090     shp.TextFrame2.TextRange.Text = "3 - " & ChrW(1492) & ChrW(1510) & ChrW(1490) & " " & ChrW(1490) & ChrW(1497) & _
        ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & " " & ChrW(1491) & ChrW(1497) & ChrW(1493) & _
        ChrW(1493) & ChrW(1495)
...
3130     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3135     shp.TextFrame2.MarginRight = 10
...
3220     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3225     shp.TextFrame2.MarginRight = 10
...
3310     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3315     shp.TextFrame2.MarginRight = 10
...
3360     shp.TextFrame2.TextRange.Text = "6 - " & ChrW(1510) & ChrW(1508) & ChrW(1497) & ChrW(1497) & ChrW(1492) & " " & _
        ChrW(1489) & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1513) & ChrW(1502) & ChrW(1493) & _
        ChrW(1512) & ChrW(1497) & ChrW(1501)
...
3400     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3405     shp.TextFrame2.MarginRight = 10
...
3418     shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignRight
3423     shp.TextFrame2.MarginRight = 10
3419     shp.OnAction = "NewClients"

    ' ---- Helper buttons ----
    ' Add Large Red Title for Agency Name
3420     wsMain.Range("A1:F1").Merge
3430     wsMain.Range("A1").Formula = "=GetActiveAgencyName()"
3440     wsMain.Range("A1").Font.Size = 28
3450     wsMain.Range("A1").Font.Bold = True
    wsMain.Range("A1").Font.Color = RGB(200, 0, 0) ' Always Red
    
3470     wsMain.Range("A1").HorizontalAlignment = xlCenter
3480     wsMain.Range("A1").VerticalAlignment = xlCenter

3490     Set btnSearch = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, wsMain.Range("G10").Left, wsMain.Range("G10").Top, wsMain.Range("G10").Width, wsMain.Range("G10").Height)
...
3680 btnReset.Name = "btnResetDefaults"
3690     btnReset.TextFrame2.TextRange.Text = ChrW(1488) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1505) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501)
3700     btnReset.TextFrame2.TextRange.Font.Size = 11
3710     btnReset.TextFrame2.TextRange.Font.Bold = msoTrue
3720     btnReset.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
3730     btnReset.Fill.ForeColor.RGB = RGB(180, 50, 50)
3740     btnReset.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
3750     btnReset.OnAction = "ResetHomeDefaults"
    
    ' Show/Hide Sheets in L19
    
    ' ---- Add Exit System Button (Row 21) ----
3850     On Error Resume Next
3860     wsMain.Shapes("btnNavExit").Delete
3870     On Error GoTo 0

    ' ---- Write Credit Text to F20 ----
' 3970 wsMain.Range("F19").ClearContents

    ' --- Add System Tools Block under Main Menu ---
    ...
    On Error Resume Next
    ' wsMain.Shapes("shpManualMsg").Delete ' KEPT FOR V2.272
    On Error GoTo ERR_HANDLER
    Set shpOpManual = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, 96.6, 171.3, 151.1, 35.3)
    ...
    shpOpManual.TextFrame2.TextRange.Font.Size = 12
    ...
    On Error GoTo ERR_HANDLER
    Set shpToggle = wsMain.Shapes.AddShape(msoShapeRoundedRectangle, 98.1, 218.2, 151.1, 35.4)
    ...
    shpToggle.TextFrame2.TextRange.Font.Size = 12
    ...
    shpToggleLayout.OnAction = "FitHomeToScreen"   ' V3.39: real fit-to-window
    ...
    shpExit.TextFrame2.TextRange.Font.Size = 12
...
3975     wsMain.Range("A19").Value = ChrW(1490) & ChrW(1493) & ChrW(1512) & ChrW(1504) & ChrW(1496) & ChrW(1511) & " v" & APP_VERSION
3980 wsMain.Range("A19").Font.Size = 10
3990 wsMain.Range("A19").Font.Color = RGB(150, 150, 150)
4000 wsMain.Range("A19").Font.Bold = False
4010 wsMain.Range("A19").HorizontalAlignment = -4108
    ...
    Err.Clear
4160     On Error GoTo ERR_HANDLER
    
4170     Set wsTemp = ThisWorkbook.Worksheets(H_SET_MESSAGES())
4180 wsTemp.Cells(2, 1).Value = ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1497) & ChrW(1497) & ChrW(1501) & " " & "-" & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " "
4190 wsTemp.Cells(3, 1).Value = " " & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1493) & ChrW(1514)
4200 wsTemp.Cells(4, 1).Value = ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1497) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492)
4210 wsTemp.Cells(5, 1).Value = ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1492) & " "
4220 wsTemp.Cells(6, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " "
4230 wsTemp.Cells(7, 1).Value = ChrW(1513) & ChrW(1500) & ChrW(1489) & ":" & " "
4240 wsTemp.Cells(8, 1).Value = ChrW(1488) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1512) & " " & ChrW(1500) & _
        ChrW(1508) & ChrW(1504) & ChrW(1497) & " " & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491)
4250 wsTemp.Cells(9, 1).Value = ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1500) & ChrW(1497) & ChrW(1497) & _
        ChrW(1513) & ChrW(1501) & " " & ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & _
        " " & ChrW(1493) & ChrW(1500) & ChrW(1497) & ChrW(1497) & ChrW(1510) & ChrW(1512) & " " & ChrW(1491) & ChrW(1493) & _
        ChrW(1495) & ChrW(1493) & ChrW(1514) & "?"
4260 wsTemp.Cells(10, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & _
        ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & _
        ChrW(1489) & ChrW(1497) & ChrW(1514) & ":" & " "
4270 wsTemp.Cells(11, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & _
        ChrW(1506) & ChrW(1491) & ChrW(1499) & ChrW(1493) & ChrW(1503) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & _
        ChrW(1492) & ":" & " "
4280 wsTemp.Cells(12, 1).Value = ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & _
        ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & ChrW(1489) & ChrW(1497) & ChrW(1514) & " " & ChrW(1492) & ChrW(1493) & _
        ChrW(1511) & ChrW(1501) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & "!"
4290 wsTemp.Cells(13, 1).Value = "1" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1492) & ChrW(1506) & _
        ChrW(1500) & ChrW(1497) & ChrW(1514) & " " & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & ChrW(1513) & _
        ChrW(1500) & ChrW(1497) & ChrW(1508) & ChrW(1492) & " " & ChrW(1506) & ChrW(1491) & ChrW(1499) & ChrW(1504) & ChrW(1497) & _
        "?"
4300 wsTemp.Cells(14, 1).Value = "2" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1513) & ChrW(1502) & _
        ChrW(1512) & ChrW(1514) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & _
        ChrW(1493) & ChrW(1491) & " " & ChrW(1492) & ChrW(1488) & ChrW(1495) & ChrW(1512) & ChrW(1493) & ChrW(1503) & " " & "(" & _
        ChrW(1492) & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & ChrW(1495) & _
        ChrW(1493) & ChrW(1489) & ChrW(1512) & ChrW(1514) & " " & ChrW(1494) & ChrW(1493) & " " & ChrW(1497) & ChrW(1502) & _
        ChrW(1495) & ChrW(1511) & ChrW(1493) & " " & ChrW(1489) & ChrW(1514) & ChrW(1492) & ChrW(1500) & ChrW(1497) & ChrW(1498) & _
        " " & ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491) & ")"
4310 wsTemp.Cells(15, 1).Value = "3" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1489) & ChrW(1495) & _
        ChrW(1512) & ChrW(1514) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1508) & ChrW(1512) & ChrW(1496) & ChrW(1497) & " " & _
        ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491) & " " & ChrW(1492) & ChrW(1504) & ChrW(1491) & _
        ChrW(1512) & ChrW(1513) & " " & ChrW(1489) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & _
        " " & ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & ChrW(1489) & ChrW(1497) & ChrW(1514) & "?"
4320 wsTemp.Cells(16, 1).Value = "4" & "." & " " & ChrW(1500) & ChrW(1495) & ChrW(1509) & " " & ChrW(1506) & ChrW(1500) & _
        " " & "O" & "K" & " " & ChrW(1500) & ChrW(1492) & ChrW(1502) & ChrW(1513) & ChrW(1498)
4330 wsTemp.Cells(17, 1).Value = ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1500) & ChrW(1492) & ChrW(1510) & _
        ChrW(1497) & ChrW(1490) & " " & ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1492) & " " & ChrW(1494) & _
        ChrW(1493) & " " & ChrW(1513) & ChrW(1493) & ChrW(1489) & "?"
4340       wsTemp.Cells(18, 1).Value = EOD_MARKER

Dim wsParams As Worksheet
4350 On Error Resume Next
4360 Set wsParams = ThisWorkbook.Worksheets(H_SET_PARAMS())
4370 On Error GoTo ERR_HANDLER
4380 If Not wsParams Is Nothing Then
    ' ---- Set Error_Email parameter if not exists ----
4390 paramStartRow = 1 + 1
    ' Find last param row (scan until EOD or empty)
4400 paramLastRow = paramStartRow
4410       Do While UCase$(Trim$(CStr(wsParams.Cells(paramLastRow, COL_PARAM_NAME).Value2))) <> EOD_MARKER _
             And Trim$(CStr(wsParams.Cells(paramLastRow, COL_PARAM_NAME).Value2)) <> ""
4420           paramLastRow = paramLastRow + 1
4430       Loop
4440       paramLastRow = paramLastRow - 1  ' last actual data row
4450 foundEmail = False
4460 For pr = paramStartRow To paramLastRow
4470 If UCase$(Trim$(CStr(wsParams.Cells(pr, COL_PARAM_NAME).Value2))) = "ERROR_EMAIL" Then foundEmail = True: Exit For
4480 Next pr
4490 If Not foundEmail Then
4500 wsParams.Cells(paramLastRow + 1, COL_PARAM_NAME).Value = "ERROR_EMAIL"
4510 wsParams.Cells(paramLastRow + 1, COL_PARAM_VALUE).Value = "zvi@gorentech.co.il"
4520 paramLastRow = paramLastRow + 1
4530 End If
      
      Dim foundDemo As Boolean
4540       foundDemo = False
4550       For pr = paramStartRow To paramLastRow
4560           If UCase$(Trim$(CStr(wsParams.Cells(pr, COL_PARAM_NAME).Value2))) = "DEMO_MODE" Then foundDemo = True: Exit For
4570       Next pr
4580       If Not foundDemo Then
4590           wsParams.Cells(paramLastRow + 1, COL_PARAM_NAME).Value = "DEMO_MODE"
4600 wsParams.Cells(paramLastRow + 1, COL_PARAM_VALUE).Value = ChrW(1499) & ChrW(1503) ' YES
4610           paramLastRow = paramLastRow + 1
4620       End If

    ' ---- Set BACKUP_PATH parameter if not exists ----
4630 foundBackup = False
4640 For pr = paramStartRow To paramLastRow
4650 If UCase$(Trim$(CStr(wsParams.Cells(pr, COL_PARAM_NAME).Value2))) = "BACKUP_PATH" Then foundBackup = True: Exit For
4660 Next pr
4670 If Not foundBackup Then
4680 wsParams.Cells(paramLastRow + 1, COL_PARAM_NAME).Value = "BACKUP_PATH"
4690 wsParams.Cells(paramLastRow + 1, COL_PARAM_VALUE).Value = "C:\LEVAV PROJECT\BACKUPS"
4700 End If
4710 End If ' End If Not wsParams Is Nothing
...
4760     On Error Resume Next
4770     wsMain.Range("R1:T21").ClearContents
    ' Removed ClearContents to protect credit text in A23
4780     Err.Clear
4790     On Error GoTo ERR_HANDLER

    
    ' ---- Add WhatsApp Credit as Footer ----
...
4810 Set wsMatachSetup = ThisWorkbook.Worksheets(MATACH_SHEET_NAME())
4820 Err.Clear
4830 On Error GoTo ERR_HANDLER
4840 If Not wsMatachSetup Is Nothing Then
4850 wsMatachSetup.Tab.Color = RGB(139, 90, 43)
4860 wsMatachSetup.Visible = xlSheetVeryHidden
4870 End If
    ' ---- Setup settings navigation menu ----
4880 SetupSettingsMenu
    ' ---- Set tab colors for ALL sheets ----
4890 Call SetAllTabColors
    ' ---- Navigate to A1 & Fit Screen ----
4900 wsMain.Activate
    ' V3.35: keep BOTH the formula bar and the Ribbon visible.
    ' Excel does not draw data-validation in-cell dropdown arrows while the
    ' formula bar is hidden (fixed in V3.30) OR while the Ribbon is minimized
    ' (this fix). Because A00 runs on every return to the home page and used to
    ' minimize the Ribbon, all the G5..G10 pickers lost their arrows the moment
    ' you entered settings and came back - even though the validations, the
    ' unprotected state and every display flag were perfect. Confirmed by hand:
    ' Ctrl+F1 (restore the Ribbon) brought the arrows straight back.
    ' Both are now forced ON, so a session an earlier build left with either
    ...
    ' V3.39: dynamic fit-to-window instead of the fixed A1:L20 range, so the
    ' home page fills the screen on any resolution (laptops were overflowing).
    FitHomeToScreen
4910 wsMain.Range("F10").Select   ' V3.41: rest on F10 (final position set again in CheckUserPermissions)
4920 ResetHomeDefaults
4930 Application.EnableEvents = True
4940 ApplyDemoLockOnOpen

    ' Add Yes/No Dropdown dynamically to DEMO_MODE value cell (Column B)
    On Error Resume Next
    Dim wsParamsSetup As Worksheet
    Set wsParamsSetup = ThisWorkbook.Worksheets(H_SET_PARAMS())
    If Not wsParamsSetup Is Nothing Then
        ' V3.28: this used to unprotect with "1234" while the rest of the module
        ' protects with "Z961814r". When the sheet was already protected the
        ' unprotect failed, and because the whole block runs under On Error
        ' Resume Next the dropdown was never rebuilt - silently. Try the real
        ' password first, then "1234" to recover sheets an older build left
        ' protected with it.
        wsParamsSetup.Unprotect Password:="Z961814r"
        ...
        For rParam = 1 To 100
            If UCase$(Trim$(CStr(wsParamsSetup.Cells(rParam, 1).Value2))) = "DEMO_MODE" Then
                ...
                ' V3.29: this used to Delete then Add unconditionally, every time
                ' A00 ran - which is on every return to the home page. The whole
                ' block sits under On Error Resume Next, so whenever the Add
                ' failed the cell was left with the validation deleted and no
                ' replacement, and the dropdown silently vanished. Reproduced by:
                ' open workbook -> settings -> B7 has an arrow -> back to home ->
                ' settings -> arrow gone.
                ' Now: look first, and only rebuild when it is actually missing.
                dvOK = False
                On Error Resume Next
                dvOK = (wsParamsSetup.Cells(rParam, 2).Validation.Type = xlValidateList)
                On Error Resume Next

                If Not dvOK Then
                    wsParamsSetup.Cells(rParam, 2).Validation.Delete
                    With wsParamsSetup.Cells(rParam, 2).Validation
                        ' Operator is not valid for xlValidateList - passing it
                        ' was another way this Add could fail.
                        .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=ChrW(1499) & ChrW(1503) & "," & ChrW(1500) & ChrW(1488)
                        .IgnoreBlank = True
                        .InCellDropdown = True
                        .ShowInput = True
                        .ShowError = True
            ...
            End If
        Next rParam
        ' V3.34: leave the params sheet UNPROTECTED, same reasoning as the home
        ' sheet - re-protecting from code hid the DEMO_MODE (B7) dropdown arrow.
        ' Unprotected always shows it. The sheet was Unprotected just above.
        ' wsParamsSetup.Protect Password:="Z961814r", DrawingObjects:=False, UserInterfaceOnly:=True
    End If
    On Error GoTo ERR_HANDLER

4950 CheckUserPermissions
    ' V3.36: A00's UI churn (Activate/Select/Goto/Protect/Unprotect/ribbon)
    ' leaves this machine's Excel in a state where in-cell dropdown arrows are
    ' not repainted, despite every property being correct. FixArrows forces a
    ' repaint as the very last step, so the "back to home" button lands with the
    ' G5..G10 arrows drawn.
    FixArrows
4960 Exit Sub
ERR_HANDLER:
...
' ============================================================================
' V3.36: Force Excel to repaint in-cell data-validation dropdown arrows.
' On some machines Excel stops drawing the arrows after heavy programmatic UI
' work (as A00 does), even though the validation, the unprotected state and
' every display flag are correct - proven by hand, where any strong window
' refresh (Ctrl+F1, a zoom change) brought them straight back. This runs the
' strongest harmless refreshes together, and is called at the end of A00 so the
' "back to home" button lands with the arrows visible. It can also be run on
' its own (Alt+F8 -> FixArrows) any time arrows go missing.
' ============================================================================
Public Sub FixArrows()
    ...
    Dim st As Long
    Set ws = ActiveSheet
    savedAddr = ActiveWindow.RangeSelection.Address
    ...
    ' Minimizing and restoring the window forces Excel to redraw it completely -
    ' the strongest repaint available - which is what finally paints the arrows.
    st = ActiveWindow.WindowState
    ActiveWindow.WindowState = xlMinimized
    DoEvents
    If st = xlMinimized Then st = xlMaximized
    ActiveWindow.WindowState = st
    DoEvents

    ' Restore the user's selection so nothing visibly moves.
    If Len(savedAddr) > 0 Then ws.Range(savedAddr).Select
    DoEvents
End Sub
...
70      wsMain.Range("rngPeriodValue").MergeArea.Validation.Delete
...
100         listName = "=lst_half_year"
        ' "riv'oni" = quarterly
110     ElseIf InStr(1, periodType, ChrW(1512) & ChrW(1489) & ChrW(1506) & ChrW(1493) & ChrW(1503), vbTextCompare) > 0 Then
...
190     End If
        ...
        If listName = "" Then GoTo CLEAN_EXIT
        
200     wsMain.Range("rngPeriodValue").MergeArea.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:=listName
...
CLEAN_EXIT:
        On Error Resume Next
        If Not wsMain Is Nothing Then wsMain.Protect Password:="Z961814r", DrawingObjects:=False, UserInterfaceOnly:=True
210     Exit Sub
...
100     If filterType = "" Or filterType = selectText Then GoTo CLEAN_EXIT
    ...
    If Not FORCE_DEMO_MODE Then
        If demoParamUI = ChrW(1499) & ChrW(1503) Or demoParamUI = "YES" Or demoParamUI = "" Then isDemoModeUI = True
...
260         MsgBoxU ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & listsName & " " & _
        ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ". " & ChrW(1492) & ChrW(1512) & _
        ChrW(1509) & " " & ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 1 " & ChrW(1514) & ChrW(1495) & _
        ChrW(1497) & ChrW(1500) & ChrW(1492) & ".", vbExclamation
270         GoTo CLEAN_EXIT
280     End If
...
ERR_HANDLER:
        Application.EnableEvents = True
        Application.DisplayAlerts = True
420     MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1506) & _
        ChrW(1491) & ChrW(1499) & ChrW(1493) & ChrW(1503) & " " & ChrW(1505) & ChrW(1497) & ChrW(1504) & ChrW(1493) & ChrW(1503) & _
...
224     shp.TextFrame.TextRange.Text = GetActiveAgencyName()
...
110     shp.ZOrder 0
    ...
    shpAgency.TextFrame.TextRange.ParagraphFormat.Alignment = 1
...
30      Set wsRev = ActiveSheet
        ...
        ' V3.27: ActiveSheet resolves against the ACTIVE WINDOW, not the sheet
        ' whose button was clicked. With a second window open on the same
        ' workbook (View > New Window) it can point at the home sheet, and the
        ' button then refused to run even though the review sheet was on screen.
        ' Fall back to locating the review sheet ourselves.
        If InStr(1, wsRev.Name, REVIEW_SHEET_NAME(), vbTextCompare) = 0 Then
            Dim wsScan As Worksheet
            Dim wsExact As Worksheet
            Dim wsAnyRev As Worksheet
            Dim revHits As Long
            Dim curYearSFR As String
            Dim wantRevName As String

            curYearSFR = ""
            On Error Resume Next
            curYearSFR = Trim$(CStr(ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Range("rngCurrentYear").Value2))
            On Error GoTo ERR_HANDLER
            wantRevName = REVIEW_SHEET_NAME() & "_" & curYearSFR

            revHits = 0
            For Each wsScan In ThisWorkbook.Worksheets
                If InStr(1, wsScan.Name, REVIEW_SHEET_NAME(), vbTextCompare) > 0 Then
                    revHits = revHits + 1
                    Set wsAnyRev = wsScan
                    If StrComp(wsScan.Name, wantRevName, vbTextCompare) = 0 Then Set wsExact = wsScan
                End If
            Next wsScan

            ' Prefer the current year's sheet; otherwise accept the only one.
            ' If several exist and none matches the current year, stay silent
            ' and let the message below fire - guessing could mail the wrong year.
            If Not wsExact Is Nothing Then
                Set wsRev = wsExact
            ElseIf revHits = 1 Then
                Set wsRev = wsAnyRev
            End If
        End If
...
50          MsgBoxU ChrW(1497) & ChrW(1513) & " " & ChrW(1500) & ChrW(1492) & ChrW(1512) & ChrW(1497) & ChrW(1509) & " " & _
        ChrW(1502) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & ChrW(1500) & ChrW(1496) & _
        ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1500), vbExclamation
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
60          Exit Sub
...
200         MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1492) & " " & _
        ChrW(1506) & ChrW(1502) & ChrW(1493) & ChrW(1491) & ChrW(1514) & " " & ChrW(1508) & ChrW(1506) & ChrW(1493) & ChrW(1500) & _
        ChrW(1492), vbExclamation
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
210         Exit Sub
...
350         MsgBoxU ChrW(1488) & ChrW(1497) & ChrW(1503) & " " & ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1493) & _
        ChrW(1514) & " " & ChrW(1500) & ChrW(1492) & ChrW(1506) & ChrW(1489) & ChrW(1512) & ChrW(1492) & "." & vbCrLf & _
        handledCount & " " & ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1496) & ChrW(1493) & _
        ChrW(1508) & ChrW(1500) & ChrW(1493), vbInformation
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
360         Exit Sub
...
410     emailAddr = GetStringParameter(ThisWorkbook.Worksheets(H_SET_PARAMS()), PARAM_ERROR_EMAIL)
420     If emailAddr = "" Then
            ' "lo hugdra ktovet email" = email address not defined
430         MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1492) & ChrW(1493) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1492) & " " & ChrW(1499) & ChrW(1514) & ChrW(1493) & ChrW(1489) & ChrW(1514) & " " & ChrW(1488) & ChrW(1497) & _
        ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & " " & ChrW(1489) & ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & _
        ChrW(1493) & ChrW(1514), vbExclamation
Application.EnableEvents = True
Application.ScreenUpdating = True
Application.DisplayAlerts = True
440         Exit Sub
...
670     tempPath = Environ$("TEMP") & "\Exceptions_" & Format$(Now, "yyyy-mm-dd_hhmmss") & ".xlsx"
...
750     bodyLine2 = ChrW(1500) & ChrW(1511) & ChrW(1512) & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1499) & _
        ChrW(1504) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & " " & ChrW(1506) & ChrW(1489) & ChrW(1493) & _
        ChrW(1512) & ChrW(1499) & ChrW(1501) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & _
        ChrW(1492) & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1497) & ChrW(1501) & " " & ChrW(1492) & ChrW(1512) & _
        ChrW(34) & ChrW(1502)
        ' "al mnat lehafik et hadoch ani mevakeshet tguvatcha al mnat she'etaken beheta'am"
760     Dim bodyLine3 As String
770     bodyLine3 = ChrW(1506) & ChrW(1500) & " " & ChrW(1502) & ChrW(1504) & ChrW(1514) & " " & ChrW(1500) & ChrW(1492) & _
        ChrW(1508) & ChrW(1497) & ChrW(1511) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1491) & ChrW(1493) & _
        ChrW(1495) & " " & ChrW(1488) & ChrW(1504) & ChrW(1497) & " " & ChrW(1502) & ChrW(1489) & ChrW(1511) & ChrW(1513) & _
        ChrW(1514) & " " & ChrW(1514) & ChrW(1490) & ChrW(1493) & ChrW(1489) & ChrW(1514) & ChrW(1498) & " " & ChrW(1506) & _
        ChrW(1500) & " " & ChrW(1502) & ChrW(1504) & ChrW(1514) & " " & ChrW(1513) & ChrW(1488) & H_FIX() & " " & ChrW(1489) & _
        ChrW(1492) & ChrW(1514) & ChrW(1488) & ChrW(1501)
        ' toda = ????
...
850     emailSubject = ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1497) & ChrW(1501) & " " & ChrW(1513) & _
        ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & ChrW(1500) & ChrW(1496) & ChrW(1497) & ChrW(1508) & _
        ChrW(1493) & ChrW(1500) & ChrW(1498)
...
950     Dim askSent As Long
        askSent = MsgBoxU( _
            ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & " " & ChrW(1492) & ChrW(1493) & ChrW(1499) & ChrW(1503) & _
        " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & " " & ChrW(1506) & ChrW(1501) & " " & _
        sendCount & " " & ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1493) & ChrW(1514) & "." & vbCrLf & vbCrLf & _
            ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1513) & ChrW(1500) & ChrW(1495) & ChrW(1514) & " " & _
        ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & "?" & vbCrLf & _
            ChrW(1499) & ChrW(1503) & " = " & ChrW(1492) & ChrW(1502) & ChrW(1513) & ChrW(1498) & " | " & ChrW(1500) & _
        ChrW(1488) & " = " & ChrW(1488) & ChrW(1504) & ChrW(1488) & " " & ChrW(1513) & ChrW(1500) & ChrW(1495) & " " & _
        ChrW(1493) & ChrW(1488) & ChrW(1513) & ChrW(1512), _
            vbYesNo + vbQuestion)
        If askSent = vbYes Then
            ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Activate
        Else
            ' "ana shlach ve'asher" = ??? ??? ????
            MsgBoxU ChrW(1488) & ChrW(1504) & ChrW(1488) & " " & ChrW(1513) & ChrW(1500) & ChrW(1495) & " " & ChrW(1488) & _
        ChrW(1514) & " " & ChrW(1492) & ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & " " & ChrW(1493) & ChrW(1488) & _
        ChrW(1513) & ChrW(1512), vbExclamation
        End If
...
ERR_HANDLER:
        Application.EnableEvents = True
        Application.DisplayAlerts = True
970     MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1513) & _
        ChrW(1500) & ChrW(1497) & ChrW(1495) & ChrW(1514) & " " & ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & ":" & _
...
Public Sub ToggleHiddenSheets()
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        If ws.Name = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1514) & ChrW(1511) & ChrW(1504) & ChrW(1492) Then ignoreSheet = True ' הוראות_התקנה
...
Public Sub SetupSettingsSheet()
    ...
    shp.TextFrame2.TextRange.Text = ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & GetActiveAgencyName()
    ...
    MsgBoxU ChrW(1491) & ChrW(1507) & " " & ChrW(1513) & ChrW(1506) & ChrW(1512) & " " & ChrW(1492) & ChrW(1493) & _
        ChrW(1490) & ChrW(1491) & ChrW(1512) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & _
        ChrW(1492), vbInformation
    Exit Sub
ERR_HANDLER:
        Application.EnableEvents = True
        Application.DisplayAlerts = True
    MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1492) & ChrW(1490) & _
        ChrW(1491) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1507) & " " & ChrW(1513) & ChrW(1506) & ChrW(1512) & ":" & _
        vbCrLf & Err.Description, vbCritical
End Sub
...
Public Sub ResetEvents()
    ...
    ' V3.43: was a raw Hebrew string literal, which gets mangled when the .bas
    ' is re-saved as ASCII; rebuilt from ChrW like every other message.
    MsgBoxU ChrW(1492) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & ChrW(1514) & " " & ChrW(1513) & ChrW(1493) & ChrW(1495) & ChrW(1512) & ChrW(1512) & ChrW(1492) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & " (Fixela)", vbInformation
End Sub
...
Public Sub SearchClientName()
    ...
    ' Row 1: Instructions
    wsSearch.Cells(1, 1).Value = ChrW(1492) & ChrW(1511) & ChrW(1500) & ChrW(1491) & " " & ChrW(1513) & ChrW(1501) & " " & _
        ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & " " & ChrW(1489) & ChrW(1514) & ChrW(1488) & " " & ChrW(1492) & _
        ChrW(1510) & ChrW(1492) & ChrW(1493) & ChrW(1489) & " " & ChrW(1493) & ChrW(1500) & ChrW(1495) & ChrW(1509) & " " & _
        ChrW(1495) & ChrW(1508) & ChrW(1513)  ' "???? ?? ???? ??? ????? ???? ???"
    wsSearch.Cells(1, 1).Font.Bold = True
    wsSearch.Cells(1, 1).Font.Size = 12
    wsSearch.Cells(1, 1).Font.Color = RGB(255, 0, 0) ' Make it red again!
    
    ' Row 2: Yellow search cell with Dropdown (Data Validation) + Wildcard support
...
wsSearch.Cells(5, 5).Value = ChrW(1492) & ChrW(1505) & ChrW(1489) & ChrW(1512) & " " & ChrW(1500) & ChrW(1495) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1513) & ":"
...
wsSearch.Cells(7, 5).Value = "- " & ChrW(1492) & ChrW(1511) & ChrW(1500) & ChrW(1491) & " *77*: " & ChrW(1497) & _
        ChrW(1502) & ChrW(1510) & ChrW(1488) & " " & ChrW(1499) & ChrW(1500) & " " & ChrW(1502) & ChrW(1497) & " " & ChrW(1513) & _
        ChrW(1502) & ChrW(1499) & ChrW(1497) & ChrW(1500) & " 77"
wsSearch.Cells(8, 5).Value = "- " & ChrW(1492) & ChrW(1511) & ChrW(1500) & ChrW(1491) & " 77*: " & ChrW(1497) & _
        ChrW(1502) & ChrW(1510) & ChrW(1488) & " " & ChrW(1499) & ChrW(1500) & " " & ChrW(1502) & ChrW(1497) & " " & ChrW(1513) & _
        ChrW(1502) & ChrW(1514) & ChrW(1495) & ChrW(1497) & ChrW(1500) & " " & ChrW(1489) & "-77"
...
Public Sub DoClientSearch()
    ...
    If searchText = "" Then
        MsgBoxU ChrW(1492) & ChrW(1511) & ChrW(1500) & ChrW(1491) & " " & ChrW(1496) & ChrW(1511) & ChrW(1505) & _
        ChrW(1496) & " " & ChrW(1500) & ChrW(1495) & ChrW(1497) & ChrW(1508) & ChrW(1493) & _
        ChrW(1513), vbExclamation  ' "???? ???? ??????"
        Application.GoTo wsSearch.Cells(2, 1)
        Exit Sub
    End If
...
If InStr(1, paddedName, paddedSearch, vbTextCompare) > 0 Then bMatch = True
    ...
    If Not foundBase Then
        MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & " " & ChrW(1490) & _
        ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & H_BASE(), vbExclamation
        Exit Sub
    End If
    
    If dict.Count = 0 Then
        MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & _
        ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1502) & ChrW(1514) & ChrW(1488) & _
        ChrW(1497) & ChrW(1502) & ChrW(1497) & ChrW(1501), vbInformation  ' "?? ????? ?????? ???????"
        Application.GoTo wsSearch.Cells(2, 1)
        Exit Sub
    End If
    
    ' Previously auto-selected if dict.Count = 1, but removed per user request
    ...
    wsSearch.Cells(3, 1).Value = ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & dict.Count & " " & _
        ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " - " & ChrW(1500) & ChrW(1495) & _
        ChrW(1509) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1513) & ChrW(1501) & " " & ChrW(1493) & ChrW(1500) & ChrW(1495) & _
        ChrW(1509) & " " & ChrW(1489) & ChrW(1495) & ChrW(1512)  ' "????? X ?????? - ??? ?? ?? ???? ???"
...
Public Sub ConfirmClientSelection()
    ...
    If selectedName = "" Then
        MsgBoxU ChrW(1489) & ChrW(1495) & ChrW(1512) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & " " & _
        ChrW(1502) & ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & _
        ChrW(1492), vbExclamation  ' "??? ???? ???????"
        Exit Sub
    End If
...
Public Sub ClearClientFilter()
    ...
    Dim wsMain As Worksheet
    Set wsMain = ThisWorkbook.Worksheets(CONTROL_SHEET_NAME())
    wsMain.Range("rngClientName").Value = ""
On Error Resume Next
...
On Error GoTo 0
    ...
    On Error Resume Next
    wsMain.Protect Password:="Z961814r", DrawingObjects:=False, UserInterfaceOnly:=True
    On Error GoTo 0
...
Private Sub ApplyZebraStriping(ByVal ws As Worksheet)
    ...
    If lastRow >= dataStart Then
        If ws.Cells(lastRow, 1).Font.Bold Then
            ...
            ws.Range(ws.Cells(lastRow, 1), ws.Cells(lastRow, 16)).Font.Size = 12
...
10      On Error GoTo ERR_HANDLER
        ...
        If sheetCount = 0 Then
            ...
            MsgBoxU ChrW(1488) & ChrW(1497) & ChrW(1503) & " " & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & _
        ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & _
        " " & ChrW(1500) & ChrW(1513) & ChrW(1502) & ChrW(1497) & ChrW(1512) & ChrW(1492) & " - " & ChrW(1492) & ChrW(1512) & _
        ChrW(1509) & " " & ChrW(1511) & ChrW(1493) & ChrW(1491) & ChrW(1501) & " " & ChrW(1499) & ChrW(1508) & ChrW(1514) & _
        ChrW(1493) & ChrW(1512) & " 2", vbExclamation
            Application.EnableEvents = True
            Application.ScreenUpdating = True
            Application.DisplayAlerts = True
            Exit Sub
...
' NOTE: a cleanup loop here used to delete every visible sheet except the home
' sheet - i.e. it destroyed the very report sheets that were just exported.
' Removed: saving reports must not delete them.

        ' Save as XLSX (no macros)
        Dim xlsxPath As String
        ' "dochot hashvaa" = comparison reports
        xlsxPath = reportsFolder & "\" & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1513) & ChrW(1493) & ChrW(1493) & ChrW(1488) & ChrW(1492) & " " & yearVal & ".xlsx"
        ...
        MsgBoxU ChrW(1492) & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1504) & _
        ChrW(1513) & ChrW(1502) & ChrW(1512) & ChrW(1493) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & _
        ChrW(1492) & "!", vbInformation
...
ERR_HANDLER:
        ...
        Next hiErr
        MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1513) & _
        ChrW(1502) & ChrW(1497) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & _
        ": " & Err.Description, vbCritical
End Sub
...
If CheckDemoLock() Then Exit Sub
    ...
    If Dir(reportsPath, vbDirectory) = "" Then
        MsgBoxU ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1497) & ChrW(1497) & ChrW(1514) & " " & ChrW(1492) & _
        ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & _
        ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1492) & ".", vbExclamation
        Exit Sub
    End If
...
Public Sub NewClients()
    ...
    If yearVal = "" Or refYear = "" Then
        MsgBoxU ChrW(1497) & ChrW(1513) & " " & ChrW(1500) & ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1497) & _
        ChrW(1512) & " " & ChrW(1513) & ChrW(1504) & ChrW(1514) & " " & H_BASE() & " " & ChrW(1493) & ChrW(1513) & ChrW(1504) & _
        ChrW(1492) & " " & ChrW(1504) & ChrW(1493) & ChrW(1499) & ChrW(1495) & ChrW(1497) & ChrW(1514) & ".", vbExclamation
        Exit Sub
    End If
    
    ' --- Check if master sheet already exists (already processed) ---
    ' V3.25: was a dead end - if the sheet existed the sub always exited, so the
    ' append-history logic below could never run and the list froze on first use.
    ' Now: Yes = view existing, No = refresh (keeps rows, appends only new), Cancel/X = quit.
    If SheetExists(NC_MASTER_SHEET_NAME()) Then
        Dim alreadyMsg6 As String
        Dim ncAns As VbMsgBoxResult
        ' "lekochot chadashim kvar nivdeku letkufa zo."
        alreadyMsg6 = ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & _
        ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501) & " " & ChrW(1499) & ChrW(1489) & ChrW(1512) & " " & ChrW(1504) & _
        ChrW(1489) & ChrW(1491) & ChrW(1511) & ChrW(1493) & " " & ChrW(1500) & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & _
        ChrW(1492) & " " & ChrW(1494) & ChrW(1493) & "." & vbCrLf & vbCrLf & _
            ChrW(1499) & ChrW(1503) & " = " & ChrW(1510) & ChrW(1508) & ChrW(1492) & " " & ChrW(1489) & ChrW(1512) & _
        ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1492) & " " & ChrW(1492) & ChrW(1511) & ChrW(1497) & ChrW(1497) & _
        ChrW(1502) & ChrW(1514) & vbCrLf & _
            ChrW(1500) & ChrW(1488) & " = " & ChrW(1506) & ChrW(1491) & ChrW(1499) & ChrW(1503) & " " & ChrW(1493) & _
        ChrW(1492) & ChrW(1493) & ChrW(1505) & ChrW(1507) & " " & ChrW(1512) & ChrW(1511) & " " & ChrW(1500) & ChrW(1511) & _
        ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & _
        ChrW(1501) & vbCrLf & _
            ChrW(1489) & ChrW(1497) & ChrW(1496) & ChrW(1493) & ChrW(1500) & " = " & ChrW(1497) & ChrW(1510) & _
        ChrW(1497) & ChrW(1488) & ChrW(1492)
        ncAns = MsgBoxU(alreadyMsg6, vbYesNoCancel + vbQuestion)
        If ncAns = vbYes Then
            ' Show the master sheet as-is
            ThisWorkbook.Worksheets(NC_MASTER_SHEET_NAME()).Visible = xlSheetVisible
            ThisWorkbook.Worksheets(NC_MASTER_SHEET_NAME()).Activate
            Exit Sub
        ElseIf ncAns <> vbNo Then
            ' Cancel, Esc or the X button
            Exit Sub
        End If
        ' vbNo: fall through and rebuild. Existing rows are preserved -
        ' dictExisting (below) skips customers already listed in column B.
    End If
    ...
    If Not SheetExists(curSheetName) Then
        ...
        ChrW(1511) & ChrW(1493) & ChrW(1491) & ChrW(1501) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1499) & ChrW(1508) & _
        ChrW(1514) & ChrW(1493) & ChrW(1512) & " 2 (" & ChrW(1497) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1501) & " " & _
...
NextNCRow:
    ...
    statusFormula = ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1495) & "," & ChrW(1500) & ChrW(1495) & ChrW(1494) & _
        ChrW(1493) & ChrW(1512) & "," & ChrW(1489) & ChrW(1493) & ChrW(1510) & ChrW(1506) & ChrW(1492) & " " & ChrW(1502) & _
        ChrW(1499) & ChrW(1497) & ChrW(1512) & ChrW(1492) & "," & ChrW(1500) & ChrW(1488) & " " & ChrW(1502) & ChrW(1506) & _
        ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1497) & ChrW(1503)
    ...
    shpHome.TextFrame.Characters.Text = ChrW(1495) & ChrW(1494) & ChrW(1512) & ChrW(1492) & " " & ChrW(1500) & _
        ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & ChrW(1489) & ChrW(1497) & ChrW(1514)  ' ???? ??? ????
    ...
    shpHome.OnAction = "NC_BackToHome"
    
    ' V3.25: drop the temp sheet. It duplicated the master's columns exactly but
    ' rewrote every status back to "open" on each run, so anyone who happened to
    ' edit it lost their work silently. The master alone carries the workflow:
    ' status is preserved per customer, and the "open" filter button shows the
    ' rows still to handle. Nothing else in the module ever referenced this sheet.
    On Error Resume Next
    Application.DisplayAlerts = False
    wsOut.Delete
    Application.DisplayAlerts = True
    ...
    ' V3.25: on the refresh path the master sheet already exists and is usually
    ' xlSheetVeryHidden (HideWorkSheets). Activate on a hidden sheet raises 1004,
    ' so make it visible first.
    If wsMaster.Visible <> xlSheetVisible Then wsMaster.Visible = xlSheetVisible
    wsMaster.Activate
    ...
    If newCount = 0 Then
        ' ?? ????? ?????? ????? ?????. ??"? ???????: X
        MsgBoxU ChrW(1500) & ChrW(1488) & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & _
        ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & _
        ChrW(1497) & ChrW(1501) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501) & "." & vbCrLf & _
        ChrW(1505) & ChrW(1492) & Chr(34) & ChrW(1499) & " " & ChrW(1489) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & _
        ChrW(1493) & ChrW(1503) & ": " & totalInMaster, vbInformation
    Else
        ' ????? X ?????? ????? ?????. ??"? ???????: Y
        MsgBoxU ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " " & newCount & " " & ChrW(1500) & _
        ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & _
        ChrW(1501) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501) & "." & vbCrLf & ChrW(1505) & _
        ChrW(1492) & Chr(34) & ChrW(1499) & " " & ChrW(1489) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & _
        ChrW(1503) & ": " & totalInMaster, vbInformation
    End If
...
NC_ERR:
    Application.ScreenUpdating = True
    MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1495) & ChrW(1497) & _
        ChrW(1508) & ChrW(1493) & ChrW(1513) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & _
        " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501) & ": " & Err.Description, vbCritical
End Sub
...
' ============================================================================
' BACK TO HOME from the New-Clients master sheet.
' V3.26: the button used to call A00_SetupMainSheet directly, which rebuilt the
' home page but left the master sheet visible behind it.
' Order matters: A00 ends by calling CheckUserPermissions, and for Limited
' Access users that explicitly re-shows the master sheet - so the hide has to
' happen after A00, not before.
' ============================================================================
Public Sub NC_BackToHome()
    A00_SetupMainSheet

    On Error Resume Next
    ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Visible = xlSheetVisible
    ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Activate
    ThisWorkbook.Worksheets(NC_MASTER_SHEET_NAME()).Visible = xlSheetVeryHidden
    On Error GoTo 0
End Sub
...
Public Sub ApplyDemoLockOnOpen()
    ...
    Else
        ...
        With wsMain.Range("G3").MergeArea.Validation
            .Delete
            .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:=xlBetween, Formula1:="2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030"
            .InCellDropdown = True
        End With
        With wsMain.Range("G4").MergeArea.Validation
            .Delete
...
Public Sub AssignButtonMacros()
    ...
    ' V3.26: the New-Clients master sheet carries its own buttons, and they are
    ' only rewired when NewClients does a full rebuild. Re-point the "back home"
    ' one here too, so a workbook created by an older version picks up
    ' NC_BackToHome on the next open instead of the old A00 call.
    Dim wsNC As Worksheet
    Set wsNC = ThisWorkbook.Worksheets(NC_MASTER_SHEET_NAME())
    If Not wsNC Is Nothing Then
        For Each shp In wsNC.Shapes
            If shp.Name = "btnBackHome" Then shp.OnAction = "NC_BackToHome"
        Next shp
    End If

    ' V3.39: repoint the "fit screen" button on already-built workbooks, so a
    ' copy saved by an older build picks up FitHomeToScreen on the next open
    ' instead of the old zoom-only ToggleDisplayMode.
    For Each shp In wsMain.Shapes
        If shp.Name = "shpToggleLayout" Then shp.OnAction = "FitHomeToScreen"
    Next shp
...
' ============================================================================
' V3.38: Restore a home-page button to its designed colour. Called for every
' shape in the Demo and Full-Access branches so that a workbook which was ever
' saved in Limited mode (which paints buttons 1-6 grey, RGB 180,180,180) does
' not keep showing grey buttons. Colours match those set when the buttons are
' first built. Non-button shapes are ignored.
' ============================================================================
Private Sub RestoreButtonColor(ByVal shp As Shape)
    On Error Resume Next
    Select Case shp.Name
        Case "btnBuildReview", "btnSaveReports"
            shp.Fill.ForeColor.RGB = RGB(0, 70, 140)        ' blue   #00468C
        Case "btnApplyCorrections", "btnShowResults", "btnViewReports"
            shp.Fill.ForeColor.RGB = RGB(0, 150, 80)        ' green  #009650
        Case "btnBuildPresentation", "btnNewClients"
            shp.Fill.ForeColor.RGB = RGB(200, 100, 0)       ' orange #C86400
    End Select
End Sub
...
Public Sub CheckUserPermissions()
    ...
    If isDemo Then
        ...
        On Error Resume Next
        For Each shp In wsMain.Shapes
            ' V3.38: restore the real button colour first, so a workbook that was
            ' ever saved in Limited mode (which greys buttons 1-6) does not stay
            ' grey forever. A demo customer must see all 7 buttons coloured.
            RestoreButtonColor shp
            Select Case shp.Name
                ...
                Case "btnSaveReports", "btnViewReports", "btnNewClients"
                    ' Buttons 5, 6, 7 - keep colour, restrict action to the demo message
    ...
    ElseIf accessLevelNum = 3 Then
        ...
        On Error Resume Next
        For Each shp In wsMain.Shapes
            RestoreButtonColor shp   ' V3.38: undo any Limited-mode grey
            Select Case shp.Name
                Case "btnBuildReview"
    ...
    Else
        ...
        For Each ws In ThisWorkbook.Worksheets
            If ws.Name <> homeSheetName And ws.Name <> ncSheetName Then
                ...
                If ws.Name = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1514) & ChrW(1511) & ChrW(1504) & ChrW(1492) Then
    ...
    ApplyDemoLockOnOpen
    ' V3.41: rest the cursor on F10 (the "שם לקוח" label, no dropdown), not G10.
    wsMain.Range("F10").Select
    ' V3.34: leave the home sheet UNPROTECTED. This is the last thing that runs
    ' when landing on the home page (end of A00 and of NavSettings_Home), so its
    ' final state is what the user sees. Re-protecting here - even with
    ' DrawingObjects:=False - hid the G5..G10 dropdown arrows: the identical
    ' Protect restores them when run by hand but not from code, the same
    ' behaviour seen on the params sheet. Unprotecting always shows the arrows
    ' (confirmed by hand), and UserInterfaceOnly protection was never real
    ' security anyway. The sheet was already Unprotected a few lines above.
    ' wsMain.Protect Password:="Z961814r", DrawingObjects:=False, UserInterfaceOnly:=True
    On Error GoTo 0
    ' -----------------------------------
    ...
    ' V3.42: fit the whole home page to the window on open (and on every return
    ' here), so the "התאמת מסך" button - which sits low on the page - is always
    ' visible without scrolling. A large saved zoom used to push it below the
    ' fold, which looked broken. FitHomeToScreen also rests the cursor on F10 and
    ' scrolls to the top-right. Runs for both Demo and Full modes.
    FitHomeToScreen
...
Public Sub EmailNewClients()
    ...
    If lastRow < 3 Then
        MsgBoxU ChrW(1488) & ChrW(1497) & ChrW(1503) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & _
        ChrW(1497) & ChrW(1501) & " " & ChrW(1500) & ChrW(1513) & ChrW(1500) & ChrW(1497) & ChrW(1495) & ChrW(1492) & _
        ".", vbExclamation
        Exit Sub
    End If
    ...
    If outR > 2 Then
        ...
        statusList = ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1495) & "," & ChrW(1500) & ChrW(1495) & ChrW(1494) & _
        ChrW(1493) & ChrW(1512) & "," & ChrW(1489) & ChrW(1493) & ChrW(1510) & ChrW(1506) & ChrW(1492) & " " & ChrW(1502) & _
        ChrW(1499) & ChrW(1497) & ChrW(1512) & ChrW(1492) & "," & ChrW(1500) & ChrW(1488) & " " & ChrW(1502) & ChrW(1506) & _
        ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1497) & ChrW(1503)
    ...
    ' Save temp file
    tempPath = Environ$("TEMP") & "\" & outSheetName & "_" & Format$(Now, "yyyy-mm-dd_hhmmss") & ".xlsx"
    Application.DisplayAlerts = False
    ...
    With olMail
        ...
        .Body = ChrW(1502) & ChrW(1510) & ChrW(1493) & ChrW(1512) & ChrW(1507) & " " & ChrW(1511) & ChrW(1493) & _
        ChrW(1489) & ChrW(1509) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & _
...
EMAIL_ERR:
    Application.DisplayAlerts = True
    MsgBoxU ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & ChrW(1513) & ChrW(1500) & _
        ChrW(1497) & ChrW(1495) & ChrW(1514) & " " & ChrW(1502) & ChrW(1497) & ChrW(1497) & ChrW(1500) & ": " & _
        Err.Description, vbCritical
End Sub
...
Public Sub PrintNewClients()
    ...
    If lastRow < 3 Then
        MsgBoxU ChrW(1488) & ChrW(1497) & ChrW(1503) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & _
        ChrW(1497) & ChrW(1501) & " " & ChrW(1500) & ChrW(1492) & ChrW(1491) & ChrW(1508) & ChrW(1505) & ChrW(1492) & _
        ".", vbExclamation
        Exit Sub
    End If
...
Public Sub ExitSystem()
    If Application.Workbooks.Count > 1 Then
        ...
        closeAll = MsgBoxU(ChrW(1497) & ChrW(1513) & " " & ChrW(1511) & ChrW(1489) & ChrW(1510) & ChrW(1497) & _
        ChrW(1501) & " " & ChrW(1504) & ChrW(1493) & ChrW(1505) & ChrW(1508) & ChrW(1497) & ChrW(1501) & " " & ChrW(1508) & _
        ChrW(1514) & ChrW(1493) & ChrW(1495) & ChrW(1497) & ChrW(1501) & ". " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & _
        ChrW(1500) & ChrW(1505) & ChrW(1490) & ChrW(1493) & ChrW(1512) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1499) & _
        ChrW(1500) & " Excel?", vbYesNo + vbQuestion)
...
10  SettingsGoToSheet H_SET_PERIODS()
End Sub
...
Public Sub SetupSettingsMenu()
    ...
    wsMgmt.Cells.Clear
      wsMgmt.Rows.Hidden = False
      wsMgmt.Columns.Hidden = False
    wsMgmt.Cells.Interior.ColorIndex = xlNone
    wsMgmt.Columns("A:Z").ColumnWidth = 10
    wsMgmt.Columns("B:E").ColumnWidth = 20
    
    wsMgmt.Range("B2:E2").Merge
    wsMgmt.Range("B2").Value = ChrW(1500) & ChrW(1493) & ChrW(1495) & " " & ChrW(1489) & ChrW(1511) & ChrW(1512) & _
        ChrW(1492) & " " & ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1493) & ChrW(1514)
    wsMgmt.Range("B2").Font.Size = 20
    wsMgmt.Range("B2").Font.Bold = True
    wsMgmt.Range("B2").HorizontalAlignment = xlCenter
    wsMgmt.Range("B2").Font.Color = RGB(0, 70, 140)
    
    ' Remove old nav buttons
    On Error Resume Next
    For Each shp In wsMgmt.Shapes
        shp.Delete
    Next shp
    Err.Clear
    On Error GoTo 0
    ...
    ' 1. Field Map
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C4").Left, wsMgmt.Range("C4").Top, 180, 40)
    shp.Name = "btnNav_FieldMap"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "1 - " & ChrW(1502) & ChrW(1497) & ChrW(1508) & ChrW(1493) & ChrW(1497) & " " & ChrW(1513) & ChrW(1491) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 2. Branch Names
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C6").Left, wsMgmt.Range("C6").Top, 180, 40)
    shp.Name = "btnNav_BranchName"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "2 - " & ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1506) & ChrW(1504) & ChrW(1508) & ChrW(1497) & ChrW(1501)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 3. Params
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C8").Left, wsMgmt.Range("C8").Top, 180, 40)
    shp.Name = "btnNav_Params"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "3 - " & ChrW(1508) & ChrW(1512) & ChrW(1502) & ChrW(1496) & ChrW(1512) & ChrW(1497) & ChrW(1501)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 4. Period Lists
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C10").Left, wsMgmt.Range("C10").Top, 180, 40)
    shp.Name = "btnNav_PeriodLists"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "4 - " & ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    shp.OnAction = "NavSettings_PeriodLists"
    
    ' 5. Messages
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C12").Left, wsMgmt.Range("C12").Top, 180, 40)
    shp.Name = "btnNav_Messages"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "5 - " & ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 6. Permissions
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C14").Left, wsMgmt.Range("C14").Top, 180, 40)
    shp.Name = "btnNav_Permissions"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "6 - " & ChrW(1492) & ChrW(1512) & ChrW(1513) & ChrW(1488) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 7. Reason Codes
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C16").Left, wsMgmt.Range("C16").Top, 180, 40)
    shp.Name = "btnNav_ReasonCode"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "7 - " & ChrW(1505) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    ' 8. Clients
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C18").Left, wsMgmt.Range("C18").Top, 180, 40)
    shp.Name = "btnNav_Clients"
    shp.Fill.ForeColor.RGB = RGB(0, 150, 80)
    shp.TextFrame2.TextRange.Text = "8 - " & ChrW(1512) & ChrW(1513) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 12
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    ...
    Set shp = wsMgmt.Shapes.AddShape(msoShapeRoundedRectangle, wsMgmt.Range("C21").Left, wsMgmt.Range("C21").Top, 180, 40)
    shp.Name = "btnNav_Home"
    shp.Fill.ForeColor.RGB = RGB(0, 70, 140)
    shp.TextFrame2.TextRange.Text = ChrW(1495) & ChrW(1494) & ChrW(1493) & ChrW(1512) & " " & ChrW(1500) & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1497)
    shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
    shp.TextFrame2.TextRange.Font.Size = 14
    shp.TextFrame2.TextRange.Font.Bold = msoTrue
    shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
    shp.OnAction = "A00_SetupMainSheet"
    
    ' Add a Back button to all 8 sheets
    Dim shtNames As Variant, wsTemp As Worksheet
    ...
    Dim iIdx As Long
    For iIdx = 0 To UBound(shtNames)
        On Error Resume Next
        Set wsTemp = ThisWorkbook.Worksheets(shtNames(iIdx))
        Err.Clear
        On Error GoTo 0
        If Not wsTemp Is Nothing Then
            ...
            Next shp
            Err.Clear
            Set shp = wsTemp.Shapes.AddShape(msoShapeRoundedRectangle, wsTemp.Range("D2").Left, wsTemp.Range("D2").Top, 120, 30)
            shp.Name = "btnNav_Dashboard"
            shp.Fill.ForeColor.RGB = RGB(180, 50, 50) ' Red back button
            shp.TextFrame2.TextRange.Text = ChrW(1495) & ChrW(1494) & ChrW(1493) & ChrW(1512) & " " & ChrW(1500) & _
        ChrW(1514) & ChrW(1508) & ChrW(1512) & ChrW(1497) & ChrW(1496) & " " & ChrW(1492) & ChrW(1492) & ChrW(1490) & ChrW(1491) & _
        ChrW(1512) & ChrW(1493) & ChrW(1514)
            shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB = RGB(255, 255, 255)
            shp.TextFrame2.TextRange.Font.Size = 10
            shp.TextFrame2.TextRange.Font.Bold = msoTrue
            shp.TextFrame2.TextRange.ParagraphFormat.Alignment = msoAlignCenter
            shp.OnAction = "NavSettings_Dashboard"
        End If
    Next iIdx
End Sub
' ============================================================================
...
50     Set dict = CreateObject("Scripting.Dictionary")
    ...
    ' Search through all base sheets (H_BASE)
...
' ============================================================================
' HELPER: Read one system message from the messages sheet, never raising.
' Returns "" when the sheet is missing (ws Is Nothing) or the cell is
' unreadable. Error handlers use this so they can never fail while trying
' to report a failure.
' ============================================================================
Private Function MsgText(ByVal ws As Worksheet, ByVal rowIdx As Long) As String
    On Error Resume Next
    If Not ws Is Nothing Then MsgText = CStr(ws.Cells(rowIdx, 1).Value)
End Function
...
' ---- HELPER: Navigate to Settings sheets ----
Private Function IsParameterSheet(ByVal sName As String) As Boolean
...
Private Sub SettingsGoToSheet(shtName As String)
On Error Resume Next
ThisWorkbook.Unprotect "Z961814r"
Dim ws As Worksheet
...
ws.Activate
Application.GoTo ws.Range("A1")
' V3.33: just leave the settings sheet UNPROTECTED while it is open.
' Re-protecting here with DrawingObjects:=False proved unreliable in practice:
' the identical Protect call restores the dropdown arrow when run by hand, but
' not when run from inside this sub, for reasons that never became clear.
' Unprotecting alone always restores the arrow (confirmed by hand), and the
' settings sheets exist to be edited, so protection buys nothing here. The
' workbook re-hides and re-secures everything when you navigate back home.
ws.Unprotect "Z961814r"
ws.Unprotect "1234"
End If
End Sub

Public Sub NavSettings_Dashboard()
10  SettingsGoToSheet MANAGEMENT_SHEET_NAME()
End Sub
Public Sub NavSettings_FieldMap()
10  SettingsGoToSheet H_SET_FIELDMAP()
End Sub
Public Sub NavSettings_BranchName()
10  SettingsGoToSheet H_SET_BRANCHES()
End Sub
Public Sub NavSettings_Params()
10  SettingsGoToSheet H_SET_PARAMS()
End Sub
Public Sub NavSettings_ReasonCode()
10  SettingsGoToSheet H_SET_REASONS()
End Sub
Public Sub NavSettings_PeriodLists()
10  SettingsGoToSheet H_SET_PERIODS()
End Sub
Public Sub NavSettings_Messages()
10  SettingsGoToSheet H_SET_MESSAGES()
End Sub
Public Sub NavSettings_Permissions()
10  SettingsGoToSheet H_SET_PERMISSIONS()
End Sub
Public Sub NavSettings_Clients()
10  SettingsGoToSheet H_SET_CLIENTS()
End Sub
Public Sub NavSettings_Home()
    On Error Resume Next
    Application.GoTo ThisWorkbook.Worksheets(CONTROL_SHEET_NAME()).Range("G10")
    DoEvents
...
Private Function IsProtectedSheet(ByVal sName As String, ByVal refBaseKeep As String, ByVal refBaseKeepOld As String, ByVal refReviewKeep As String) As Boolean
    ...
    If sName = MATACH_SHEET_NAME() Then IsProtectedSheet = True: Exit Function
    If sName = NC_MASTER_SHEET_NAME() Then IsProtectedSheet = True: Exit Function
    If sName = refBaseKeep Then IsProtectedSheet = True: Exit Function
    If sName = refBaseKeepOld Then IsProtectedSheet = True: Exit Function
    If sName = refReviewKeep Then IsProtectedSheet = True: Exit Function
    
    If sName = H_SET_FIELDMAP() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_BRANCHES() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_PERIODS() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_MESSAGES() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_REASONS() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_PERMISSIONS() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_CLIENTS() Then IsProtectedSheet = True: Exit Function
    If sName = H_SET_PARAMS() Then IsProtectedSheet = True: Exit Function
...
Public Sub Z_FixMessages()
    Dim wsTemp As Worksheet
    On Error Resume Next
    Set wsTemp = ThisWorkbook.Worksheets(H_SET_MESSAGES())
    On Error GoTo 0
    If wsTemp Is Nothing Then
        MsgBox "Cannot find messages sheet!", vbCritical
        Exit Sub
    End If
    
    wsTemp.Cells(2, 1).Value = ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1497) & ChrW(1497) & ChrW(1501) & " " & "-" & " " & ChrW(1504) & ChrW(1502) & ChrW(1510) & ChrW(1488) & ChrW(1493) & " "
    wsTemp.Cells(3, 1).Value = " " & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1493) & ChrW(1514)
    wsTemp.Cells(4, 1).Value = ChrW(1492) & ChrW(1505) & ChrW(1514) & ChrW(1497) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492)
    wsTemp.Cells(5, 1).Value = ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1492) & " "
    wsTemp.Cells(6, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " "
    wsTemp.Cells(7, 1).Value = ChrW(1513) & ChrW(1500) & ChrW(1489) & ":" & " "
    wsTemp.Cells(8, 1).Value = ChrW(1488) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1512) & " " & ChrW(1500) & _
        ChrW(1508) & ChrW(1504) & ChrW(1497) & " " & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491)
    wsTemp.Cells(9, 1).Value = ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1500) & ChrW(1497) & ChrW(1497) & _
        ChrW(1513) & ChrW(1501) & " " & ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & _
        " " & ChrW(1493) & ChrW(1500) & ChrW(1497) & ChrW(1497) & ChrW(1510) & ChrW(1512) & " " & ChrW(1491) & ChrW(1493) & _
        ChrW(1495) & ChrW(1493) & ChrW(1514) & "?"
    wsTemp.Cells(10, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & _
        ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & _
        ChrW(1489) & ChrW(1497) & ChrW(1514) & ":" & " "
    wsTemp.Cells(11, 1).Value = ChrW(1513) & ChrW(1490) & ChrW(1497) & ChrW(1488) & ChrW(1492) & " " & ChrW(1489) & _
        ChrW(1506) & ChrW(1491) & ChrW(1499) & ChrW(1493) & ChrW(1503) & " " & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & _
        ChrW(1492) & ":" & " "
    wsTemp.Cells(12, 1).Value = ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & _
        ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & ChrW(1489) & ChrW(1497) & ChrW(1514) & " " & ChrW(1492) & ChrW(1493) & _
        ChrW(1511) & ChrW(1501) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & "!"
    wsTemp.Cells(13, 1).Value = "1" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1492) & ChrW(1506) & _
        ChrW(1500) & ChrW(1497) & ChrW(1514) & " " & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & ChrW(1513) & _
        ChrW(1500) & ChrW(1497) & ChrW(1508) & ChrW(1492) & " " & ChrW(1506) & ChrW(1491) & ChrW(1499) & ChrW(1504) & ChrW(1497) & _
        "?"
    wsTemp.Cells(14, 1).Value = "2" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1513) & ChrW(1502) & _
        ChrW(1512) & ChrW(1514) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & _
        ChrW(1493) & ChrW(1491) & " " & ChrW(1492) & ChrW(1488) & ChrW(1495) & ChrW(1512) & ChrW(1493) & ChrW(1503) & " " & "(" & _
        ChrW(1492) & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & ChrW(1495) & _
        ChrW(1493) & ChrW(1489) & ChrW(1512) & ChrW(1514) & " " & ChrW(1494) & ChrW(1493) & " " & ChrW(1497) & ChrW(1502) & _
        ChrW(1495) & ChrW(1511) & ChrW(1493) & " " & ChrW(1489) & ChrW(1514) & ChrW(1492) & ChrW(1500) & ChrW(1497) & ChrW(1498) & _
        " " & ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491) & ")"
    wsTemp.Cells(15, 1).Value = "3" & "." & " " & ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1489) & ChrW(1495) & _
        ChrW(1512) & ChrW(1514) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1508) & ChrW(1512) & ChrW(1496) & ChrW(1497) & " " & _
        ChrW(1492) & ChrW(1506) & ChrW(1497) & ChrW(1489) & ChrW(1493) & ChrW(1491) & " " & ChrW(1492) & ChrW(1504) & ChrW(1491) & _
        ChrW(1512) & ChrW(1513) & " " & ChrW(1489) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & _
        " " & ChrW(1491) & ChrW(1507) & " " & ChrW(1492) & ChrW(1489) & ChrW(1497) & ChrW(1514) & "?"
    wsTemp.Cells(16, 1).Value = "4" & "." & " " & ChrW(1500) & ChrW(1495) & ChrW(1509) & " " & ChrW(1506) & ChrW(1500) & _
        " " & "O" & "K" & " " & ChrW(1500) & ChrW(1492) & ChrW(1502) & ChrW(1513) & ChrW(1498)
    wsTemp.Cells(17, 1).Value = ChrW(1492) & ChrW(1488) & ChrW(1501) & " " & ChrW(1500) & ChrW(1492) & ChrW(1510) & _
        ChrW(1497) & ChrW(1490) & " " & ChrW(1492) & ChrW(1493) & ChrW(1491) & ChrW(1506) & ChrW(1492) & " " & ChrW(1494) & _
        ChrW(1493) & " " & ChrW(1513) & ChrW(1493) & ChrW(1489) & "?"
    MsgBox "Messages populated successfully!", vbInformation
End Sub
...
Public Sub DemoModeRestricted()
    MsgBoxU ChrW(1492) & ChrW(1488) & ChrW(1508) & ChrW(1513) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1494) & _
        ChrW(1502) & ChrW(1497) & ChrW(1504) & ChrW(1492) & " " & ChrW(1489) & ChrW(1490) & ChrW(1512) & ChrW(1505) & ChrW(1492) & _
        " " & ChrW(1492) & ChrW(1502) & ChrW(1500) & ChrW(1488) & ChrW(1492) & ".", vbInformation, ChrW(1502) & ChrW(1506) & _
        ChrW(1512) & ChrW(1499) & ChrW(1514) & " " & ChrW(1491) & ChrW(1502) & ChrW(1493)
End Sub
...
Public Sub FixExcel()
    ...
    Application.StatusBar = False
    MsgBoxU ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1493) & ChrW(1514) & " " & ChrW(1488) & ChrW(1511) & _
        ChrW(1505) & ChrW(1500) & " " & ChrW(1488) & ChrW(1493) & ChrW(1508) & ChrW(1505) & ChrW(1493) & " " & ChrW(1489) & _
        ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & ".", vbInformation
End Sub
...
Public Sub CreateInstructionSheets()
    ...
    sInstall = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1514) & _
        ChrW(1511) & ChrW(1504) & ChrW(1492)
    sOp = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1514) & ChrW(1508) & _
        ChrW(1506) & ChrW(1493) & ChrW(1500)
    ...
    ' --- Setup Install Sheet ---
    With wsInstall
        .Range("B2").Value = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & " " & ChrW(1492) & _
        ChrW(1514) & ChrW(1511) & ChrW(1504) & ChrW(1492) & " " & ChrW(1502) & ChrW(1492) & ChrW(1497) & ChrW(1512) & ChrW(1493) & _
        ChrW(1514) & " " & ChrW(1502) & ChrW(1511) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1512) & " " & ChrW(1492) & _
        ChrW(1488) & ChrW(1497) & ChrW(1504) & ChrW(1496) & ChrW(1512) & ChrW(1504) & ChrW(1496)
        .Range("B2").Font.Size = 24
        .Range("B2").Font.Bold = True
        .Range("B2").Font.Color = RGB(0, 51, 102)
        
        .Range("B4").Value = "" & ChrW(1513) & ChrW(1489) & _
        " " & ChrW(1513) & ChrW(1500) & ChrW(1499) & ChrW(1501) & ", " & ChrW(1489) & ChrW(1510) & ChrW(1506) & ChrW(1493) & ":"
        .Range("B4").Font.Size = 14
        .Range("B4").Font.Bold = True
        
        .Range("B6").Value = "1. " & ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1492) & " " & ChrW(1500) & _
        ChrW(1502) & ChrW(1495) & ChrW(1513) & ChrW(1489) & " (" & ChrW(1495) & ChrW(1493) & ChrW(1489) & ChrW(1492) & "):"
        .Range("C6").Value = ChrW(1499) & ChrW(1488) & ChrW(1503) & " " & ChrW(1489) & ChrW(1491) & ChrW(1508) & ChrW(1491) & _
        ChrW(1508) & ChrW(1503) & ", " & ChrW(1500) & ChrW(1495) & ChrW(1510) & ChrW(1493) & " " & ChrW(1500) & ChrW(1502) & _
        ChrW(1506) & ChrW(1500) & ChrW(1492) & " " & ChrW(1506) & ChrW(1500) & " '" & ChrW(1511) & ChrW(1493) & ChrW(1489) & _
        ChrW(1509) & "' (File) -> '" & ChrW(1513) & ChrW(1502) & ChrW(1497) & ChrW(1512) & ChrW(1492) & " " & ChrW(1489) & _
        ChrW(1513) & ChrW(1501) & "' (Save As) -> '" & ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1491) & " " & ChrW(1506) & _
        ChrW(1493) & ChrW(1514) & ChrW(1511) & "' (Download a Copy)."
        .Range("B7").Value = "2. " & ChrW(1489) & ChrW(1497) & ChrW(1496) & ChrW(1493) & ChrW(1500) & " " & ChrW(1495) & _
        ChrW(1505) & ChrW(1497) & ChrW(1502) & ChrW(1514) & " " & ChrW(1488) & ChrW(1489) & ChrW(1496) & ChrW(1495) & ChrW(1492) & _
        " (Unblock):"
        .Range("C7").Value = ChrW(1489) & ChrW(1502) & ChrW(1495) & ChrW(1513) & ChrW(1489) & ", " & ChrW(1500) & ChrW(1495) & _
        ChrW(1510) & ChrW(1493) & " " & ChrW(1511) & ChrW(1500) & ChrW(1497) & ChrW(1511) & " " & ChrW(1497) & ChrW(1502) & _
        ChrW(1504) & ChrW(1497) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1492) & ChrW(1511) & ChrW(1493) & ChrW(1489) & _
        ChrW(1509) & " " & ChrW(1513) & ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1491) & ChrW(1514) & ChrW(1501) & " -> '" & _
        ChrW(1502) & ChrW(1488) & ChrW(1508) & ChrW(1497) & ChrW(1497) & ChrW(1504) & ChrW(1497) & ChrW(1501) & _
        "' (Properties) -> " & ChrW(1505) & ChrW(1502) & ChrW(1504) & ChrW(1493) & " V " & ChrW(1489) & ChrW(1514) & ChrW(1497) & _
        ChrW(1489) & ChrW(1514) & " '" & ChrW(1489) & ChrW(1496) & ChrW(1500) & " " & ChrW(1495) & ChrW(1505) & ChrW(1497) & _
        ChrW(1502) & ChrW(1492) & "' (Unblock) " & ChrW(1493) & ChrW(1500) & ChrW(1495) & ChrW(1510) & ChrW(1493) & " " & _
        ChrW(1488) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1512) & "."
        .Range("B8").Value = "3. " & ChrW(1488) & ChrW(1497) & ChrW(1513) & ChrW(1493) & ChrW(1512) & " " & ChrW(1502) & _
        ChrW(1488) & ChrW(1511) & ChrW(1512) & ChrW(1493) & ":"
        .Range("C8").Value = ChrW(1508) & ChrW(1514) & ChrW(1495) & ChrW(1493) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & ChrW(1489) & ChrW(1488) & ChrW(1511) & ChrW(1505) & _
        ChrW(1500) & " " & ChrW(1489) & ChrW(1502) & ChrW(1495) & ChrW(1513) & ChrW(1489) & " (" & ChrW(1500) & ChrW(1488) & " " & _
        ChrW(1489) & ChrW(1491) & ChrW(1508) & ChrW(1491) & ChrW(1508) & ChrW(1503) & "!). " & ChrW(1488) & ChrW(1501) & " " & _
        ChrW(1502) & ChrW(1493) & ChrW(1508) & ChrW(1497) & ChrW(1506) & " " & ChrW(1508) & ChrW(1505) & " " & ChrW(1510) & _
        ChrW(1492) & ChrW(1493) & ChrW(1489) & " " & ChrW(1500) & ChrW(1502) & ChrW(1506) & ChrW(1500) & ChrW(1492) & ", " & _
        ChrW(1500) & ChrW(1495) & ChrW(1510) & ChrW(1493) & " " & ChrW(1506) & ChrW(1500) & " '" & ChrW(1492) & ChrW(1508) & _
        ChrW(1493) & ChrW(1498) & " " & ChrW(1514) & ChrW(1493) & ChrW(1499) & ChrW(1503) & " " & ChrW(1500) & ChrW(1494) & _
        ChrW(1502) & ChrW(1497) & ChrW(1503) & "' (Enable Content)."
        .Range("B9").Value = "4. " & ChrW(1492) & ChrW(1490) & ChrW(1491) & ChrW(1512) & ChrW(1514) & " " & ChrW(1494) & _
        ChrW(1502) & ChrW(1504) & ChrW(1497) & ChrW(1501) & ":"
        .Range("C9").Value = ChrW(1489) & ChrW(1502) & ChrW(1505) & ChrW(1498) & " " & ChrW(1492) & ChrW(1512) & ChrW(1488) & _
        ChrW(1513) & ChrW(1497) & ", " & ChrW(1489) & ChrW(1495) & ChrW(1512) & ChrW(1493) & " " & ChrW(1513) & ChrW(1504) & _
        ChrW(1514) & " " & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505) & " " & ChrW(1493) & ChrW(1513) & ChrW(1504) & _
        ChrW(1492) & " " & ChrW(1504) & ChrW(1493) & ChrW(1499) & ChrW(1495) & ChrW(1497) & ChrW(1514) & " " & ChrW(1502) & _
        ChrW(1492) & ChrW(1514) & ChrW(1508) & ChrW(1512) & ChrW(1497) & ChrW(1496) & ChrW(1497) & ChrW(1501) & " " & ChrW(1492) & _
        ChrW(1504) & ChrW(1508) & ChrW(1514) & ChrW(1495) & ChrW(1497) & ChrW(1501) & " " & ChrW(1499) & ChrW(1491) & ChrW(1497) & _
        " " & ChrW(1500) & ChrW(1492) & ChrW(1514) & ChrW(1495) & ChrW(1497) & ChrW(1500) & " " & ChrW(1500) & ChrW(1492) & _
        ChrW(1513) & ChrW(1514) & ChrW(1502) & ChrW(1513) & " " & ChrW(1489) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & _
        ChrW(1514) & "."
    ...
    ' --- Setup Op Sheet ---
    With wsOp
        .Range("B2").Value = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & " " & ChrW(1514) & _
        ChrW(1508) & ChrW(1506) & ChrW(1493) & ChrW(1500) & " " & ChrW(1500) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & _
        ChrW(1514)
        .Range("B2").Font.Size = 24
        .Range("B2").Font.Bold = True
        .Range("B2").Font.Color = RGB(0, 51, 102)
        
        .Range("B4").Value = ChrW(1500) & ChrW(1492) & ChrW(1500) & ChrW(1503) & " " & ChrW(1508) & ChrW(1497) & ChrW(1512) & _
        ChrW(1493) & ChrW(1496) & " " & ChrW(1513) & ChrW(1500) & ChrW(1489) & ChrW(1497) & " " & ChrW(1492) & ChrW(1506) & _
        ChrW(1489) & ChrW(1493) & ChrW(1491) & ChrW(1492) & " " & ChrW(1506) & ChrW(1501) & " 7 " & ChrW(1499) & ChrW(1508) & _
        ChrW(1514) & ChrW(1493) & ChrW(1512) & ChrW(1497) & " " & ChrW(1492) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & _
        ChrW(1514) & " " & ChrW(1492) & ChrW(1512) & ChrW(1488) & ChrW(1513) & ChrW(1497) & ChrW(1497) & ChrW(1501) & ":"
        .Range("B4").Font.Size = 14
        .Range("B4").Font.Bold = True
        
        .Range("B6").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 1 - " & ChrW(1489) & ChrW(1491) & _
        ChrW(1497) & ChrW(1511) & ChrW(1514) & " " & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & _
        ":"
        .Range("C6").Value = ChrW(1489) & ChrW(1493) & ChrW(1491) & ChrW(1511) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1504) & ChrW(1514) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1493) & ChrW(1502) & _
        ChrW(1514) & ChrW(1512) & ChrW(1497) & ChrW(1506) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1513) & ChrW(1490) & _
        ChrW(1497) & ChrW(1488) & ChrW(1493) & ChrW(1514) & " " & ChrW(1488) & ChrW(1493) & " " & ChrW(1508) & ChrW(1506) & _
        ChrW(1512) & ChrW(1497) & ChrW(1501) & ". " & ChrW(1504) & ChrW(1497) & ChrW(1514) & ChrW(1503) & " " & ChrW(1500) & _
        ChrW(1514) & ChrW(1511) & ChrW(1503) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1513) & ChrW(1490) & _
        ChrW(1497) & ChrW(1488) & ChrW(1493) & ChrW(1514) & " " & ChrW(1497) & ChrW(1513) & ChrW(1497) & ChrW(1512) & ChrW(1493) & _
        ChrW(1514) & ", " & ChrW(1488) & ChrW(1493) & " " & ChrW(1500) & ChrW(1513) & ChrW(1500) & ChrW(1493) & ChrW(1495) & " " & _
        ChrW(1500) & ChrW(1489) & ChrW(1499) & ChrW(1497) & ChrW(1512) & " " & ChrW(1500) & ChrW(1511) & ChrW(1489) & ChrW(1500) & _
        ChrW(1514) & " " & ChrW(1492) & ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & _
        ". " & ChrW(1500) & ChrW(1488) & ChrW(1495) & ChrW(1512) & " " & ChrW(1502) & ChrW(1499) & ChrW(1503) & " " & ChrW(1497) & _
        ChrW(1513) & " " & ChrW(1500) & ChrW(1492) & ChrW(1511) & ChrW(1497) & ChrW(1513) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1502) & _
        ChrW(1495) & ChrW(1491) & ChrW(1513) & "."
        .Range("B7").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 2 - " & ChrW(1497) & ChrW(1497) & _
        ChrW(1513) & ChrW(1493) & ChrW(1501) & " " & ChrW(1493) & ChrW(1491) & ChrW(1493) & ChrW(34) & ChrW(1495) & ChrW(1493) & _
        ChrW(1514) & ":"
        .Range("C7").Value = ChrW(1502) & ChrW(1495) & ChrW(1497) & ChrW(1500) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1514) & ChrW(1497) & ChrW(1511) & ChrW(1493) & ChrW(1504) & ChrW(1497) & ChrW(1501) & " " & ChrW(1513) & _
        ChrW(1492) & ChrW(1494) & ChrW(1504) & ChrW(1514) & ChrW(1501) & " " & ChrW(1493) & ChrW(1502) & ChrW(1499) & ChrW(1497) & _
        ChrW(1503) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505) & " " & ChrW(1500) & ChrW(1506) & ChrW(1489) & ChrW(1493) & _
        ChrW(1491) & ChrW(1492) & "."
        .Range("B8").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 3 - " & ChrW(1492) & ChrW(1510) & _
        ChrW(1490) & " " & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & _
        " " & ChrW(1491) & ChrW(1497) & ChrW(1493) & ChrW(1493) & ChrW(1495) & ":"
        .Range("C8").Value = ChrW(1502) & ChrW(1510) & ChrW(1497) & ChrW(1490) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & " " & ChrW(1492) & _
        ChrW(1491) & ChrW(1497) & ChrW(1493) & ChrW(1493) & ChrW(1495) & " " & ChrW(1500) & ChrW(1489) & ChrW(1491) & ChrW(1497) & _
        ChrW(1511) & ChrW(1492) & " " & ChrW(1493) & ChrW(1510) & ChrW(1508) & ChrW(1497) & ChrW(1497) & ChrW(1492) & " " & _
        ChrW(1504) & ChrW(1493) & ChrW(1495) & ChrW(1492) & " (" & ChrW(1500) & ChrW(1502) & ChrW(1511) & ChrW(1512) & _
        ChrW(1492) & " " & ChrW(1513) & ChrW(1492) & ChrW(1493) & ChrW(1505) & ChrW(1514) & ChrW(1512) & ChrW(1493) & ")."
        .Range("B9").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 4 - " & ChrW(1497) & ChrW(1497) & _
        ChrW(1510) & ChrW(1493) & ChrW(1512) & " " & ChrW(1502) & ChrW(1510) & ChrW(1490) & ChrW(1514) & ":"
        .Range("C9").Value = ChrW(1489) & ChrW(1493) & ChrW(1504) & ChrW(1492) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1502) & ChrW(1510) & ChrW(1490) & ChrW(1514) & " " & ChrW(1493) & ChrW(1492) & ChrW(1491) & ChrW(1493) & _
        ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1492) & ChrW(1505) & ChrW(1493) & ChrW(1508) & ChrW(1497) & ChrW(1497) & _
        ChrW(1501) & " " & ChrW(1513) & ChrW(1500) & " " & ChrW(1492) & ChrW(1502) & ChrW(1506) & ChrW(1512) & ChrW(1499) & _
        ChrW(1514) & "."
        .Range("B10").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 5 - " & ChrW(1513) & ChrW(1502) & _
        ChrW(1497) & ChrW(1512) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & ":"
        .Range("C10").Value = ChrW(1513) & ChrW(1493) & ChrW(1502) & ChrW(1512) & " " & ChrW(1488) & ChrW(1514) & " " & _
        ChrW(1492) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & " " & _
        ChrW(1489) & ChrW(1491) & ChrW(1497) & ChrW(1505) & ChrW(1511) & " " & ChrW(1513) & ChrW(1500) & " " & ChrW(1492) & _
        ChrW(1502) & ChrW(1495) & ChrW(1513) & ChrW(1489) & "."
        .Range("B11").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 6 - " & ChrW(1510) & ChrW(1508) & _
        ChrW(1497) & ChrW(1497) & ChrW(1492) & " " & ChrW(1489) & ChrW(1491) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & _
        " " & ChrW(1513) & ChrW(1502) & ChrW(1493) & ChrW(1512) & ChrW(1497) & ChrW(1501) & ":"
        .Range("C11").Value = ChrW(1508) & ChrW(1493) & ChrW(1514) & ChrW(1495) & " " & ChrW(1500) & ChrW(1510) & ChrW(1508) & _
        ChrW(1497) & ChrW(1497) & ChrW(1492) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1491) & ChrW(1493) & _
        ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1513) & ChrW(1513) & ChrW(1502) & ChrW(1512) & ChrW(1504) & ChrW(1493) & _
        " " & ChrW(1511) & ChrW(1493) & ChrW(1491) & ChrW(1501) & " " & ChrW(1489) & ChrW(1502) & ChrW(1495) & ChrW(1513) & _
        ChrW(1489) & "."
        .Range("B12").Value = ChrW(1499) & ChrW(1508) & ChrW(1514) & ChrW(1493) & ChrW(1512) & " 7 - " & ChrW(1500) & ChrW(1511) & _
        ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & ChrW(1495) & ChrW(1491) & ChrW(1513) & ChrW(1497) & ChrW(1501) & _
        ":"
        .Range("C12").Value = ChrW(1492) & ChrW(1508) & ChrW(1511) & ChrW(1514) & " " & ChrW(1491) & ChrW(1493) & ChrW(1495) & _
        " " & ChrW(1513) & ChrW(1500) & " " & ChrW(1500) & ChrW(1511) & ChrW(1493) & ChrW(1495) & ChrW(1493) & ChrW(1514) & " " & _
        ChrW(1513) & ChrW(1500) & ChrW(1488) & " " & ChrW(1492) & ChrW(1497) & ChrW(1493) & " " & ChrW(1508) & ChrW(1506) & _
        ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & ChrW(1514) & ChrW(1511) & ChrW(1493) & ChrW(1508) & _
        ChrW(1514) & " " & ChrW(1492) & ChrW(1489) & ChrW(1505) & ChrW(1497) & ChrW(1505) & " " & ChrW(1493) & ChrW(1499) & _
        ChrW(1503) & " " & ChrW(1508) & ChrW(1506) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1501) & " " & ChrW(1489) & _
        ChrW(1513) & ChrW(1504) & ChrW(1492) & " " & ChrW(1492) & ChrW(1504) & ChrW(1493) & ChrW(1499) & ChrW(1495) & ChrW(1497) & _
        ChrW(1514) & "."
        ...
        btnOp.TextFrame2.TextRange.Text = ChrW(1492) & ChrW(1489) & ChrW(1504) & ChrW(1514) & ChrW(1497) & ", " & ChrW(1488) & _
        ChrW(1500) & " " & ChrW(1514) & ChrW(1510) & ChrW(1497) & ChrW(1490) & " " & ChrW(1500) & ChrW(1497) & " " & ChrW(1497) & _
        ChrW(1493) & ChrW(1514) & ChrW(1512) & " " & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & _
        " " & ChrW(1494) & ChrW(1492)
    ...
    wsInstall.Activate
    MsgBoxU ChrW(1492) & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1504) & ChrW(1493) & ChrW(1514) & " " & ChrW(1504) & ChrW(1493) & ChrW(1510) & ChrW(1512) & ChrW(1493) & " " & ChrW(1489) & ChrW(1492) & ChrW(1510) & ChrW(1500) & ChrW(1495) & ChrW(1492) & "! " & ChrW(1513) & ChrW(1502) & ChrW(1493) & ChrW(1512) & " " & ChrW(1488) & ChrW(1514) & " " & ChrW(1492) & ChrW(1511) & ChrW(1493) & ChrW(1489) & ChrW(1509) & " " & ChrW(1499) & ChrW(1506) & ChrW(1514) & " " & ChrW(1499) & ChrW(1513) & ChrW(1492) & ChrW(1493) & ChrW(1488) & " " & ChrW(1506) & ChrW(1500) & " " & ChrW(1490) & ChrW(1497) & ChrW(1500) & ChrW(1497) & ChrW(1493) & ChrW(1503) & " " & ChrW(1492) & ChrW(1492) & ChrW(1514) & ChrW(1511) & ChrW(1504) & ChrW(1492) & ".", vbInformation
    On Error GoTo 0
End Sub
...
Public Sub HideInstallInst()
    ...
    Dim sInstall As String
    sInstall = ChrW(1492) & ChrW(1493) & ChrW(1512) & ChrW(1488) & ChrW(1493) & ChrW(1514) & "_" & ChrW(1492) & ChrW(1514) & ChrW(1511) & ChrW(1504) & ChrW(1492)
...
Public Sub SetupInstructionsSheet()
    ...
    Set shp = wsInst.Shapes.AddShape(msoShapeRoundedRectangle, wsInst.Range("B20").Left, wsInst.Range("B20").Top, 250, 40)
...
Public Sub ToggleDisplayMode()
    ...
    ' V3.28: was "1234" on both calls, which left the home sheet protected with
    ' a password every other routine fails to unprotect.
    wsMain.Unprotect Password:="Z961814r"
...
' -------------------------------------------------------------------------
' V3.39: FitHomeToScreen - what the "התאמת מסך" button should have done.
' ToggleDisplayMode only flips zoom between 80 and 100, and A00 fitted a FIXED
' range (A1:L20), so on a laptop with a different resolution the home page
' overflowed and had to be scrolled. This measures the actual extent of every
' shape on the home sheet and zooms that whole range to fit the window, so it
' adapts to any screen size.
' -------------------------------------------------------------------------
Public Sub FitHomeToScreen()
    On Error Resume Next
    Dim wsMain As Worksheet
    Dim shp As Shape
    ...
    wsMain.Activate
    ' Start from a sensible minimum, then grow to cover every shape/button.
    maxRow = 20
    maxCol = 12
    For Each shp In wsMain.Shapes
        ' V3.40: ignore stray shapes parked far off-screen (e.g. a leftover
        ' "Drop Down" form control at column ~thousands) - they would blow up
        ' the fit range and shrink everything to nothing.
        If shp.BottomRightCell.Column <= 40 And shp.BottomRightCell.Row <= 60 Then
        ...
        End If
    Next shp
    ' One row/column of breathing room.
    maxRow = maxRow + 1
    maxCol = maxCol + 1

    wsMain.Range(wsMain.Cells(1, 1), wsMain.Cells(maxRow, maxCol)).Select
    ActiveWindow.Zoom = True          ' fit the selected range to the window
    ...
    ' V3.41: show the top-right (right side + the fit button) and rest on F10.
    wsMain.Range("F10").Select
