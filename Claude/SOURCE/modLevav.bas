...
NextRow:
    ...
    ' Header row: bold, centered, wrapped (so narrow columns can fit long names)
    wsRev.Rows(1).Font.Bold = True
    wsRev.Rows(1).WrapText = True
    wsRev.Rows(1).HorizontalAlignment = xlCenter
    wsRev.Rows(1).VerticalAlignment = xlCenter
    wsRev.Rows(1).RowHeight = 50

    ' Compress every column: AutoFit, then cap to max width
    Dim colIdx As Long
    Const MAX_COL_WIDTH As Single = 14
    Const MIN_COL_WIDTH As Single = 6
    For colIdx = 1 To cnt + 4
        wsRev.Columns(colIdx).AutoFit
        If wsRev.Columns(colIdx).ColumnWidth > MAX_COL_WIDTH Then wsRev.Columns(colIdx).ColumnWidth = MAX_COL_WIDTH
        If wsRev.Columns(colIdx).ColumnWidth < MIN_COL_WIDTH Then wsRev.Columns(colIdx).ColumnWidth = MIN_COL_WIDTH
    Next colIdx
    ' Source-row column can be narrow
    wsRev.Columns(1).ColumnWidth = 7
    ' Reason column slightly wider for readability
    wsRev.Columns(cnt + 2).ColumnWidth = 18
    ' Action and Fix columns special-cased (wider, color-coded)
    wsRev.Cells(1, actionCol).Interior.Color = RGB(255, 165, 0)
    wsRev.Cells(1, actionCol + 1).Interior.Color = RGB(255, 165, 0)
    wsRev.Columns(actionCol).ColumnWidth = 14
    wsRev.Columns(actionCol + 1).ColumnWidth = 22
    If outRow > 2 Then
    ...
    ' Red "סיימתי לעדכן" button — positioned BELOW the last data row, columns A:D
    Dim shpBtn As Shape
    Dim btnRow As Long
    btnRow = outRow + 1
    With wsRev.Range(wsRev.Cells(btnRow, 1), wsRev.Cells(btnRow + 1, 4))
        Set shpBtn = wsRev.Shapes.AddShape(msoShapeRoundedRectangle, .Left, .Top, .Width, .Height)
    End With
...
' C:\LEVAV PROJECT\Claude\SOURCE\
Private Function SOURCE_FOLDER() As String
    SOURCE_FOLDER = "C:\LEVAV PROJECT\Claude\SOURCE\"
End Function
