...
End Function
' ============================================================================
' HELPER: Safe read of a message cell.
' Falls back to English text if the sheet/cell is unavailable, so the error
' handler never crashes while trying to report an error.
' ============================================================================
Private Function SafeMsg(ByVal ws As Worksheet, ByVal rowIdx As Long, ByVal colIdx As Long, ByVal fallback As String) As String
    ...
    If ws Is Nothing Then
        ...
        Exit Function
    End If
    v = CStr(ws.Cells(rowIdx, colIdx).Value2)
    If v = "" Then
        SafeMsg = fallback
    Else
        SafeMsg = v
...
End Function
' ============================================================================
' HELPER: Map a reason (Hebrew text or reason code) to a BASE column.
' Replaces the fragile InStr-on-Hebrew-substring logic.
' ============================================================================
Private Function ReasonToColumn(ByVal reasonText As String, ByVal dictHelper As Object) As Long
    On Error Resume Next
    Dim dictReverse As Object
    Dim k As Variant
    Dim code As String
    Set dictReverse = CreateObject("Scripting.Dictionary")
    dictReverse.CompareMode = vbTextCompare
    For Each k In dictHelper.keys
        dictReverse(CStr(dictHelper(k))) = CStr(k)
    Next k
    If dictReverse.Exists(reasonText) Then
        code = UCase$(CStr(dictReverse(reasonText)))
    Else
        code = UCase$(reasonText)
    End If
    Select Case code
        Case "MISSING_AGENT_NAME":            ReasonToColumn = BASE_COL_AGENTNAME
        Case "MISSING_UNDERWRITER_TELLER_NAME", "MISSING_TELLER_NAME": ReasonToColumn = BASE_COL_TELLER
        Case "MISSING_COMPANY_NAME":          ReasonToColumn = BASE_COL_COMPANY
        Case "MISSING_BRANCH_NAME":           ReasonToColumn = BASE_COL_BRANCHNAME
        Case "MISSING_PREMIUM", "PREMIUM_OVER_THRESHOLD", "PREMIUM_NOT_NUMERIC": ReasonToColumn = BASE_COL_PREMIUM
        Case "MISSING_COMPANY_COMMISSION":    ReasonToColumn = BASE_COL_COMMISSION
        Case "MISSING_CUSTOMER_NAME":         ReasonToColumn = BASE_COL_CUSTNAME
        Case "MISSING_CUSTOMER_NUMBER":       ReasonToColumn = BASE_COL_CUSTOMER
        Case "MISSING_POLICY":                ReasonToColumn = BASE_COL_POLICY
        Case "MISSING_ADDENDUM":              ReasonToColumn = BASE_COL_ADDENDUM
        Case Else:                            ReasonToColumn = 0
    End Select
End Function
' ============================================================================
...
1240    MsgBoxU SafeMsg(wsMsgSrc, 2, 19, "Done - found ") & (outRow - 2) & SafeMsg(wsMsgSrc, 3, 19, " issues"), vbInformation
1250    Exit Sub
ERR_HANDLER:
        Dim errLine As Long
        Dim errNum As Long
...
1360    MsgBoxU SafeMsg(wsMsgSrc, 5, 19, "Line: ") & errLine & vbCrLf & SafeMsg(wsMsgSrc, 6, 19, "Err #: ") & errNum & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
End Sub
' ============================================================================
...
1886                If oneFix <> "" Then
                        ' Map reason text to the correct base column via reverse-lookup
                        Dim tgtCol As Long
1887                    tgtCol = ReasonToColumn(reasonText, dictHelper)
1888                    If tgtCol = BASE_COL_PREMIUM Then
1890                        If TryParseVariantNumber(oneFix, fixPrem) Then
1892                            wsBase.Cells(outRow, BASE_COL_PREMIUM).Value = fixPrem
1894                            premVal = fixPrem
1896                        End If
1898                    ElseIf tgtCol = BASE_COL_COMMISSION Then
1900                        If TryParseVariantNumber(oneFix, fixComm) Then
1902                            wsBase.Cells(outRow, BASE_COL_COMMISSION).Value = fixComm
1904                        End If
1906                    ElseIf tgtCol > 0 Then
1908                        wsBase.Cells(outRow, tgtCol).Value = oneFix
1910                    End If
1912                End If
1914            Next fixKey
1916        ElseIf Not dictHasUnhandled Is Nothing Then
1917            If dictHasUnhandled.Exists(CStr(r)) Then wsBase.Cells(outRow, BASE_COL_ISSUE).Value = "UNHANDLED"
1920        End If
...
2005        countRef = wsBaseRef.Cells(wsBaseRef.Rows.Count, 1).End(xlUp).Row - 1
2006        If countRef < 0 Then countRef = 0
2007        GoTo BUILD_COMPARISONS
        End If
...
2570                If oneFix <> "" Then
                        Dim tgtColR As Long
2571                    tgtColR = ReasonToColumn(refReasonText, dictHelper)
2572                    If tgtColR = BASE_COL_PREMIUM Then
2574                        If TryParseVariantNumber(oneFix, fixPrem) Then
2576                            wsBaseRef.Cells(outRow, BASE_COL_PREMIUM).Value = fixPrem
2578                        End If
2580                    ElseIf tgtColR = BASE_COL_COMMISSION Then
2582                        If TryParseVariantNumber(oneFix, fixComm) Then
2584                            wsBaseRef.Cells(outRow, BASE_COL_COMMISSION).Value = fixComm
2586                        End If
2588                    ElseIf tgtColR > 0 Then
2590                        wsBaseRef.Cells(outRow, tgtColR).Value = oneFix
2592                    End If
2594                End If
2596            Next refFixKey
2598        ElseIf Not dictRefHasUnhandled Is Nothing Then
2599            If dictRefHasUnhandled.Exists(CStr(r)) Then wsBaseRef.Cells(outRow, BASE_COL_ISSUE).Value = "UNHANDLED"
2602        End If
...
2960    MsgBoxU SafeMsg(wsMsgSrc2, 5, 19, "Line: ") & errLine & vbCrLf & SafeMsg(wsMsgSrc2, 6, 19, "Err #: ") & errNum & vbCrLf & SafeMsg(wsMsgSrc2, 7, 19, "Step: ") & debugStep & vbCrLf & errSrc & vbCrLf & errDesc, vbCritical
End Sub
' ============================================================================
...
869     wsMgmt.Cells(5, 19).Value = ChrW(1513) & ChrW(1493) & ChrW(1512) & ChrW(1492) & " "
        ...
        ' Reset on every setup so the user sees the warning once per setup run
        wsMgmt.Cells(20, 19).Value = ""
...
230     tmpPath = Environ$("TEMP") & "\"
        Dim uid As String
        Randomize
232     uid = Format$(Now, "hhmmss") & "_" & CLng((Rnd * 9000) + 1000)
        Dim imgTotal As String
240     imgTotal = tmpPath & "levav_total_" & uid & ".gif"
...
500         imgFiles(si * 2 - 1) = tmpPath & "levav_prem_" & uid & "_" & si & ".gif"
510         imgFiles(si * 2) = tmpPath & "levav_comm_" & uid & "_" & si & ".gif"
...
380     rowH = 22
390     If tblRows > 15 Then rowH = 18
392     If tblRows > 20 Then rowH = 16
...
60          Exit Sub
70      End If
        ' Find action column (header contains "peula" = פעולה)
...
670     tempPath = ThisWorkbook.Path & "\" & ChrW(1495) & ChrW(1512) & ChrW(1497) & ChrW(1490) & ChrW(1497) & ChrW(1501) & "_" & wsRev.Name & "_" & Format$(Now, "yyyymmdd_hhmmss") & ".xlsx"
...
940     olMail.Display
        ' Success message: "email huchan be'hatzlacha im X shurot" = מייל הוכן בהצלחה עם X שורות
